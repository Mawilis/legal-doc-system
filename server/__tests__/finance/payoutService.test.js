...
describe('💰 WILSY OS - Cap Table & Payout Service', () => {
  it('should accurately calculate $650M ARR and Investor Equity Value', () => {
    ...
    expect(getCapTable().investors).to.equal(0.2);
  ...
  it('should return correct cap table structure', () => {
    ...
    expect(capTable.founders).to.equal(0.6);
    expect(capTable.investors).to.equal(0.2);
    expect(capTable.employeePool).to.equal(0.2);
    ...
    console.log(
      `  • Founders: ${breakdown.founders.percentage} (${breakdown.founders.shares.toLocaleString()} shares)`
    );
    console.log(
      `  • Investors: ${breakdown.investors.percentage} (${breakdown.investors.shares.toLocaleString()} shares)`
    );
    console.log(
      `  • Employee Pool: ${breakdown.employeePool.percentage} (${breakdown.employeePool.shares.toLocaleString()} shares)`
    );
  });
  ...
  it('should project valuations at different multiples', () => {
    ...
    projections.forEach((p) => {
      console.log(
        `  • ${p.multiple}x: $${(p.valuation / 1e9).toFixed(2)}B (Investor: $${(p.investorValue / 1e9).toFixed(2)}B)`
      );
    });
