...
beforeEach(() => {
  ...
  Valuation.aggregate = jest.fn().mockResolvedValue([{
    trends: [
      { _id: { year: 2026, month: 2, day: 24 }, count: 1, averageValue: 45000000 }
    ],
    byMethod: [
      { _id: 'dcf', count: 2, avgValue: 65000000 }
    ],
    summary: {
      totalValuations: 3,
...
describe('InvestorService - Forensic Worker Verification Suite', () => {
  ...
  describe('Forensic Logging', () => {
    ...
    test('should log multiple forensic events in chain', async () => {
      ...
      const completionCall = calls.find(call => 
        call[0].eventType.includes('complete')
      );
  ...
  describe('Error Handling', () => {
    ...
    test('should handle database errors gracefully', async () => {
      ...
      // Act & Assert
      await expect(
        getInvestorDashboardData(params, TEST_CORRELATION_ID)
      ).rejects.toThrow('FORENSIC_WORKER_FAILED');
  ...
  describe('Economic Metrics', () => {
    ...
    test('should calculate and print risk reduction metrics', () => {
      ...
      console.log(`   Breach probability without forensic: ${breachProbabilityWithoutForensic * 100}%`);
