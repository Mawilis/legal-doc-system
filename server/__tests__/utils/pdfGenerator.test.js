...
jest.mock('../utils/cryptoUtils', () => ({
  ...
  generateKey: (algorithm, seed) => createHash('sha256').update(seed || 'test').digest('hex').substring(0, 32)
}));
...
describe('PDFGenerator - Investor Due Diligence Suite', () => {
  ...
  describe('Tenant Isolation', () => {
    ...
    test('should enforce tenant isolation on document retrieval', async () => {
      ...
      const retrieved = await generator1.retrieveDocument(
        result.documentHash, 
        'tenant-a-12345678'
      );
    ...
    test('should prevent cross-tenant access even with valid hash', async () => {
      ...
      // Act & Assert
      await expect(
        generator.retrieveDocument(result.documentHash, TEST_TENANT_ID)
  ...
  describe('Specialized Document Generation', () => {
    ...
    test('should validate valuation report required fields', async () => {
      ...
      // Act & Assert
      await expect(
        generator.generateValuationReport(invalidData)
      ).rejects.toThrow('Valuation report requires valuationId and companyName');
    });
  ...
  describe('Error Handling & Edge Cases', () => {
    ...
    test('should enforce maximum document size', async () => {
      ...
      // Act & Assert
      await expect(
        generator.generateValuationReport(largeData)
  ...
  describe('Economic Metrics & Investor Evidence', () => {
    ...
    test('should generate investor-grade evidence file', async () => {
      ...
      console.log(`   Evidence path: ${path.join(__dirname, 'evidence', 'pdf-generator-evidence.json')}`);
      console.log(`   Audit entries: ${evidence.auditEntries.length}`);
      console.log(`   SHA256: ${evidence.hash}`);
      
      // Write verification command
      console.log('\n📋 Verification command:');
      console.log(`   jq -c '.auditEntries[]' ${path.join(__dirname, 'evidence', 'pdf-generator-evidence.json')} | sha256sum`);
    });
  });
});
