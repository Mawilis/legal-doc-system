...
export class LegalTrendDetector {
  ...
  classifyTrend(text) {
    ...
    if (text_lower.includes('esg') || text_lower.includes('governance') || 
        text_lower.includes('board') || text_lower.includes('shareholder')) {
      return 'corporate';
    }
    if (text_lower.includes('ai') || text_lower.includes('algorithm') || 
        text_lower.includes('digital') || text_lower.includes('software')) {
      return 'technology';
    }
    if (text_lower.includes('privacy') || text_lower.includes('data') || 
        text_lower.includes('gdpr') || text_lower.includes('popia')) {
      return 'privacy';
    }
    if (text_lower.includes('employee') || text_lower.includes('worker') || 
        text_lower.includes('labor') || text_lower.includes('remote')) {
      return 'employment';
    }
    if (text_lower.includes('contract') || text_lower.includes('supply chain') || 
        text_lower.includes('commercial') || text_lower.includes('transaction')) {
      return 'commercial';
    }
    ...
    Object.entries(data).forEach(([region, topics]) => {
      topics.forEach(topic => {
        trends.push({
          ...
          effectiveDate: new Date(Date.now() + Math.random() * 180 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
