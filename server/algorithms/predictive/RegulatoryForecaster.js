...
export class RegulatoryForecaster {
  ...
  extractFeatures(history, jurisdiction) {
    ...
    // Major changes ratio
    features.push(
      history.reduce((sum, year) => sum + year.major, 0) /
        history.reduce((sum, year) => sum + year.changes, 0)
    );
  ...
  calculateConfidence(forecast, history) {
    ...
    const variance =
      forecast.reduce((sum, f) => sum + Math.pow(f.probability - avgProbability, 2), 0) /
      forecast.length;
