...
const AuditTrailSchema = new Schema(
  {
    ...
    actor: {
      tenantId: {
        ...
        index: true,
        match: [
          /^tenant_[a-zA-Z0-9_]{8,32}$/,
          'Tenant ID must match pattern: tenant_[8-32 alphanumeric chars]',
        ],
      },
      userId: {
        type: Schema.Types.ObjectId,
...
AuditTrailSchema.virtual('requiresRegulatorNotification').get(function () {
  return (
    this.compliance?.regulatorNotificationRequired
...
AuditTrailSchema.virtual('isHighSensitivity').get(function () {
  return (
    this.compliance?.sensitivityLevel === 'RESTRICTED'
...
AuditTrailSchema.statics.logDocumentAccess = async function (params) {
  ...
  const auditEntry = new this({
    ...
    compliance: {
      ...
      sensitivityLevel:
        documentMetadata.classification === 'RESTRICTED' ? 'RESTRICTED' : 'CONFIDENTIAL',
    },
    metadata: {
...
AuditTrailSchema.statics.getDocumentAccessHistory = async function (documentId, options = {}) {
  const {
    tenantId, startDate, endDate, userId, accessType, limit = 100, skip = 0,
  } = options;
...
AuditTrailSchema.statics.generateComplianceReport = async function (params = {}) {
  const {
    jurisdiction = 'ZA', startDate, endDate, tenantId, legalBasis,
  } = params;
  ...
  const events = await this.find(query).sort({ timestamp: -1 }).limit(5000).lean();
  ...
  const potentialIssues = events.filter(
    (event) => event.compliance?.regulatorNotificationRequired
  ...
  return {
    ...
    executiveSummary: {
      ...
      averageResponseTime:
        events.reduce((sum, e) => sum + (e.outcome?.responseTimeMs || 0), 0) / totalEvents || 0,
    },
    detailedBreakdown: {
...
AuditTrailSchema.pre('save', async function (next) {
  try {
    ...
    if (
      ...
      this.actor.encryptedMetadata.ipAddress = encryptField(this.actor.encryptedMetadata.ipAddress);
    }
    ...
    if (
      ...
      this.actor.encryptedMetadata.userAgent = encryptField(this.actor.encryptedMetadata.userAgent);
    }
...
function encryptField(plaintext) {
  ...
  const encryptionKey = process.env.AUDIT_ENCRYPTION_KEY || 'dev-fallback-key-32-bytes-long-for-testing';
...
function generateComplianceRecommendations(events) {
  ...
  const failedDocumentAccesses = events.filter(
    (e) => e.action?.category === 'DOCUMENT_ACCESS' && !e.outcome?.success,
  );
  ...
  const documentShares = events.filter(
    (e) => e.action?.category === 'DOCUMENT_ACCESS'
  ...
  const securityIncidents = events.filter(
    (e) => e.action?.category === 'SYSTEM_SECURITY' && !e.outcome?.success,
  );
