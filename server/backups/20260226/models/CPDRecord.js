...
const cpdRecordSchema = new Schema(
  {
    ...
    activityDate: {
      ...
      index: true,
      validate: {
        validator: (v) => v.getFullYear() >= 2020 && v.getFullYear() <= new Date().getFullYear(),
      },
    },
    year: {
      type: Number,
    ...
    provider: {
      ...
      accreditationNumber: {
        type: String,
        validate: { validator: (v) => !v || /^LPC-ACC-\d{6}$|^LPC-\d{4}-\d{4}$/i.test(v) },
      },
    ...
    evidence: {
      certificateUrl: {
        type: String,
        required: true,
        validate: { validator: (v) => /^https?:\/\/.+\..+/.test(v) },
      },
      certificateHash: {
        type: String,
        required: true,
        validate: { validator: (v) => /^[a-f0-9]{64}$/i.test(v) },
      },
      evidenceUrls: [
        { url: String, description: String, uploadedAt: { type: Date, default: Date.now } },
      ],
      ...
      completionDate: {
        type: Date,
        required: true,
        validate: { validator: (v) => v <= new Date() },
      },
    },
    verificationCode: {
      type: String,
    ...
    compliance: {
      ...
      rolloverHours: {
        ...
        min: 0,
        max: 6,
        validate: { validator: (v) => !v || v <= this.hours },
      },
    ...
    complianceCertificate: {
      certificateId: { type: String, sparse: true, unique: true },
      issuedAt: Date,
      expiresAt: Date,
...
cpdRecordSchema.virtual('verificationProgress').get(function () {
  ...
  let p = 0;
  if (this.evidence.certificateHash) p += 25;
  if (this.verificationCode) p += 25;
  if (this.provider.accreditationNumber) p += 25;
  if (this.blockchain?.verified) p += 25;
  if (this.accreditation.isLpcAccredited) p += 15;
  return Math.min(100, p);
});
cpdRecordSchema.virtual('complianceStatus').get(function () {
  if (this.isVerified) {
    if (this.category === 'ETHICS' && this.hours >= 2) return 'ETHICS_COMPLIANT';
...
cpdRecordSchema.statics = {
  async getAttorneySummary(attorneyId, tenantId, year = new Date().getFullYear()) {
    ...
    const prevRecords = await this.find({
      attorneyId,
      tenantId,
      year: year - 1,
      verificationStatus: { $in: ['VERIFIED', 'AUTO_VERIFIED'] },
    ...
    return {
      ...
      requirements: {
        hoursRequired: 12,
        ethicsRequired: 2,
        hoursRemaining: parseFloat(Math.max(0, 12 - effectiveHours).toFixed(1)),
        ethicsRemaining: parseFloat(Math.max(0, 2 - ethicsHours).toFixed(1)),
      },
      compliance: {
        isCompliant,
        status: isCompliant
          ? 'COMPLIANT'
          : effectiveHours >= 10 || ethicsHours >= 1
            ? 'WARNING'
            : 'NON_COMPLIANT',
        isOverdue: Date.now() > deadline && !isCompliant,
        deadline: deadline.toISOString(),
        daysUntilDeadline: Math.ceil((deadline - Date.now()) / (1000 * 60 * 60 * 24)),
      },
  ...
  async findNonCompliant(tenantId, year = new Date().getFullYear()) {
    const attorneys = await mongoose
    ...
    return this.aggregate([
      ...
      },
      {
        $group: {
          ...
          ethicsHours: { $sum: { $cond: [{ $eq: ['$category', 'ETHICS'] }, '$hours', 0] } },
          averageHours: { $avg: '$hours' },
          verifiedCount: { $sum: 1 },
          autoVerifiedCount: {
            $sum: { $cond: [{ $eq: ['$verificationStatus', 'AUTO_VERIFIED'] }, 1, 0] },
          },
...
cpdRecordSchema.methods = {
  async verify(userId, method = 'MANUAL', notes = '') {
    ...
    if (attorney) {
      ...
      attorney.cpd.complianceStatus = s.compliance.isCompliant ? 'COMPLIANT' : 'NON_COMPLIANT';
  ...
  async generateAuditReport() {
    ...
    return {
      ...
      generatedAt: new Date().toISOString(),
      generatedBy: 'WilsyOS CPD Forensic Engine v5.0.1',
    };
  },
  async updateVerificationStatus(status, userId, notes = '') {
    const old = this.verificationStatus;
    ...
    this.auditTrail.push({
      ...
      performedBy: userId,
      performedAt: new Date(),
      changes: { oldStatus: old, newStatus: status, notes },
    });
    await this.save();
    return {
      success: true,
