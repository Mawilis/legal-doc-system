...
const DISCOUNT_TYPES = {
  ...
  PRO_BONO: 'PRO_BONO',
  RURAL_PRACTICE: 'RURAL_PRACTICE',
  REMOTE_RURAL: 'REMOTE_RURAL',
  GOVERNMENT: 'GOVERNMENT',
  NON_PRACTICING: 'NON_PRACTICING',
  EARLY_PAYMENT: 'EARLY_PAYMENT',
  BULK_RENEWAL: 'BULK_RENEWAL',
...
const fidelityFundSchema = new Schema(
  {
    ...
    expiryDate: {
      ...
      required: true,
      index: true,
      validate: { validator: (v) => v > this.issueDate },
    },
    status: {
      type: String,
    ...
    statusHistory: [
      {
        status: { type: String, enum: Object.values(CERTIFICATE_STATUS) },
    ...
    discounts: [
      {
        ...
        amount: { type: Number, get: (v) => parseFloat(v.toFixed(2)) },
    ...
    paymentReminders: [
      {
        ...
        method: { type: String, enum: ['EMAIL', 'SMS', 'PORTAL', 'LETTER'] },
        status: String,
        error: String,
      },
    ],
    ...
    complianceCheck: {
      ...
      practiceRegistered: { type: Boolean, default: false },
      disciplinaryClear: { type: Boolean, default: false },
    ...
    verificationMethods: [
      {
        method: { type: String, enum: Object.values(VERIFICATION_METHODS) },
        verifiedAt: Date,
        verifiedBy: String,
    ...
    blockchainAnchor: {
      anchorId: { type: String, sparse: true, unique: true },
    ...
    renewal: {
      previousCertificateId: {
        type: String,
        sparse: true,
        validate: { validator: (v) => !v || /^FFC-\d{4}-[A-F0-9]{8}$/.test(v) },
      },
      nextCertificateId: {
        type: String,
        sparse: true,
...
fidelityFundSchema.virtual('isOverdue').get(function () {
  return this.payment.status === 'PENDING' && this.paymentDeadline < new Date();
});
fidelityFundSchema.virtual('daysUntilExpiry').get(function () {
  return this.expiryDate ? Math.ceil((this.expiryDate - Date.now()) / (1000 * 60 * 60 * 24)) : null;
});
fidelityFundSchema.virtual('daysSinceExpiry').get(function () {
  return this.expiryDate
    ? this.expiryDate > new Date()
      ? 0
      : Math.ceil((Date.now() - this.expiryDate) / (1000 * 60 * 60 * 24))
    : null;
});
fidelityFundSchema.virtual('daysUntilPaymentDeadline').get(function () {
  return this.paymentDeadline && this.payment.status !== 'PAID'
    ? Math.ceil((this.paymentDeadline - Date.now()) / (1000 * 60 * 60 * 24))
    : 0;
});
fidelityFundSchema.virtual('totalDiscount').get(function () {
  return parseFloat(this.discounts.reduce((s, d) => s + (d.amount || 0), 0).toFixed(2));
});
fidelityFundSchema.virtual('netContribution').get(function () {
  return parseFloat(Math.max(0, this.contributionAmount - this.totalDiscount).toFixed(2));
});
fidelityFundSchema.virtual('contributionRate').get(function () {
  return this.turnoverDeclared === 0
    ? 0
    : parseFloat(((this.baseContribution / this.turnoverDeclared) * 100).toFixed(3));
});
fidelityFundSchema.virtual('discountEfficiency').get(function () {
  return this.baseContribution === 0
    ? 0
    : parseFloat(((this.discountAmount / this.baseContribution) * 100).toFixed(1));
});
...
fidelityFundSchema.pre('save', async function (next) {
  try {
    ...
    this.digitalSignature = crypto
      .createHmac('sha3-512', process.env.QUANTUM_SECRET || 'wilsy-os-quantum-secure-2026')
      .update([this.certificateId, this.certificateHash, this.issueDate.toISOString()].join(':'))
      .digest('hex');
...
fidelityFundSchema.statics = {
  calculateContribution(
    ...
    if (['NON_PRACTICING', 'GOVERNMENT', 'RETIRED'].includes(practiceType)) {
      return {
        ...
        isExempt: true,
        exemptionReason: practiceType,
        discounts: [],
        statutoryReference: 'LPC Rule 55(3)',
    ...
    if (discountPct > 50) {
      const r = 50 / discountPct;
      discountAmount *= r;
      discountPct = 50;
      discounts.forEach((d) => {
        d.amount = Math.round(d.amount * r * 100) / 100;
        d.percentage = Math.round(d.percentage * r);
      });
    }
    final = Math.max(0, Math.round((base - discountAmount) * 100) / 100);
    return {
      baseContribution: base,
      finalContribution: final,
  ...
  async getComplianceStats(tenantId) {
    ...
    const totalContribution = await this.aggregate([
      { $match: { tenantId, status: { $in: ['ISSUED', 'RENEWED'] }, deleted: false } },
...
fidelityFundSchema.methods = {
  async renew(turnover, userContext) {
    ...
    const calc = this.constructor.calculateContribution(
      turnover,
    ...
    const cert = new this.constructor({
      ...
      payment: { amount: calc.finalContribution, status: 'PENDING' },
      renewal: {
    ...
    await cert.save();
    this.status = 'RENEWED';
    this.renewal.nextCertificateId = cert.certificateId;
    ...
    this.auditTrail.push({
      ...
      changes: {
        previousCertificateId: this.certificateId,
        newCertificateId: cert.certificateId,
        contributionAmount: cert.contributionAmount,
      },
    });
    await this.save();
    return {
  ...
  async revoke(reason, userContext) {
    ...
    this.auditTrail.push({
      ...
      performedBy: userContext.userId,
      performedAt: new Date(),
      changes: { previousStatus: prev, newStatus: 'REVOKED', reason },
    });
    await this.save();
    return {
      success: true,
  ...
  async generateVerificationProof() {
    return {
      ...
      digitalSignature: crypto
        .createHmac('sha3-512', process.env.QUANTUM_SECRET || 'wilsy-os-quantum-secure-2026')
        .update(`${this.certificateId}:${Date.now()}`)
        .digest('hex'),
  ...
  async anchorToBlockchain() {
    ...
    this.auditTrail.push({
      ...
      performedBy: 'SYSTEM',
      performedAt: new Date(),
      changes: { anchorId, transactionHash: txHash, timestamp: this.blockchainAnchor.timestamp },
    });
    await this.save();
    return {
      success: true,
  ...
  async sendRenewalReminder(method = 'EMAIL', days = 30) {
    ...
    this.renewal.renewalReminders.push({
      sentAt: new Date(),
    ...
    this.auditTrail.push({
      ...
      performedBy: 'SYSTEM',
      performedAt: new Date(),
      changes: { method, daysBeforeExpiry: days, expiryDate: this.expiryDate },
    });
    await this.save();
    return {
      success: true,
      certificateId: this.certificateId,
      reminder: this.renewal.renewalReminders[this.renewal.renewalReminders.length - 1],
    };
  },
  async generateAuditReport() {
    ...
    return {
      ...
      generatedAt: new Date().toISOString(),
      generatedBy: 'WilsyOS Fidelity Fund Engine v5.0.1',
    };
  },
};
