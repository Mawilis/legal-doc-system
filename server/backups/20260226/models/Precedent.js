...
const PrecedentSchema = new Schema(
  {
    ...
    precedentReference: {
      ...
      default() {
        ...
        const tenantCode = this.tenantId.replace('tenant_', '').substring(0, 6).toUpperCase();
...
PrecedentSchema.virtual('yearsSinceJudgment').get(function () {
  ...
  const diffTime = Math.abs(new Date() - this.court.dateJudgment);
...
PrecedentSchema.virtual('primaryJudge').get(function () {
  ...
  for (let i = 0; i < this.court.judge.length; i++) {
...
PrecedentSchema.statics.findByCitation = function (tenantId, citation) {
  const citationRegex = new RegExp(citation, 'i');
  return this.find({
    tenantId,
    $or: [
...
PrecedentSchema.methods.updateAIAnalysis = function (analysis, updatedBy) {
  ...
  const existingKeys = Object.keys(this.aiAnalysis);
  ...
  for (let j = 0; j < analysisKeys.length; j++) {
    const analysisKey = analysisKeys[j];
