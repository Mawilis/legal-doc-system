...
ReportLedgerSchema.pre('remove', async function (next) {
  ...
  if (this.status === REPORT_STATUS.QUARANTINED) {
    const error = new Error('Cannot delete report under legal hold. Contact compliance officer.');
...
ReportLedgerSchema.virtual('complianceRequirements').get(function () {
  const requirements = {
    ...
    requiresSubmission: !!this.submissionDetails?.submittedTo,
...
ReportLedgerSchema.virtual('fileSizeFormatted').get(function () {
  ...
  return `${parseFloat((this.fileSize / 1024 ** i).toFixed(2))} ${sizes[i]}`;
});
