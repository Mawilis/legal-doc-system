...
sessionSchema.statics.cleanupExpiredSessions = async function (tenantId = null) {
  ...
  const expiredSessions = await this.find(query).select('_id tenantId userId createdAt').lean();
        ...
        sessionStats: [
          {
            $group: {
              ...
              averageDurationHours: {
                $avg: {
                  $divide: [{ $subtract: ['$lastActivity', '$createdAt'] }, 1000 * 60 * 60],
                },
              },
              highRiskSessions: {
...
sessionSchema.post('deleteOne', { document: true }, async (doc) => {
