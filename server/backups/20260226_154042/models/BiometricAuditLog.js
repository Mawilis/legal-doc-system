...
const BiometricAuditLogSchema = new mongoose.Schema(
  {
    ...
    legalFirmId: {
      ...
      required: [
        true,
        'COMPLIANCE VIOLATION: Legal firm context required for POPIA accountability',
      ],
      index: true,
    },
    ...
    forensicData: {
      ...
      deviceFingerprint: {
        ...
        set(fingerprint) {
          return crypto.createHmac('sha256', AUDIT_HASH_SECRET).update(fingerprint).digest('hex');
        },
      },
    ...
    integrityHash: {
      ...
      default() {
        ...
        return crypto.createHmac('sha512', AUDIT_HASH_SECRET).update(data).digest('hex');
      },
      immutable: true,
    },
...
BiometricAuditLogSchema.virtual('isCompliant').get(function () {
  const { popia } = this.complianceMarkers;
...
BiometricAuditLogSchema.virtual('chainIntegrity').get(function () {
  ...
  const calculatedHash = crypto.createHmac('sha512', AUDIT_HASH_SECRET).update(data).digest('hex');
 ...
 */
BiometricAuditLogSchema.post('save', async (doc) => {
...
BiometricAuditLogSchema.statics.generateComplianceReport = async function (
  ...
  logs.forEach((log) => {
    ...
    // Security incidents
    if (log.eventCategory === 'SECURITY_INCIDENT' && log.eventMetadata.severity === 'CRITICAL') {
...
BiometricAuditLogSchema.methods.generatePreviousHash = function () {
  ...
  return this.constructor
    ...
    .sort({ _id: -1 })
    .then((previousLog) => (previousLog ? previousLog.integrityHash : 'GENESIS-BLOCK'));
};
 ...
 */
BiometricAuditLogSchema.methods.placeLegalHold = function (
  placedBy,
  reason,
  caseReference,
  expectedReleaseDate,
