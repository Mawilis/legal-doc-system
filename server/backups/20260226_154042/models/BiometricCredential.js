...
BiometricCredentialSchema.methods.revoke = async function (revocationData) {
  ...
  this.updatedBy = revocationData.revokedBy
    ? new mongoose.Types.ObjectId(revocationData.revokedBy)
    : null;
...
BiometricCredentialSchema.methods.checkValidity = function () {
  ...
  const checks = {
    ...
    consentValid: this.consent?.consentId && this.consent?.informationOfficerApproval?.approved,
    compliant: this.compliance.popiaCompliant && this.compliance.ectActCompliant,
  ...
  return {
    ...
    daysUntilExpiry: this.retention?.expiresAt
      ? Math.ceil((this.retention.expiresAt - now) / (1000 * 60 * 60 * 24))
      : null,
    lockoutRemaining: this.lockedUntil ? Math.ceil((this.lockedUntil - now) / 1000) : null,
            ...
            $cond: [
              {
                $and: [
                  ...
                  {
                    $lt: ['$retention.expiresAt', new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)],
                  },
