    ...
    registrationDetails: {
      registrationNumber: {
        ...
        validate: {
          validator(v) {
            if (!v) return true;
            const formats = [/^\d{4}\/\d{6}\/\d{2}$/, /^K\d{7}$/, /^[A-Z]{2}\d{6}$/];
...
ClientSchema.methods.verifyFICACompliance = function () {
  ...
  return {
    ...
    daysUntilExpiry: this.ficaDetails.expiryDate
      ? Math.ceil((this.ficaDetails.expiryDate - now) / (1000 * 60 * 60 * 24))
      : null,
...
ClientSchema.statics.getFICAComplianceReport = async function (tenantId) {
  ...
  report.summary.complianceMetrics = {
    ficaComplianceRate:
      clients.length > 0 ? Math.round((verifiedCount / clients.length) * 100) : 100,
