...
complianceAuditSchema.index({
  tenantId: 1,
  'workflow.status': 1,
  'scheduling.nextScheduledDate': 1,
});
...
complianceAuditSchema.index({
  tenantId: 1,
  'complianceIssues.severity': 1,
  'complianceIssues.remediation.status': 1,
});
...
complianceAuditSchema.statics = {
  ...
  async getRegulatoryStatus(tenantId, framework) {
    ...
    const openFindings = allFindings.filter((f) => ['OPEN', 'IN_PROGRESS'].includes(f.status));
...
complianceAuditSchema.methods = {
  ...
  async verifyIntegrity() {
    ...
    return {
      ...
      checks: {
        reportHash: {
          valid: reportHashValid,
          expected: this.reportHash,
          computed: recalculatedReportHash,
        },
        integrityHash: {
          valid: integrityHashValid,
          expected: this.integrityHash,
          computed: recalculatedIntegrityHash,
        },
        quantumSignature: {
          valid: signatureValid,
          expected: this.quantumSignature,
          computed: recalculatedSignature,
        },
