...
const validateConsentEnvironment = () => {
  const requiredVars = ['MONGODB_URI', 'CONSENT_ENCRYPTION_KEY', 'DEFAULT_CONSENT_RETENTION_YEARS'];
...
const ConsentRecordSchema = new mongoose.Schema(
  {
    ...
    consentId: {
      ...
      default() {
        return consentEncryption.generateConsentId(this.userId?.toString(), this.consentType);
      },
    },
...
ConsentRecordSchema.methods.verifyValidity = function () {
  ...
  const checks = {
    ...
    compliant: this.complianceFlags.popiaCompliant || this.complianceFlags.gdprCompliant,
  };
...
ConsentRecordSchema.methods.renewConsent = async function (renewalData) {
  ...
  const newConsent = new ConsentRecord({
    ...
    grantedAt: new Date(),
    expiresAt:
      renewalData.newExpiry || new Date(new Date().setFullYear(new Date().getFullYear() + 5)),
    auditTrail: [],
...
ConsentRecordSchema.statics.findActiveConsents = function (userId, consentType = null) {
  ...
  return this.find(query).sort({ grantedAt: -1 }).lean();
};
