...
const generationalLedgerSchema = new mongoose.Schema(
  {
    ...
    epoch: {
      ...
      required: true,
      immutable: true,
      enum: ['GENESIS-2024', 'EXPANSION-2025', 'PAN-AFRICAN-2026', 'GLOBAL-2027', 'LEGACY-2028+'],
    ...
    scope: {
      ...
      required: true,
      immutable: true,
      enum: ['INDIVIDUAL', 'FIRM', 'JURISDICTION', 'PLATFORM_WIDE', 'PAN_AFRICAN', 'GLOBAL'],
      default: 'INDIVIDUAL',
      index: true,
    },
    ...
    retentionYears: {
      ...
      validate: {
        ...
        message:
          'Genesis and billion-Rand blocks must be kept 1000 years, million-Rand blocks 100 years',
      },
    },
  ...
  },
  {
    ...
    toJSON: {
      ...
      transform(doc, ret) {
        ...
        if (ret.baseValue) {
          ret.baseValue = `R${ret.baseValue.toLocaleString('en-ZA', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          })}`;
        }
        if (ret.finalValue) {
          ret.finalValue = `R${ret.finalValue.toLocaleString('en-ZA', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          })}`;
        }
        if (ret.cumulativeValue) {
          ret.cumulativeValue = `R${ret.cumulativeValue.toLocaleString('en-ZA', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          })}`;
        }
...
generationalLedgerSchema.pre('save', async function (next) {
  ...
  if (this.isNew) {
    try {
      ...
      const previousBlock = await this.constructor
        .findOne()
        .sort({ blockHeight: -1 })
    ...
    },
    {
      $project: {
        ...
        wealthPercentage: {
          $multiply: [
            {
              $divide: ['$totalWealth', { $sum: '$totalWealth' }],
            },
            100,
          ],
        },
