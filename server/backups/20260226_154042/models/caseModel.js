...
const QuantumCaseSchema = new mongoose.Schema(
  {
    ...
    retentionPolicy: {
      ...
      default:
        process.env.DEFAULT_RETENTION_YEARS === '10' ? 'EXTENDED_10_YEARS' : 'STANDARD_5_YEARS',
      required: true,
    },
...
QuantumCaseSchema.pre('remove', async function (next) {
  if (this.legalHold.active) {
    throw new SecurityError('Cannot delete case under legal hold', 'ATTORNEY_EYES_ONLY');
  }
...
QuantumCaseSchema.statics.getFirmAnalytics = async function (firmId, startDate, endDate) {
        ...
        financial: [
          {
            $group: {
              ...
              byFeeArrangement: {
                $push: { arrangement: '$feeArrangement', value: '$metrics.matterValue' },
              },
            },
          },
        ],
  ...
  const insights = {
    ...
    revenuePerAttorney:
      analytics[0]?.financial[0]?.totalRevenue / (analytics[0]?.team?.length || 1),
    riskExposure: analytics[0]?.performance[0]?.prescriptionAlerts > 10 ? 'HIGH' : 'MANAGEABLE',
    recommendation:
      analytics[0]?.performance[0]?.avgResourceEfficiency < 70
