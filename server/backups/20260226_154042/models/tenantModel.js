...
const TENANT_CONFIG = {
  ...
  // Firm Statuses
  FIRM_STATUSES: ['ONBOARDING', 'ACTIVE', 'SUSPENDED', 'CANCELLED', 'PAST_DUE', 'TRIAL_EXPIRED'],
        ...
        features: [
          {
            ...
            default() {
              return (
                TENANT_CONFIG.SUBSCRIPTION_PLANS[this.subscription?.plan || 'TRIAL'].features || []
              );
            },
          },
        ],
...
TenantSchema.plugin(mongooseEncryption, {
  ...
  excludeFromEncryption: [
    'sovereignId',
    'legalIdentity.slug',
    'legalIdentity.firmNumber',
    'sovereignty.status',
    'sovereignty.isActive',
    'createdAt',
  ],
  encryptSave: true,
  decryptSave: true,
...
TenantSchema.statics.getComplianceStatistics = async function () {
  ...
  stats.rates = {
    lpcRate:
      totalTenants > 0 ? Math.round((stats.compliance.lpcRegistered / totalTenants) * 100) : 0,
    popiaRate:
      totalTenants > 0 ? Math.round((stats.compliance.popiaCompliant / totalTenants) * 100) : 0,
    vatRate:
      totalTenants > 0 ? Math.round((stats.compliance.vatRegistered / totalTenants) * 100) : 0,
    trustRate:
      totalTenants > 0 ? Math.round((stats.compliance.trustAccounting / totalTenants) * 100) : 0,
  };
