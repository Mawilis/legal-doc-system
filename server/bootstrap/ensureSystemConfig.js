...
async function ensureSystemConfig(opts = {}) {
  ...
  while (attempt <= retries) {
    ...
    try {
      ...
      if (!cfg || typeof cfg !== 'object' || cfg.scope !== 'global') {
        logger.warn('ensureSystemConfig: unexpected config shape, returning defaults', {
          ...logMeta,
          cfg,
        });
      }
      ...
      if (emitAudit) {
        try {
          ...
          logger.warn('Failed to write bootstrap audit', {
            ...logMeta,
            err: auditErr && auditErr.message ? auditErr.message : auditErr,
          });
        }
      }
      ...
      logger.error('ensureSystemConfig: attempt failed', {
        ...logMeta,
        attempt,
  ...
  logger.error('ensureSystemConfig: bootstrap failed after retries', {
    ...logMeta,
    retries,
    lastError: lastErr && lastErr.message ? lastErr.message : lastErr,
  });
...
function invalidateSystemConfigCache() {
  try {
    ...
    logger.warn('invalidateSystemConfigCache: cache invalidation failed', {
