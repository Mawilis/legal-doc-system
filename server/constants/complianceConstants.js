...
const DATA_PROCESSING_CONSTANTS = {
  ...
  DATA_SUBJECT_CATEGORIES: ['CLIENT', 'EMPLOYEE', 'VENDOR', 'PROSPECT', 'VISITOR', 'OTHER'],
...
const SARS_INTEREST_RATES = {
  ...
  REFUND_DUE: 0.07, // 7.0% per annum
  UNDERSTATEMENT: 0.2, // 20% penalty
};
...
const LPC_FEE_GUIDELINES = {
  ...
  LITIGATION_ADVANCE_MAX: 0.5, // Maximum 50% of estimated fees as advance
  SETTLEMENT_NEGOTIATION_PERCENTAGE: 0.1, // 10% of settlement amount
...
const NCA_INTEREST_RATES = {
  ...
  DEBT_COLLECTION_FEE: 0.1, // 10% of outstanding balance
...
const NCA_AFFORDABILITY_RULES = {
  DEBT_TO_INCOME_MAX: 0.4, // Maximum 40% of income
...
const LEGAL_FEE_CAPS = {
  ...
  LABOUR_CASE: 0.2, // Maximum 20% of award
  ...
  LITIGATION_ADVANCE_MAX: 0.5, // Maximum 50% of estimated fees
  ...
  DIVORCE_CONTESTED: 0.2, // Maximum 20% of assets for contested divorce
  ...
  LIQUIDATION_FEES: 0.1, // Maximum 10% of assets for liquidation
...
const COMPLIANCE_PENALTIES = {
  ...
  SARS: {
    LATE_SUBMISSION: 0.1, // 10% of tax due
    UNDERSTATEMENT_PENALTY: 2.0, // 200% of tax avoided
...
function getDocumentRetentionPeriod(documentType, jurisdiction = 'ZA') {
  const retentionMap = {
    ...
    privacy_impact_assessments:
      DOCUMENT_RETENTION.DATA_PROTECTION_RECORDS.PRIVACY_IMPACT_ASSESSMENTS,
...
function calculateCompliancePenalty(breachType, severity = 'MINOR', jurisdiction = 'ZA') {
  ...
  switch (breachType.toUpperCase()) {
    case 'POPIA':
      return severity === 'SERIOUS' ? penalties.POPIA.SERIOUS_BREACH : penalties.POPIA.MINOR_BREACH;
    ...
    case 'GDPR':
      return severity === 'TIER_2' ? penalties.GDPR.TIER_2_PENALTY : penalties.GDPR.TIER_1_PENALTY;
