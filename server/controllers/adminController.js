...
class ComplianceService {
  ...
  async _checkInformationOfficer() {
    return {
      ...
      details: process.env.POPIA_IO_NAME
        ? 'Information officer designated'
        : 'Information officer not designated',
...
const createForensicAudit = async (action, userId, firmId, metadata = {}) => {
  ...
  try {
    ...
    const previousHash = previousAudit
      ? previousAudit.integrityHash
    ...
    console.log(`[AUDIT] Forensic audit created: ${action}`, {
      auditId,
    ...
    const duration = Date.now() - startTime;
    console.error('Forensic audit creation failed:', {
      auditId,
      error: error.message,
      duration: `${duration}ms`,
    });
...
const analyzeComplianceTrend = async () => {
  try {
    ...
    const trends = complianceTrends.map((month, index, array) => {
      ...
      const trend = previousMonth ? month.averageScore - previousMonth.averageScore : 0;
    ...
    return {
      ...
      currentScore:
        complianceTrends.length > 0
          ...
          : 'N/A',
      overallTrend:
        trends.length > 1 ? trends[trends.length - 1].trendDirection : 'INSUFFICIENT_DATA',
      monthlyTrends: trends,
      recommendations:
        trends.length > 1 && trends[trends.length - 1].trend < 0
  ...
  try {
    ...
    const dashboardData = {
      ...
      valuation: {
        ...
        progress: `${(
          (businessValue.totalProtectedValue / GENERATIONAL_ADMIN.VALUATION_TARGET) *
          100
        ).toFixed(2)}%`,
...
exports.getUserManagement = async (req, res) => {
  ...
  try {
    ...
    const maskedUsers = users.map((user) => maskUserDataForRole(user.toObject(), requester.role));
