...
const { AI_SERVICE_SECRET } = process.env;
...
class AISovereigntyClient {
  ...
  calculateAICost(capability, metrics) {
    const costMatrix = {
      RISK_ASSESSMENT: 0.5, // R0.50 per request
      DOCUMENT_ANALYSIS: 2.0, // R2.00 per document
      LEGAL_PRECEDENT: 1.5, // R1.50 per search
      COMPLIANCE_CHECK: 3.0, // R3.00 per check
      CONTRACT_REVIEW: 5.0, // R5.00 per review
      CLIENT_INTELLIGENCE: 1.0, // R1.00 per analysis
    };

    const baseCost = costMatrix[capability] || 1.0;
...
exports.getSafetyRiskAssessment = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const aiResult = await aiSovereigntyClient.executeAIRequest(
      'RISK_ASSESSMENT',
      {
        address,
        coordinates,
...
exports.reviewContractAI = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await emitAudit(req, {
      ...
      severity:
        aiResult.overallRisk === 'HIGH'
          ? 'HIGH'
          : aiResult.overallRisk === 'MEDIUM'
            ? 'MEDIUM'
            : 'LOW',
...
exports.checkComplianceAI = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await emitAudit(req, {
      ...
      severity:
        aiResult.complianceStatus === 'NON_COMPLIANT'
          ? 'HIGH'
          : aiResult.complianceStatus === 'PARTIAL'
            ? 'MEDIUM'
            : 'LOW',
    ...
    Object.entries(AI_CAPABILITIES).forEach(([capability, config]) => {
      if (!config.premium || this.checkAIAccess(tenant)) {
        availableCapabilities[capability] = {
          ...config,
          available: true,
          estimatedCost: this.calculateAICost(capability, {
            tokensUsed: 1000,
            processingTime: 5000,
          }),
        };
...
exports.getAIUsage = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Get monthly usage
    const monthlyUsage = await mongoose.connection.db
      .collection('ai_usage')
      .aggregate([
        {
          $match: {
