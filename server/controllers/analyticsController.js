...
exports.getFinancialDashboard = asyncHandler(async (req, res) => {
  ...
  if (!validPeriods.includes(period)) {
    return errorResponse(req, res, 400, 'Invalid period parameter', { validPeriods });
  }

  // Validate currency
  const validCurrencies = ['ZAR', 'USD', 'EUR', 'GBP', 'NGN', 'KES'];
  if (!validCurrencies.includes(currency)) {
    return errorResponse(req, res, 400, 'Invalid currency parameter', { validCurrencies });
  }
  ...
  const [subscriptionStats, invoiceStats, tenantStats, churnData] = await Promise.all([
...
exports.getSecurityHeatmap = asyncHandler(async (req, res) => {
  ...
  return successResponse(req, res, encryptedHeatmap, 'Security threat heatmap generated', {
    security: {
