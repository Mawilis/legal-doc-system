...
exports.createClient = async (req, res, next) => {
  try {
    ...
    const existingClient = await Client.findOne({
      ...
      $or: [{ email: email.toLowerCase() }, { registrationNumber: registrationNumber }],
      isDeleted: false,
    });
...
exports.getAllClients = async (req, res, next) => {
  try {
    ...
    const cacheKey = `clients:${req.user.tenantId}:${status || 'all'}:${
      ficaStatus || 'all'
    ...
    const response = {
      ...
      complianceSummary: {
        ...
        pendingVerification: await Client.countDocuments({
          ...query,
          ficaStatus: { $ne: 'VERIFIED' },
        }),
...
exports.verifyFica = async (req, res, next) => {
  try {
    ...
    await emitAudit(req, {
      ...
      regulatoryRequirements: ['FICA_§21', 'FICA_§22A', 'FICA_§25', 'POPIA_§11', 'AML_ACT_2001'],
    });
  ...
  try {
    ...
    res.status(200).json({
      ...
      compliance: {
        ...
        dualApproval:
          requiresDualApprovalByAmount || requiresDualApproval
