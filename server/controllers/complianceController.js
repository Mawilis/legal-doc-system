...
const complianceValidationSchemas = {
  ...
  createCheck: Joi.object({
    ...
    riskOverride: Joi.string()
      .valid(...Object.keys(COMPLIANCE_RISK_LEVELS))
      .label('Risk Override')
      .messages({
    ...
    verificationMethod: Joi.string()
      .valid('MANUAL', 'AUTOMATED', 'THIRD_PARTY')
      .default('MANUAL')
    ...
    language: Joi.string()
      .valid('en', 'zu', 'xh', 'af', 'nso', 'tn', 'st', 'ts', 'ss', 've', 'nr')
      .default('en')
  ...
  updateRisk: Joi.object({
    ...
    notes: Joi.string().max(1000).required().label('Risk Override Notes'),
  ...
  generateReport: Joi.object({
    reportType: Joi.string()
      .valid('FIC_STR', 'POPIA_BREACH', 'ANNUAL_COMPLIANCE', 'RISK_ASSESSMENT', 'AUDIT_TRAIL')
      .required(),
    ...
    try {
      ...
      const reportPayload = {
        ...
        breachDetails: {
          ...
          likelyConsequences:
            breachData.consequences || 'Unauthorized access to personal information',
...
class ComplianceEncryptionService {
  ...
  decryptReportData(encryptedPackage, tenantId) {
    ...
    const key = crypto.scryptSync(this.encryptionKey, tenantId, 32);
  ...
  try {
    ...
    if (cachedClient) {
      ...
      client = await Client.findOne({
        ...
        .select('+encryptedIdNumber +encryptedTaxNumber')
        .lean();
    ...
    for (const docType of documentsProvided) {
      ...
      const docCacheKey = `verification_${docType}_${
    ...
    if (enhancedDueDiligence?.pepDeclaration || automaticRiskScore > 50) {
      const sanctionsCacheKey = `sanctions_${
        documentMetadata?.idNumber || documentMetadata?.passportNumber || 'unknown'
      }`;
    ...
    const complianceMessage = await multiLingualService.getFICAVerificationMessage(language, {
      ...
      status,
      nextSteps:
        finalRiskLevel === 'HIGH' ? 'Enhanced monitoring required' : 'Standard monitoring applied',
    });
