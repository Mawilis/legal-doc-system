...
(async () => {
  ...
  if (envWarnings.length > 0) {
    quantumLogger.warn('ComplianceExportController', 'Environment warnings', {
...
const popiaDSARSchema = Joi.object({
  ...
  requestType: Joi.string()
    ...
    .messages({
      'any.only':
        'Invalid request type. Must be FULL, PII_ONLY, CONSENT_HISTORY, or PROCESSING_ACTIVITIES',
    }),
  ...
  encryptionLevel: Joi.string().valid('STANDARD', 'ENHANCED', 'GOVERNMENT').default('STANDARD'),
...
const paiaRequestSchema = Joi.object({
  ...
  feePreference: Joi.string().valid('WAIVE_FEE', 'STANDARD_FEE', 'PREPAID').default('STANDARD_FEE'),
  ...
  formatPreference: Joi.string().valid('ELECTRONIC', 'PAPER', 'BOTH').default('ELECTRONIC'),

  language: Joi.string().valid('EN', 'AF', 'ZU', 'XH', 'NS').default('EN'),
});
/*
...
const legalSubpoenaSchema = Joi.object({
  ...
  courtName: Joi.string().min(3).max(200).required(),

  presidingOfficer: Joi.string().min(5).max(100).optional(),
  ...
  evidenceStandard: Joi.string().valid('ADMISSIBLE', 'FORENSIC', 'DISCOVERY').default('ADMISSIBLE'),
});
// =============================================================================
...
const encryptExportArchive = async (archivePath, encryptionLevel = 'STANDARD') => {
  try {
    ...
    await fs.writeFile(encryptedPath, JSON.stringify(encryptedPackage, null, 2), 'utf8');
        ...
        attachments: [
          {
            ...
            fields: [
              ...
              {
                title: 'Retention',
                value: `${process.env.COMPLIANCE_RETENTION_DAYS || 90} days`,
                short: true,
              },
            ],
          },
    ...
    try {
        ...
        case 'FULL': {
          const [biometricAudits, documents, consentRecords] = await Promise.all([
            ...
            Document.find({ ownerId: dataSubjectId })
              ...
              .session(session)
              .lean(),
            ConsentRecord.find({ dataSubjectId }).session(session).lean(),
          ]);
      ...
      exportData.compliance = {
        popia: {
          ...
          retentionNotice: `Data will be retained for ${
            process.env.COMPLIANCE_RETENTION_DAYS || 90
          } days as per POPIA requirements`,
        },
      };
...
const generateForensicIntegrityPackage = async (
  _data,
  _exportId,
  _caseNumber,
  _evidenceStandard
      ...
      return res.status(200).json({
        ...
        data: {
          ...
          legalNotice: {
            ...
            notice:
              'This export constitutes admissible electronic evidence in South African courts',
          },
        },
      });
    } catch (error) {
...
const verifyExportIntegrity = [
  ...
  async (req, res) => {
    try {
      ...
      const currentHash = crypto.createHash('sha256').update(manifestContent).digest('hex');
      ...
      const verificationReport = {
        ...
        legalWeight:
          integrityVerified && encryptionVerified ? 'ADMISSIBLE_EVIDENCE' : 'INTEGRITY_COMPROMISED',
      };
