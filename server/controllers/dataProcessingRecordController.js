...
class DataProcessingRecordController {
  ...
  static async createProcessingRecord(req, res) {
    ...
    if (!req.user || !req.user.roles.some((role) => allowedRoles.includes(role))) {
      return res.status(403).json({
        ...
        message:
          'Insufficient permissions. Requires INFORMATION_OFFICER, COMPLIANCE_OFFICER, or SYSTEM_ADMIN role.',
    ...
    try {
      ...
      return res.status(isValidationError ? 400 : isUniqueError ? 409 : 500).json({
        ...
        error: isValidationError
          ...
          : isUniqueError
            ? 'QUANTUM_UNIQUE_CONSTRAINT'
            : 'QUANTUM_INTERNAL_ERROR',
        message: isValidationError
          ? 'Database validation failed'
          : isUniqueError
            ? 'Duplicate processing record detected'
    ...
    try {
      ...
      if (!req.user || !req.user.roles.some((role) => allowedRoles.includes(role))) {
        return res.status(403).json({
          ...
          message:
            'Only Data Protection Officers, Information Officers, or System Admins can access DSAR records',
      ...
      const redactedRecords = records.map((record) => ({
        ...
        automatedDecisionLogic: record.automatedDecisionMaking
          ? record.automatedDecisionLogic
          : undefined,
        ...
        consentStatus:
          record.consentRecords.length > 0 ? 'CONSENT_PROVIDED' : 'LEGITIMATE_INTEREST',
      }));
      ...
      return res.json({
        ...
        summary: {
          ...
          timePeriod: {
            oldest:
              records.length > 0
                ? Math.min(...records.map((r) => new Date(r.createdAt).getTime()))
                : null,
            newest:
              records.length > 0
  ...
  static async _validateAccess(user, recordJurisdiction, recordInfoOfficer) {
    ...
    if (user.roles.includes('INFORMATION_OFFICER')) {
      return user.jurisdiction === recordJurisdiction || user.email === recordInfoOfficer;
    }
  ...
  static async _getJurisdictionSummary(whereClause) {
    const results = await DataProcessingRecord.findAll({
      where: whereClause,
      attributes: ['jurisdiction', [sequelize.fn('COUNT', sequelize.col('id')), 'count']],
      group: ['jurisdiction'],
    });
  ...
  static async _getLawfulBasisSummary(whereClause) {
    const results = await DataProcessingRecord.findAll({
      where: whereClause,
      attributes: ['lawfulBasis', [sequelize.fn('COUNT', sequelize.col('id')), 'count']],
      group: ['lawfulBasis'],
    });
