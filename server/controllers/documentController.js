...
class DocumentController {
    ...
    try {
      ...
      const document = new Document({
        ...
        retentionPolicy: {
          ...
          legalReference:
            this.RETENTION_POLICIES[retentionPolicy]?.legalReference || 'Companies Act 71 of 2008',
        },
        storage: {
    ...
    try {
      ...
      return res.status(200).json({
        ...
        audit: {
          ...
          retentionPeriod:
            document.retentionPolicy?.rule === 'PERMANENT' ? 'PERMANENT' : '10 years',
        },
      });
    } catch (error) {
  ...
  async downloadDocument(req, res) {
    ...
    try {
      ...
      if (!isAuthorized) {
        ...
        return this._sendError(res, 403, 'UNAUTHORIZED', 'Not authorized to download', auditId);
      }
      ...
      if (requiresWatermark) {
        finalContent = await this._applyWatermark(finalContent, {
          userId,
          documentId: id,
      ...
      return this._sendError(res, 500, 'DOWNLOAD_FAILED', 'Failed to download document', auditId);
    }
  }
  ...
  async updateDocument(req, res) {
    ...
    try {
      ...
      if (!isAuthorized) {
        ...
        return this._sendError(res, 403, 'UNAUTHORIZED', 'Not authorized to update', auditId);
      }
      ...
      return this._sendError(res, 500, 'UPDATE_FAILED', 'Failed to update document', auditId);
    }
  }
  ...
  async deleteDocument(req, res) {
    ...
    try {
      ...
      return this._sendError(res, 500, 'DELETE_FAILED', 'Failed to delete document', auditId);
    }
  }
  ...
  async searchDocuments(req, res) {
    ...
    try {
      ...
      return this._sendError(res, 500, 'SEARCH_FAILED', 'Failed to search documents', auditId);
    }
  }
  ...
  async getDocumentAccessHistory(req, res) {
    ...
    try {
      ...
      return this._sendError(res, 500, 'AUDIT_FAILED', 'Failed to retrieve audit history', auditId);
    }
  }
  ...
  async _updateAccessStats(documentId, tenantId, accessType, success) {
    try {
      ...
      await Document.updateOne({ _id: documentId, tenantId }, update);
    } catch (error) {
