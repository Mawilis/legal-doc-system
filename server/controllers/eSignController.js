  ...
  initiateSignature: {
    ...
    signatureType: {
      ...
      isIn: {
        ...
        errorMessage:
          'Signature type must be SIMPLE, ADVANCED, QUALIFIED, BIOMETRIC, or BLOCKCHAIN',
      },
    },
    expiryHours: {
      in: ['body'],
    ...
    jurisdiction: {
      ...
      isIn: {
        options: [
          ['ZA', 'EU', 'US', 'KE', 'NG', 'GH', 'TZ', 'UG', 'RW', 'BW', 'NA', 'ZM', 'ZW', 'MU'],
        ],
  ...
  try {
    ...
    res.status(200).json({
      ...
      data: {
        ...
        nextSignatory: allSigned
          ? null
          : signatureRequest.signatories.find((s) => s.signatureStatus === 'PENDING')?.userId,
    ...
    res.status(200).json({
      ...
      data: {
        ...
        verificationCertificate:
          verificationLevel === 'FORENSIC' ? verificationCertificate : undefined,
      },
      compliance: {
        act: 'ECT Act 25 of 2002',
        section: '18(1)',
...
const downloadSignedDocument = asyncHandler(async (req, res) => {
  try {
    ...
    const signatureRequest = await ESignatureRequest.findOne({ requestId }).populate('documentId');
