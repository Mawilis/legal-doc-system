...
class PaymentController {
   ...
   */
  validateEnvironment() {
    const requiredEnvVars = ['ENCRYPTION_KEY', 'JWT_SECRET', 'DATABASE_URL', 'VAT_RATE'];
  ...
  async performFICAScreening(paymentData, clientData) {
    try {
      ...
      const riskLevel = this.calculateFICARiskLevel(
        paymentData,
        clientData,
        suspiciousPatterns,
        sanctionCheck,
  ...
  generatePayFastSignature(paymentData, gatewayConfig) {
    ...
    const paramString = `merchant_id=${gatewayConfig.merchantId}&merchant_key=${
      gatewayConfig.merchantKey
    }&amount=${paymentData.amount.toFixed(2)}&item_name=Payment&return_url=${
      process.env.APP_URL
    }/payments/return&cancel_url=${process.env.APP_URL}/payments/cancel&notify_url=${
      gatewayConfig.itnUrl
  ...
  generateReceiptSignature(receiptId, amount) {
    ...
    return crypto
      .createHmac('sha256', process.env.RECEIPT_SIGNING_KEY || process.env.JWT_SECRET)
      .update(data)
      .digest('hex');
  }
