...
class PopiaConsentController {
  ...
  static async createConsent(req, res, next) {
    ...
    const auditId = `AUDIT-CREATE-${Date.now()}-${require('crypto')
      .randomBytes(4)
      .toString('hex')}`;
  ...
  static async withdrawConsent(req, res, next) {
    const transaction = await POPIAConsent.sequelize.transaction();
    const auditId = `AUDIT-WITHDRAW-${Date.now()}-${require('crypto')
      .randomBytes(4)
      .toString('hex')}`;
  ...
  static async verifyConsent(req, res, next) {
    const auditId = `AUDIT-VERIFY-${Date.now()}-${require('crypto')
      .randomBytes(4)
      .toString('hex')}`;
  ...
  static async authorizeConsentAccess(user, consent) {
    ...
    if (user.role === 'USER' && consent.userId !== user.id) {
      throw new ConsentControllerError('Unauthorized to access this consent', 403, 'ACCESS_DENIED');
    }
  ...
  static async decryptConsentData(consent) {
    ...
    try {
      ...
      throw new ConsentControllerError('Failed to decrypt consent data', 500, 'DECRYPTION_ERROR');
    }
  }
  ...
  static async calculateConsentStatistics() {
    ...
    const monthly = await POPIAConsent.findAll({
      attributes: [
        [
          POPIAConsent.sequelize.fn('DATE_TRUNC', 'month', POPIAConsent.sequelize.col('createdAt')),
          'month',
        ],
        [POPIAConsent.sequelize.fn('COUNT', POPIAConsent.sequelize.col('id')), 'count'],
      ],
    ...
    stats.withdrawalRate = stats.totalConsents > 0 ? (withdrawals / stats.totalConsents) * 100 : 0;
  ...
  static async calculateComplianceMetrics() {
    ...
    compliance.popiaComplianceRate = totalConsents > 0 ? (validConsents / totalConsents) * 100 : 0;
