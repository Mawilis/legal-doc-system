...
const ReportValidationSchemas = {
  ...
  complianceReport: Joi.object({
    ...
    complianceType: Joi.string()
      ...
      .default('ALL'),
    riskLevel: Joi.string().valid('LOW', 'MEDIUM', 'HIGH', 'CRITICAL', 'ALL').default('ALL'),
    format: Joi.string().valid('json', 'pdf', 'csv', 'excel').default('json'),
...
class QuantumReportController {
  ...
  async getComplianceReport(req, res) {
    try {
      ...
      return this.sendFormattedResponse(res, encryptedReport, format, {
        ...
        legalDisclaimer:
          'This report contains confidential compliance information. Protected under POPIA.',
      });
    } catch (error) {
      await this.logReportError({
        userId: req.user?.userId,
              ...
              successRate: {
                $cond: [
                  ...
                  0,
                  {
                    $round: [
                      {
                        $multiply: [{ $divide: ['$successfulDeliveries', '$totalVolume'] }, 100],
                      },
                      2,
                    ],
                  },
              ...
              failureRate: {
                $cond: [
                  ...
                  0,
                  {
                    $round: [
                      {
                        $multiply: [{ $divide: ['$failedDeliveries', '$totalVolume'] }, 100],
                      },
                      2,
                    ],
                  },
                ...
                },
                {
                  $cond: [
                    { $gt: ['$totalDistance', 0] },
                    {
                      $multiply: [
                        {
                          $divide: [100, { $divide: ['$totalCost', '$totalDistance'] }],
                        },
                        0.5,
                      ],
                    },
      ...
      const retentionAnalysis = await Document.aggregate([
        ...
        },
        {
          $group: {
            ...
            complianceScore: {
              $avg: {
                $cond: [{ $eq: ['$_id.retentionStatus', 'ARCHIVED'] }, 100, 0],
              },
            },
          },
        },
              ...
              deadlineCompliance: {
                $cond: [
                  ...
                  0,
                  {
                    $round: [
                      {
                        $multiply: [{ $divide: ['$totalWithinDeadline', '$totalRequests'] }, 100],
                      },
                      2,
                    ],
                  },
    ...
    try {
      ...
      return successResponse(req, res, {
        ...
        scheduleInfo: value.schedule
          ? 'Report scheduled for automatic generation'
          : 'One-time report',
      });
    } catch (error) {
      await this.logReportError({
  ...
  async streamReport(req, res) {
    try {
      ...
      if (!reportMetadata) {
        return errorResponse(req, res, 404, 'Report not found', 'ERR_REPORT_NOT_FOUND');
      }
  ...
  async calculateAgedReceivables(startDate, endDate, firmId) {
    ...
    agedReceivables.forEach((item) => {
      item.percentage = totalAmount > 0 ? Math.round((item.totalAmount / totalAmount) * 100) : 0;
    });
  ...
  async analyzeFICACompliance(startDate, endDate, firmId, riskLevel) {
    ...
    return {
      ...
      recommendations: verificationRate < 95 ? ['Complete FICA verification for all clients'] : [],
    };
  }
  ...
  async generateSARSReference(firmId) {
    const firm = await Firm.findById(firmId)
      .select('sarsReference companyRegistrationNumber')
      .lean();
  ...
  generateLogisticsSummary(logisticsStats) {
    ...
    return {
      ...
      overallSuccessRate:
        totalDeliveries > 0 ? Math.round((totalSuccessful / totalDeliveries) * 100) : 0,
