...
try {
  ...
  const clientSchema = new mongoose.Schema({
    ...
    kycStatus: {
      type: String,
      enum: ['PENDING', 'VERIFIED', 'ENHANCED_DD_REQUIRED', 'DECLINED'],
      default: 'PENDING',
    },
    ficaStatus: {
      type: String,
      enum: ['NOT_REQUIRED', 'PENDING', 'COMPLIANT', 'NON_COMPLIANT'],
...
try {
  ...
  const userSchema = new mongoose.Schema({
    ...
    role: {
      type: String,
      enum: ['ASSOCIATE', 'PARTNER', 'COMPLIANCE_OFFICER', 'RISK_MANAGER', 'SYSTEM_ADMIN'],
      required: true,
    },
    ...
    return {
      createAssessment: Joi.object({
        ...
        riskCategory: Joi.string()
          .required()
          .valid(...Object.keys(RISK_CONSTANTS.RISK_CATEGORIES)),
        ...
        ficaCompliance: Joi.object({
          clientType: Joi.string()
            .valid('INDIVIDUAL', 'COMPANY', 'TRUST', 'PARTNERSHIP')
            .required(),
        ...
        supportingDocuments: Joi.array()
          .items(
            Joi.object({
              ...
              type: Joi.string()
                .valid(...RISK_CONSTANTS.VALIDATION.ALLOWED_FILE_TYPES)
                .required(),
        ...
        mitigationActions: Joi.array()
          .items(
            Joi.object({
              ...
              status: Joi.string()
                .valid('PENDING', 'IN_PROGRESS', 'COMPLETED', 'VERIFIED', 'FAILED')
                .required(),
            ...
            errors: [
              {
                ...
                message:
                  'Enhanced Due Diligence required for PEP or sanctioned clients per FICA §22',
  ...
  static async validateUpdateAssessment(data, existingAssessment) {
    try {
      ...
      return {
        isValid: false,
        errors: [
          { field: 'validation', message: 'Update validation system error', type: 'system' },
        ],
        sanitizedData: null,
      };
    }
    ...
    try {
      ...
      const report = {
        ...
        complianceAnalysis: {
          ...
          companiesAct: {
            status: complianceCheck.frameworks.companiesAct.compliant
              ? 'COMPLIANT'
              : 'NON_COMPLIANT',
        ...
        riskAssessmentSummary: {
          ...
          completedActions:
            assessment.mitigationActions?.filter((a) => a.status === 'COMPLETED').length || 0,
        },
...
exports.createRiskAssessment = [
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const assessmentId = `RA-${moment().format('YYYYMMDD')}-${uuidv4()
        .substr(0, 8)
        .toUpperCase()}`;
      ...
      res.status(201).json({
        ...
        metadata: {
          ...
          complianceCheck: {
            ...
            companiesAct: complianceCheck.frameworks.companiesAct.compliant
              ? 'COMPLIANT'
              : 'NON_COMPLIANT',
          },
    ...
    try {
      ...
      if (assessmentAge < requiredRetention && !hardDelete) {
        ...
        return res.status(400).json({
          ...
          canDeleteAfter: moment(assessment.createdAt)
            .add(requiredRetention, 'years')
            .format('YYYY-MM-DD'),
          complianceReference: 'COMPANIES_ACT_SECTION_24',
        });
      }
      ...
      const report = {
        ...
        digitalSignature: {
          ...
          signatureHash: crypto
            .createHash('sha256')
            .update(JSON.stringify(complianceReport))
            .digest('hex'),
          algorithm: 'SHA-256',
    ...
    try {
      ...
      if (!['COMPLIANCE_OFFICER', 'PARTNER', 'RISK_MANAGER'].includes(userRole)) {
        return res.status(403).json({
          ...
          message:
            'Compliance status access requires COMPLIANCE_OFFICER, PARTNER, or RISK_MANAGER role',
          userRole,
      ...
      const complianceStatus = {
        ...
        frameworkCompliance: {
          POPIA: {
            ...
            status:
              popiaComplianceRate >= 90
                ? 'COMPLIANT'
                : popiaComplianceRate >= 70
                  ? 'PARTIALLY_COMPLIANT'
                  : 'NON_COMPLIANT',
          },
          FICA: {
            ...
            status:
              ficaComplianceRate >= 90
                ? 'COMPLIANT'
                : ficaComplianceRate >= 70
                  ? 'PARTIALLY_COMPLIANT'
                  : 'NON_COMPLIANT',
          },
          COMPANIES_ACT: {
            ...
            status:
              companiesActComplianceRate >= 90
                ? 'COMPLIANT'
                : companiesActComplianceRate >= 70
                  ? 'PARTIALLY_COMPLIANT'
                  : 'NON_COMPLIANT',
          },
        },
