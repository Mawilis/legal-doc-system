...
exports.globalSearch = asyncHandler(async (req, res) => {
  ...
  const [clients, cases, documents] = await Promise.all([
    ...
    Document.find({
      ...
      .limit(searchLimit)
      .select('title documentCode status')
      .lean(),
  ]);
...
exports.searchDocuments = asyncHandler(async (req, res) => {
  ...
  const query = {
    ...req.tenantFilter,
    $or: [{ title: { $regex: q, $options: 'i' } }, { documentCode: { $regex: q, $options: 'i' } }],
  };

  if (status) query.status = status;
  if (type) query.type = type;

  const documents = await Document.find(query).sort({ updatedAt: -1 }).limit(100).lean();
