...
const firmSettingsSchema = Joi.object({
  ...
  billing: Joi.object({
    ...
    paymentTerms: Joi.string()
      .valid('IMMEDIATE', '7_DAYS', '14_DAYS', '30_DAYS', '60_DAYS')
      .default('30_DAYS'),
  }),
  settings: Joi.object({
...
exports.getFirmAnalytics = asyncHandler(async (req, res) => {
  ...
  res.status(200).json({
    ...
    meta: {
      generatedAt: new Date(),
      periodCovered: `${dateRange.startDate.toISOString().split('T')[0]} to ${
  ...
  try {
    ...
    return {
      ...
      acquisition: {
        ...
        growthRate:
          previousClients > 0 ? ((clients.length - previousClients) / previousClients) * 100 : 100,
      },
      retention: {
        rate: retentionRate,
  ...
  try {
    ...
    return {
      ...
      summary: {
        ...
        improvement:
          trends.length > 1
            ? trends[trends.length - 1].scores.overall - trends[0].scores.overall
            : 0,
        bestArea:
          Object.entries(trends[trends.length - 1]?.scores || {})
            .filter(([k]) => k !== 'overall')
  ...
  try {
    ...
    return {
      ...
      summary: {
        ...
        topPerformer:
          productivity.length > 0
            ? productivity.sort((a, b) => b.documents - a.documents)[0]
            : null,
      },
      period: dateRange,
    };
  ...
  try {
    ...
    return {
      ...
      costs: {
        ...
        currency: 'ZAR',
        costPerDocument:
          documents.length > 0 ? (totalCost / documents.length).toFixed(4) : '0.0000',
      },
      optimization: {
...
async function getFirmNotifications(tenantId, firm) {
  ...
  const complianceScore = await ComplianceService.getFirmComplianceScore(tenantId).catch(() => ({
    score: 0,
  }));
  ...
  switch (firmPlan) {
    ...
    case 'ENTERPRISE':
      ...
      break; // 1TB
    default:
      storageLimit = 5 * 1024 * 1024 * 1024; // 5GB for Trial
  }
