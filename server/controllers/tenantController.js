...
exports.getComplianceDashboard = async (req, res, next) => {
  try {
    ...
    if (!complianceData) {
      ...
      const popiaScore = popiaStatus.compliant
        ? 100
      ...
      complianceData = {
        ...
        breakdown: {
          popia: {
            score: popiaScore,
            status: popiaStatus.compliant ? 'COMPLIANT' : 'NON_COMPLIANT',
          },
...
exports.createTenant = async (req, res, next) => {
  ...
  try {
    ...
    const newTenant = new Tenant({
      ...
      trialEndsAt: moment()
        .add(process.env.DEFAULT_TRIAL_DAYS || 14, 'days')
        .toDate(),
      createdBy: 'SYSTEM',
      isActive: true,
      settings: {
