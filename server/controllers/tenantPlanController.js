...
async function getPlanWithOverrides(tenantId) {
  try {
    ...
    const tenant = await Tenant.findById(tenantId).select('plan billing settings features').lean();
    ...
    const effectivePlan = {
      ...
      overrides: !!overrides.customPricing,
...
async function applyPlanOverride(tenantId, overrideData, actorId) {
  ...
  try {
    ...
    if (overrideData.customFeatures) {
      await FeatureFlagService.updateTenantFeatures(tenantId, overrideData.customFeatures, actorId);
    }

    // 5. Sync with billing system
    await BillingService.updateSubscriptionFromOverride(tenantId, overrideData, actorId).catch(
      (err) => {
...
exports.listAllPlans = async (req, res, next) => {
  try {
        ...
        try {
          ...
          return {
            ...
            pricing: {
              ...
              vatIncluded:
                plan.customPricing?.vatIncluded !== undefined
            ...
            usage: {
              ...
              utilization:
                (usage.users / (plan.customLimits?.maxUsers || basePlan.features.maxUsers)) * 100,
            },
            billing: await BillingService.getBillingStatus(tenant._id).catch(() => ({
              status: 'UNKNOWN',
            })),
    ...
    const summary = {
      ...
      overrideRate:
        (enrichedTenants.filter((t) => t.hasOverride).length / enrichedTenants.length) * 100,
    };
...
exports.manageFeature = async (req, res, next) => {
  try {
    ...
    const result = await FeatureFlagService.manageFeature(tenantId, featureRequest, req.user._id);
    ...
    await emitPlanAudit({
      ...
      } ${featureRequest.action.toLowerCase()}d for tenant ${tenantId}`,
      metadata: {
        featureRequest,
        result,
    ...
    res.status(200).json({
      ...
      message: `Feature ${
        featureRequest.feature
      } ${featureRequest.action.toLowerCase()}d successfully`,
      data: result,
    });
  } catch (err) {
...
exports.checkCompliance = async (req, res, next) => {
  try {
    ...
    const [tenant, plan] = await Promise.all([
      Tenant.findById(tenantId).select('plan billing lpcNumber province').lean(),
...
function calculateTotalMRR(tenants) {
  return tenants.reduce((total, tenant) => total + (tenant.pricing.monthly || 0), 0);
}
