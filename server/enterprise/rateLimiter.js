...
class HighPrecisionRateLimiter {
  reset() {
    ...
    if (this.hits && typeof this.hits.clear === 'function') {
      this.hits.clear();
    }
    return true;
  }
  ...
  _logInitialization() {
    ...
    console.log(
      `║  • Platinum: ${this.defaultQPS * 10} QPS | Gold: ${this.defaultQPS * 5} QPS | Silver: ${this.defaultQPS} QPS`
    );
  ...
  getStats() {
    ...
    const errorRate = total > 0 ? ((this.stats.blocked / total) * 100).toFixed(2) + '%' : '0.00%';
