...
const validateQuantumEnvironment = () => {
  ...
  if (key.length !== 32) {
    throw new Error('[FICA] FICA_ENCRYPTION_KEY must be 64 hex characters (32 bytes) for AES-256');
  }
...
const verifyAddress = async (encryptedAddress, verificationId) => {
  ...
  for (const provider of providers) {
    const result = await executeProviderRequest(provider, provider.endpoints.individual, {
      ...payload,
      checkType: 'address',
    });
