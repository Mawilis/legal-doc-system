...
const HLF_CONSTANTS = {
  ...
  // Network Policies
  ENDORSEMENT_POLICY: {
    AND: [{ role: { name: 'peer', mspId: process.env.HLF_ORG_MSP_ID } }],
  },
...
class HyperledgerClient {
  ...
  async submitTransaction(transactionData) {
    ...
    while (retryCount < HLF_CONSTANTS.MAX_RETRIES) {
      try {
        ...
        transaction.setEndorsingPeers(connection.network.getChannel().getEndorsers());
  ...
  async verifyRecord(recordType, recordId) {
    try {
      ...
      const clientVerification = {
        ...
        merkleProofValid: record.record.merkleProof ? this.verifyMerkleProof(record.record) : true,
      };
      ...
      return {
        ...
        clientVerification,
        overallValid:
          verification.valid && Object.values(clientVerification).every((v) => v === true),
  ...
  validateTransactionData(data) {
    ...
    if (typeof data.data !== 'object' || Array.isArray(data.data)) {
      throw new ValidationError('Data field must be a JSON object', 'data');
    }
  ...
  verifyHashIntegrity(record) {
    ...
    const calculatedHash = createHash('sha256').update(JSON.stringify(record.data)).digest('hex');
...
const logToBlockchain = async (data) =>
  await hyperledgerClient.submitTransaction({
...
if (process.env.NODE_ENV === 'test' || process.env.RUN_HLF_TESTS === 'true') {
  (async () => {
    ...
    try {
      ...
      if (leaf.hash && leaf.hash.length === 64) {
        // SHA256 hex length
