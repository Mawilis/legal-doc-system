...
class CompanyCrypto {
  ...
  _deriveTenantKey(tenantId) {
    ...
    const hkdf = crypto.createHmac('sha256', salt).update(this.masterKey).digest();

    const derivedKey = crypto.createHmac('sha256', hkdf).update(info).digest().slice(0, KEY_LENGTH); // Truncate to required length
  ...
  async encryptCompanyObject(companyData, fieldsToEncrypt, tenantId, context = {}) {
    ...
    for (const field of fieldsToEncrypt) {
      if (companyData[field] !== undefined && companyData[field] !== null) {
        try {
          const payload = await this.encryptField(String(companyData[field]), tenantId, {
            ...context,
            field,
          });
   ...
   */
  _hashField(value) {
    return crypto.createHash('sha256').update(String(value)).digest('hex').substring(0, 16); // Truncated for storage efficiency
  }
