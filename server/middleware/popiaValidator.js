...
const QUANTUM_CONFIG = {
  ...
  ANONYMIZATION_SALT:
    process.env.POPIA_ANONYMIZATION_SALT || 'default-popia-salt-change-in-production',
  SALT_ROUNDS: 12,
  ...
  PII_PATTERNS: {
    ...
    ADDRESS:
      /\b(\d+)\s+([A-Za-z\s]+)(\s+(Street|St|Avenue|Ave|Road|Rd|Drive|Dr|Lane|Ln|Court|Ct))?\\,?\s+([A-Za-z\s]+)\\,?\s+(\d{4})\b/i,
  },
...
const POPIA_VALIDATION_SCHEMAS = {
  ...
  CLIENT_REGISTRATION: Joi.object({
    ...
    personalInfo: Joi.object({
      ...
      // Contact Information
      email: Joi.string().email().required().description('Email address - PII'),
      phone: Joi.string()
        .pattern(QUANTUM_CONFIG.PII_PATTERNS.PHONE)
        .required()
    ...
    popiaConsent: Joi.object({
      ...
      purpose: Joi.string().max(500).required().description('Purpose of data processing'),
  ...
  MATTER_CREATION: Joi.object({
    ...
    clientId: Joi.string()
      .pattern(/^CLIENT-[A-Z0-9]{8}-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{12}$/)
      .required(),
  ...
  DOCUMENT_UPLOAD: Joi.object({
    ...
    documentType: Joi.string()
      .valid('CONTRACT', 'AFFIDAVIT', 'PLEADING', 'EVIDENCE', 'CORRESPONDENCE', 'OTHER')
      .required(),
    ...
    consentReference: Joi.string().when('piiClassification.containsPII', {
      is: true,
      then: Joi.string()
        .pattern(/^CONSENT-[A-Z0-9]{8}-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{12}$/)
        .required(),
  ...
  DSAR_REQUEST: Joi.object({
    requestType: Joi.string()
      .valid('ACCESS', 'CORRECTION', 'DELETION', 'OBJECTION', 'COMPLAINT')
      .required(),
    ...
    identityVerification: Joi.object({
      ...
      verificationMethod: Joi.string()
        .valid('ID_DOCUMENT', 'BANK_VERIFICATION', 'BIOMETRIC')
        .required(),
...
class POPIAValidatorMiddleware {
    ...
    return async (req, res, next) => {
      try {
        ...
        return res.status(500).json({
          ...
          details:
            process.env.NODE_ENV === 'development' ? error.message : 'Contact Information Officer',
  ...
  static async _detectAndHandlePII(req, options) {
    ...
    req.piiMetadata = {
      ...
      riskLevel:
        piiDetection.detectedPII.length > 0
          ? _.maxBy(piiDetection.detectedPII, 'riskLevel')?.riskLevel || 'LOW'
          : 'LOW',
  ...
  static _validateSecuritySafeguards(req) {
    ...
    const securityHeaders = ['x-api-key', 'authorization', 'x-csrf-token'];
  ...
  static async _validateConsentForProcessing(req) {
    const consentToken = req.headers['x-popia-consent'] || req.body?.popiaConsentToken;
  ...
  static async _logValidationSuccess(req, validationId, piiDetection, popiaCompliance) {
    const logEntry = {
      ...
      timestamp: new Date().toISOString(),
      durationMs:
        Date.now() - new Date(popiaCache.get(`validation:${validationId}`)?.startedAt).getTime(),
    };
  ...
  static _validateSAIDNumber(idNumber) {
    ...
    const sum = digits
      .slice(0, 12)
      .reduce((acc, digit, index) => acc + digit * ((index % 2) + 1), 0);
