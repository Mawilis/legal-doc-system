...
const RBAC_CONFIG = {
  ...
  LEGAL_ROLES: {
    ...
    FINANCE_OFFICER: {
      ...
      permissions: ['BILLING_ALL', 'REPORT_FINANCIAL', 'CLIENT_VIEW', 'DOCUMENT_VIEW_BILLING'],
      scope: 'TENANT',
      canDelegated: false,
      requiresMFA: true,
      auditLevel: 'HIGH',
    ...
    CLIENT_REPRESENTATIVE: {
      ...
      permissions: ['DOCUMENT_VIEW_OWN', 'CASE_VIEW_OWN', 'MESSAGES', 'BILLING_VIEW'],
      scope: 'CLIENT',
      canDelegated: false,
      requiresMFA: true,
      auditLevel: 'MEDIUM',
  ...
  SCOPES: {
    ...
    TEAM: {
      ...
      allowedRoles: ['ADVOCATE', 'ATTORNEY', 'CANDIDATE_ATTORNEY', 'PARALEGAL', 'EXTERNAL_COUNSEL'],
    },
...
const getRoleConfig = (role) => {
  ...
  return (
    RBAC_CONFIG.LEGAL_ROLES[normalizedRole] || {
      level: 0,
...
const getPermissionConfig = (permission) => {
  ...
  return (
    RBAC_CONFIG.PERMISSIONS[normalizedPermission] || {
      resource: 'UNKNOWN',
      action: 'UNKNOWN',
      level: 0,
    }
 ...
 */
exports.requirePermission = (requiredPermission) => async (req, res, next) => {
  const startTime = performance.now();
 ...
 */
exports.requireResourcePermission = (resourceType, action) => async (req, res, next) => {
  const startTime = performance.now();
 ...
 */
exports.requireFeature = (featureKey) => async (req, res, next) => {
  const startTime = performance.now();
  ...
  try {
    ...
    const { subscriptionTier } = mockTenant;
    const { features } = mockTenant;
