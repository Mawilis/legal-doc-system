...
const REQUIRED_ENV_VARS = ['NODE_ENV', 'ENCRYPTION_KEY'];
...
class RegulatoryComplianceEnforcer extends EventEmitter {
  constructor(config = {}) {
    ...
    this.ruleEngine = complianceRuleEngine
      ? complianceRuleEngine()
      : this.createFallbackRuleEngine();
  ...
  async handleAllowAction(req, res, enforcementAction) {
    ...
    return {
      ...
      headersAdded: [
        'X-Compliance-Status',
        'X-Compliance-Enforcement-Id',
        'X-Compliance-Timestamp',
      ],
      performance: {
        totalMs: Date.now() - startTime,
      },
  ...
  async handleBlockAction(req, res, enforcementAction) {
    ...
    const blockResponse = {
      ...
      timestamp: new Date().toISOString(),
      violations:
        enforcementAction.violations?.map((v) => ({
          ruleId: v.ruleId,
          severity: v.severity,
  ...
  initEnforcementQueues() {
    ...
    try {
      ...
      this.enforcementWorker = new BullMQ.Worker(
        'enforcement-processing',
        async (job) => await this.processEnforcementJob(job.data),
        {
          connection: this.redisClient,
          concurrency: 10,
          limiter: {
  ...
  getActionForSeverity(severity) {
    switch (severity) {
      ...
      case 2:
        return ENFORCEMENT_CONSTANTS.ENFORCEMENT_ACTIONS.REQUIRE_AUTH;
      case 1:
        return ENFORCEMENT_CONSTANTS.ENFORCEMENT_ACTIONS.WARN;
      default:
        return ENFORCEMENT_CONSTANTS.ENFORCEMENT_ACTIONS.LOG_ONLY;
    }
  }
  ...
  async checkRuleEngineIntegration() {
    try {
      ...
      const testResult = await this.ruleEngine.evaluateRule('TEST_RULE', 'TEST_CONTEXT', {
        test: true,
      });
  ...
  async checkRateLimitingFunctionality() {
    try {
      ...
      return {
        ...
        message:
          error.constructor.name === 'RateLimiterRes'
