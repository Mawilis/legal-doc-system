...
AIUsageSchema.methods.calculateFinancials = async function () {
  ...
  const finalCostZAR = Math.max(costZAR, pricingModel.minimumCharge);
...
AIUsageSchema.methods.enforceCompliance = function () {
  ...
  this.compliance.vatCompliant = this.jurisdiction === 'RSA' ? this.financials.vatAmount > 0 : true;
...
AIUsageSchema.statics.calculateForecast = function (summary, startDate, endDate) {
  ...
  const forecast = {
    ...
    annualProjection: {
      ...
      valueGenerated:
        summary.valueGenerated.totalValueZAR > 0
...
AIUsageSchema.statics.identifyGrowthOpportunities = function (summary) {
  ...
  if (summary.financials.costPerToken > 0.01) {
    // More than 1 cent per token
    opportunities.push({
