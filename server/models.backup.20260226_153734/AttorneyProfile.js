...
import { redactSensitiveData } from '../utils/popiaRedaction.js.js';
...
const attorneyProfileSchema = new Schema(
  {
    ...
    personalInfo: {
      ...
      passportNumber: {
        type: String,
        sparse: true,
        validate: { validator: (v) => !v || /^[A-Z]{2}\d{7}$/.test(v) },
      },
    ...
    contact: {
      ...
      phone: {
        type: String,
        required: true,
        validate: { validator: (v) => /^(\+27|0)[1-9][0-9]{8}$/.test(v) },
      },
      alternativePhone: {
        type: String,
        validate: { validator: (v) => !v || /^(\+27|0)[1-9][0-9]{8}$/.test(v) },
      },
      fax: {
        type: String,
        validate: { validator: (v) => !v || /^(\+27|0)[1-9][0-9]{8}$/.test(v) },
    ...
    disciplinaryHistory: [
      {
        ...
        imposedAt: Date,
        resolvedAt: Date,
        status: { type: String, enum: ['PENDING', 'UNDER_INVESTIGATION', 'RESOLVED', 'APPEALED'] },
        notes: String,
        recordedBy: String,
...
attorneyProfileSchema.virtual('fullAddress').get(function () {
  const a = this.address?.physical;
  return a ? `${a.line1}, ${a.suburb}, ${a.city}, ${a.province}, ${a.postalCode}` : '';
});
attorneyProfileSchema.virtual('isFidelityValid').get(function () {
  return this.fidelityFund?.status === 'ACTIVE' && this.fidelityFund?.expiryDate > new Date();
});
attorneyProfileSchema.virtual('isTrustCompliant').get(function () {
  return this.trustAccount?.isActive === true && this.trustAccount?.complianceScore >= 70;
});
...
attorneyProfileSchema.virtual('daysUntilFidelityExpiry').get(function () {
  return this.fidelityFund?.expiryDate
    ? Math.ceil((this.fidelityFund.expiryDate - Date.now()) / (1000 * 60 * 60 * 24))
    : null;
});
attorneyProfileSchema.virtual('complianceScore').get(function () {
  let s = 100;
  if (!this.isFidelityValid) s -= 30;
  if (!this.isTrustCompliant) s -= 25;
  if (!this.isCPDCompliant) s -= 20;
  if (this.disciplinaryHistory?.length > 0) s -= 15;
  return Math.max(0, s);
});
...
attorneyProfileSchema.statics = {
  ...
  async findCPDNonCompliant(tenantId, y = new Date().getFullYear()) {
    return this.find({
      tenantId,
      status: 'ACTIVE',
      deleted: false,
  ...
  async search(tenantId, q) {
    return this.find({
      ...
      deleted: false,
      $or: [
        { lpcNumber: new RegExp(q, 'i') },
        { 'personalInfo.fullName': new RegExp(q, 'i') },
        { 'practice.name': new RegExp(q, 'i') },
      ],
    }).limit(50);
  },
};
...
attorneyProfileSchema.methods = {
  canPractice() {
    return this.status === 'ACTIVE' && this.isFidelityValid === true && !this.deleted;
  },
  async calculateComplianceScore() {
    let s = 100;
    if (!this.isFidelityValid) s -= 30;
    if (!this.trustAccount?.isActive) s -= 25;
  ...
  async updateCPDStatus(h, e, uid) {
    this.cpd.hoursCompleted += h;
    this.cpd.ethicsHours += e;
  ...
  async updateFidelityCertificate(fd, uid) {
    this.fidelityFund = {
      ...this.fidelityFund,
      ...fd,
      status: 'ACTIVE',
  ...
  async generateAuditReport() {
    const cs = await this.calculateComplianceScore();
    return {
      attorneyId: this._id,
      ...
      integrity: { hash: this.integrityHash, signature: this.quantumSignature },
      auditTrail: this.auditTrail.slice(-10),
      generatedAt: new Date().toISOString(),
      generatedBy: 'WilsyOS Attorney Forensic Engine v5.0.1',
    };
  },
};
