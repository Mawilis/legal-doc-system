...
const UNIT_TYPES = Object.freeze([
  ...
  },
  {
    ...
    min: 0.1,
    max: 10000,
    description: 'Travel distance (SARS rates)',
  },
  {
    code: 'FLAT_RATE',
    precision: 2,
    min: 0.01,
    max: 10000000,
    description: 'Fixed amount services',
  },
  {
    code: 'PERCENTAGE',
    precision: 4,
...
const feeSchema = new Schema(
  {
    ...
    requiresApproval: {
      ...
      validate: {
        validator(v) {
          ...
          if (this.unit === 'PERCENTAGE' && this.amount > 0.25) {
            // > 25%
            return v === true;
          }
          return true;
    ...
    integrityHash: {
      ...
      default() {
        ...
        return crypto
          .createHmac('sha256', process.env.FEE_AUDIT_SECRET)
...
feeSchema.statics.findApplicableRate = async function findApplicableRate(
  ...
  if (!fee && options.allowCategoryFallback) {
    ...
    const categoryFee = await this.findOne(categoryQuery).sort({ effectiveDate: -1 }).exec();
...
feeSchema.statics.generateTariffReport = async function generateTariffReport(
  ...
  const matchStage = {
    $match: {
      $or: [{ tenantId: tenantId ? mongoose.Types.ObjectId(tenantId) : null }, { isGlobal: true }],
      isActive: true,
  ...
  const report = await this.aggregate([matchStage, groupStage, { $sort: { '_id.category': 1 } }]);
...
feeSchema.post('save', async (doc) => {
  ...
  if (process.env.REDIS_ENABLED === 'true') {
    const cacheKeys = [`fee:${doc._id}`, `tenant_fees:${doc.tenantId}`, 'global_fees'];
