...
onboardingDocumentSchema.index({
  tenantId: 1,
  documentType: 1,
  'processingStatus.verification.status': 1,
});
...
// Bulk soft delete
onboardingDocumentSchema.static(
  'bulkSoftDelete',
  async function (documentIds, tenantId, deletedBy, reason, authorization = null) {
...
onboardingDocumentSchema.static('generateDocumentId', (documentType) => {
...
onboardingDocumentSchema.method(
  'updateProcessingStatus',
  async function (type, status, result = {}, performedBy = null) {
...
onboardingDocumentSchema.method(
  'addToChainOfCustody',
  async function (action, performedBy, options = {}) {
    const entry = {
      action,
      performedBy,
      ...
      correlationId:
        options.correlationId || `corr_${Date.now()}_${crypto.randomBytes(8).toString('hex')}`,
      timestamp: new Date(),
    };
...
onboardingDocumentSchema.method(
  'softDelete',
  async function (deletedBy, reason, authorization = null) {
    this.isDeleted = true;
...
onboardingDocumentSchema.method(
  'placeOnLegalHold',
  async function (reason, initiatedBy, caseNumber = null) {
...
onboardingDocumentSchema.method('verifyIntegrity', function () {
  ...
  const currentHash = crypto.createHash('sha256').update(this.fileData).digest('hex');
...
onboardingDocumentSchema.pre('save', function (next) {
  ...
  if (!this.fileHash && this.fileData) {
    this.fileHash = crypto.createHash('sha256').update(this.fileData).digest('hex');
  }
