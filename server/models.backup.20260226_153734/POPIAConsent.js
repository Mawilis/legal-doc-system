...
class POPIAConsent extends Model {
    ...
    try {
      ...
      return {
        ...
        compliance: {
          ...
          specialInfoRequirementsMet:
            !specialInfoCheck.requiresExplicitConsent || consentData.explicitConsent,
        },
      };
    } catch (error) {
        ...
        if (category.toLowerCase().includes(specialType.toLowerCase())) {
          detectedTypes.push({
            ...
            requiresExplicitConsent:
              SPECIAL_PERSONAL_INFORMATION[specialType].requiresExplicitConsent,
          });
        }
      });
    });
  ...
  static async applyQuantumEncryption(consentData, specialInfoCheck) {
    ...
    if (process.env.HOMOMORPHIC_ENCRYPTION === 'true' && specialInfoCheck.hasSpecialInfo) {
      ...
      return {
        ...
        encryptedDataCategories: await homomorphic.encrypt(
          JSON.stringify(dataToEncrypt.dataCategories),
        ),
        encryptedProcessingActivities: await homomorphic.encrypt(
          JSON.stringify(dataToEncrypt.processingActivities),
        ),
        encryptionMetadata: {
          algorithm: 'HE_FHE',
          keyVersion: '2.0',
    ...
    return {
      ...
      encryptedProcessingActivities: await encryptField(
        JSON.stringify(dataToEncrypt.processingActivities),
      ),
      encryptionMetadata: {
  ...
  static async generateConsentCertificate(consent) {
    ...
    if (consent.encryptionMetadata?.homomorphic) {
      ...
      processingActivities = JSON.parse(
  ...
  async generateWithdrawalCertificate() {
    const certificateId = `WITHDRAW-${Date.now()}-${crypto
      .randomBytes(4)
      .toString('hex')
      .toUpperCase()}`;
  ...
  async notifyThirdPartyWithdrawal(thirdParty) {
    ...
    console.log(
      `Notifying third party ${thirdParty.name} of consent withdrawal for consent ${this.id}`,
    );
...
POPIAConsent.init(
  {
    ...
    validity: {
      type: DataTypes.ENUM('VALID', 'INVALID', 'EXPIRED', 'WITHDRAWN', 'UNDER_REVIEW'),
      allowNull: false,
      defaultValue: 'VALID',
      index: true,
    },
    ...
    withdrawalMethod: {
      type: DataTypes.ENUM('EMAIL', 'PORTAL', 'PHONE', 'IN_PERSON', 'AUTOMATED'),
      allowNull: true,
    },
    ...
    describe('QUANTUM POPIA CONSENT MODEL TEST SUITE', () => {
      ...
      describe('QUANTUM STATIC METHODS', () => {
        ...
        it('should validate consent data correctly', async () => {
          ...
          await expect(POPIAConsent.validateQuantumConsent(validConsentData)).resolves.toBe(true);
        });
      ...
      describe('QUANTUM INSTANCE METHODS', () => {
        it('should validate withdrawal correctly', async () => {
          ...
          await expect(testConsent.validateWithdrawal(withdrawalData)).resolves.toBe(true);
        });
        ...
        it('should reject withdrawal for inactive consent', async () => {
          ...
          await expect(testConsent.validateWithdrawal(withdrawalData)).rejects.toThrow(
            'Cannot withdraw consent with status: EXPIRED',
          );
        });
