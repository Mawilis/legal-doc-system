 ...
 * Add audit log entry
 */
retentionPolicySchema.methods.addAuditLog = async function (
  action,
  userId,
  changes = {},
  req = {},
  ...
  return this.find({
    ...
    $or: [
      { reviewFrequency: 'annually', updatedAt: { $lte: oneYearAgo } },
      {
        ...
        updatedAt: { $lte: new Date(Date.now() - 180 * 24 * 60 * 60 * 1000) },
      },
      {
...
retentionPolicySchema.statics.getComplianceReport = async function (tenantId) {
  const stats = await this.aggregate([
    ...
    {
      $facet: {
        byMatterType: [{ $group: { _id: '$matterType', count: { $sum: 1 } } }],
        byLegalBasis: [{ $group: { _id: '$legalBasis', count: { $sum: 1 } } }],
        byRetentionYears: [{ $group: { _id: '$retentionYears', count: { $sum: 1 } } }],
        byDataResidency: [{ $group: { _id: '$dataResidency', count: { $sum: 1 } } }],
        complianceMetrics: [
          {
            $group: {
              _id: null,
