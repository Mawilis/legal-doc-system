...
const validationAuditSchema = new mongoose.Schema(
  {
    ...
    retentionPolicy: {
      type: String,
      enum: {
        values: ['companies_act_10_years', 'popia_1_year', 'tax_act_5_years', 'forensic_permanent'],
      },
...
validationAuditSchema.index({
  'complianceFrameworks.framework': 1,
  'complianceFrameworks.verified': 1,
});
...
validationAuditSchema.statics.getRecordValidationHistory = function (targetModel, targetId) {
  return this.find({ targetModel, targetId }).sort({ timestamp: -1 }).limit(50).lean();
};
...
validationAuditSchema.statics.verifyHashChain = async function (tenantId, fromDate = null) {
  ...
  const validations = await this.find(query).sort({ timestamp: 1 }).lean();
