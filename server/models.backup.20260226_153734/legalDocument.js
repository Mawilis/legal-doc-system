...
const LegalDocumentSchema = new Schema(
  {
    ...
    storagePath: {
      // Path in S3/Azure Blob/Quantum Storage
      type: String,
      required: true,
      match: [/^https?:\/\/.*|^\/[^\\/].*$/, 'Invalid storage path format'],
    },
    fileHash: {
      // Quantum Shield: SHA-256 for integrity verification
      type: String,
      required: true,
    ...
    applicableRegulations: {
      type: [
        {
          code: {
            // e.g., POPIA, GDPR, COMPANIES_ACT_2008
            type: String,
            uppercase: true,
            required: true,
          },
    ...
    popiaCompliance: {
      ...
      dataSubjectConsentObtained: {
        type: Boolean,
        required() {
          return this.popiaCompliance.personalInformationIdentified === true;
        },
      },
    ...
    accessControlMatrix: {
      ...
      publicAccess: {
        // For PAIA Section 23 manual disclosures
        type: Boolean,
        default: false,
      },
    ...
    electronicSignatures: [
      {
        ...
        signatureData: {
          // Encrypted payload from SignRequest/DocuSign
          type: String,
          required: true,
        },
        timestamp: {
        ...
        isAdvanced: {
          // ECT Act "Advanced Electronic Signature"
          type: Boolean,
          default: false,
        },
      },
    ...
    previousVersionId: {
      // Link to prior version for audit trail
      type: Schema.Types.ObjectId,
      ref: 'LegalDocument',
    },
  ...
  },
  {
    // SCHEMA OPTIONS
    timestamps: {
      // Mongoose built-in createdAt, updatedAt
 ...
 */
LegalDocumentSchema.post('save', (doc, next) => {
...
LegalDocumentSchema.methods.grantAccess = function (userId, accessLevel = 'VIEW') {
  ...
  if (!alreadyGranted) {
    ...
    this.auditTrail.push({
      ...
      changesSnapshot: { grantedTo: userId, accessLevel },
    });
  }
  return this.save();
};
