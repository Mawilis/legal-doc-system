...
const saLegalValidationSchema = Joi.object({
  ...
  // ECT Act Compliance
  ectSignatureType: Joi.string()
    .valid('ADVANCED_ELECTRONIC', 'QUALIFIED_ELECTRONIC', 'DIGITAL_SIGNATURE')
    .required(),
  ...
  cipcRegistrationNumber: Joi.string()
    ...
    breaches: [
      {
        ...
        breachType: {
          type: String,
          enum: ['CONTRACTUAL', 'STATUTORY', 'REGULATORY', 'DATA_PROTECTION', 'FINANCIAL'],
          required: true,
        },
 ...
 */
agreementSchema.post('find', (docs) => {
  docs.forEach((doc) => {
    if (doc.signatures) {
...
agreementSchema.statics.generateComplianceReport = async function (agreementId) {
  ...
  return {
    ...
    complianceStatus: {
      ...
      ect:
        agreement.saLegalCompliance.ectComplianceLevel === 'ADVANCED'
          ? 'COMPLIANT'
          : 'REVIEW_REQUIRED',
      companiesAct: agreement.saLegalCompliance.companiesActCompliant
        ? 'COMPLIANT'
        : 'NON_COMPLIANT',
      fica:
        agreement.saLegalCompliance.ficaVerificationStatus === 'VERIFIED' ? 'COMPLIANT' : 'PENDING',
    },
    riskAssessment: {
