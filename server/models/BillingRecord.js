...
class BillingRecord extends Model {
  ...
  generateSARSVATData() {
    return {
      ...
      vatPeriod: `${new Date(this.issueDate).getFullYear()}-${(
        new Date(this.issueDate).getMonth() + 1
      )
        .toString()
        .padStart(2, '0')}`,
...
BillingRecord.init(
  {
    ...
    vatRate: {
      ...
      allowNull: false,
      defaultValue: process.env.TAX_RATE || 15.0,
      validate: {
        isDecimal: true,
        min: 0,
        max: 100,
...
BillingRecord.prototype.toSARSFormat = function () {
  return {
    taxPeriod: `${new Date(this.issueDate).getFullYear()}-${(
      new Date(this.issueDate).getMonth() + 1
    )
      .toString()
      .padStart(2, '0')}`,
