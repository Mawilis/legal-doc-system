  ...
  committees: [
    {
      name: {
        type: String,
        enum: ['audit', 'remuneration', 'nominations', 'risk', 'social_ethics', 'investment'],
      },
  ...
  documents: [
    {
      type: {
        type: String,
        enum: ['id', 'passport', 'proof_of_address', 'cv', 'consent', 'other'],
      },
      documentId: { type: mongoose.Schema.Types.ObjectId, ref: 'Document' },
      uploadedAt: Date,
      verified: { type: Boolean, default: false },
...
const companySchema = new mongoose.Schema(
  {
    ...
    retention: {
      policy: {
        type: String,
        enum: ['companies_act_10_years', 'tax_act_5_years', 'popia_1_year', 'forensic_permanent'],
    ...
    forensic: {
      ...
      blockchainAnchor: {
        ...
        blockNumber: Number,
        network: { type: String, enum: ['ethereum', 'hyperledger', 'none', 'hyperledger-fabric-2.5'], default: 'none' },
        timestamp: Date,
...
companySchema.methods.updateValuationSummary = async function (valuation) {
  ...
  this.valuations = {
    ...
    averageValue: current.averageValue
      ? (current.averageValue * (newCount - 1) + newValue) / newCount
      : newValue,
...
companySchema.virtual('isJSEListed').get(function () {
  return [
    COMPANY_SIZE.JSE_TOP_40,
    COMPANY_SIZE.JSE_MID_CAP,
    COMPANY_SIZE.JSE_SMALL_CAP,
    COMPANY_SIZE.ALTX,
  ].includes(this.size);
});
/**
