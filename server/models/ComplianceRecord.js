...
const encryptComplianceDocument = async (documentUrl, firmId, documentType) => {
  try {
    ...
    const cipher = crypto.createCipheriv('aes-256-gcm', Buffer.from(documentKey, 'hex'), iv);
...
const complianceRecordSchema = new Schema(
  {
    ...
    complianceType: {
      ...
      enum: [
        ...
        'CYBERCRIMES_ACT',
        'PEPUDA',
        'LEGAL_PRACTICE_ACT_28',
        'TRUST_PROPERTY_CONTROL_ACT',
        ...
        'NIGERIA_NDPR_2019',
        'GHANA_DATA_PROTECTION_ACT_2012',
        'TANZANIA_CYBERSECURITY_ACT',
        'UGANDA_DATA_PROTECTION_PRIVACY_ACT_2019',
        'RWANDA_DATA_PROTECTION_LAW',
        'BOTSWANA_DATA_PROTECTION_ACT',
        'NAMIBIA_DATA_PROTECTION_ACT',
        'ZAMBIA_DATA_PROTECTION_ACT',
        'ZIMBABWE_CYBERSECURITY_ACT',
        'MAURITIUS_DATA_PROTECTION_ACT_2017',
        'SENEGAL_DATA_PROTECTION_LAW',
        'ETHIOPIA_COMPUTER_CRIME_PROCLAMATION',
        'ANGOLA_DATA_PROTECTION_LAW',
      ...
      validate: {
        async validator(v) {
          ...
          const jurisdiction = await mongoose
            .model('Firm')
            .findById(this.firmId)
            .select('jurisdiction');
    ...
    status: {
      ...
      validate: {
        validator(v) {
          ...
          return (
            !this._originalStatus || validTransitions[this._originalStatus]?.includes(v) || false
          );
        },
...
complianceRecordSchema.pre('save', async function (next) {
  try {
    ...
    if (this.isModified('complianceDocuments')) {
      for (const doc of this.complianceDocuments) {
...
complianceRecordSchema.post('find', (docs) => {
  if (!Array.isArray(docs)) return;

  docs.forEach(async (doc) => {
    if (doc._decryptionAuthorized && doc.complianceDocuments) {
      for (const document of doc.complianceDocuments) {
...
complianceRecordSchema.methods.escalateComplianceIssue = async function (reason, urgency = 'HIGH') {
  ...
  const recipients = firm.complianceOfficers.filter((co) => co.role === escalationRole);
...
complianceRecordSchema.methods.autoEscalateComplianceIssue = async function () {
  if (this.riskAssessment.inherentRisk === 'CRITICAL') {
    await this.escalateComplianceIssue('Automatic escalation: CRITICAL risk detected', 'CRITICAL');
...
complianceRecordSchema.methods.anchorToBlockchain = async function (actionType) {
  try {
    ...
    if (process.env.NODE_ENV === 'production' && process.env.HYPERLEDGER_NETWORK) {
      blockchainResult = await anchorToBlockchain('compliance-records', blockchainPayload);
    } else {
...
complianceRecordSchema.statics.generateComplianceReport = async function (options = {}) {
  ...
  records.forEach((record) => {
    ...
    report.summary.byRiskLevel[riskLevel] = (report.summary.byRiskLevel[riskLevel] || 0) + 1;
  ...
  // Calculate averages
  report.summary.averageScore = records.length > 0 ? Math.round(totalScore / records.length) : 0;
...
complianceRecordSchema.virtual('regulatoryStatus').get(function () {
  return {
    compliant: this.status === 'APPROVED' && (this.complianceMetrics?.overallScore || 0) >= 70,
