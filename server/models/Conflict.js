...
const ConflictSchema = new Schema(
  {
    ...
    conflictReference: {
      ...
      index: true,
      default() {
        const tenantCode = this.tenantId.replace('tenant_', '').substring(0, 8).toUpperCase();
    ...
    parties: {
      primaryParty: {
        ...
        role: {
          type: String,
          enum: ['client', 'adverse_party', 'witness', 'expert', 'related_party'],
          required: true,
        },
        relationship: String,
      ...
      affectedIndividuals: [
        {
          ...
          role: {
            type: String,
            enum: ['lawyer', 'paralegal', 'support_staff', 'consultant', 'director'],
            required: true,
          },
          involvementLevel: {
            type: String,
            enum: ['direct', 'supervisory', 'administrative', 'peripheral'],
            required: true,
          },
    ...
    matter: {
      ...
      matterType: {
        type: String,
        enum: ['litigation', 'transaction', 'advisory', 'regulatory', 'compliance'],
      },
      jurisdiction: String,
    ...
    detection: {
      detectedBy: {
        type: String,
        enum: ['automated', 'manual', 'client_disclosure', 'third_party'],
        required: true,
      },
    ...
    metadata: {
      ...
      retentionPolicy: {
        rule: {
          type: String,
          enum: ['companies_act_7yr', 'lpc_6yr', 'permanent', 'matter_duration'],
          default: 'lpc_6yr',
        },
...
ConflictSchema.virtual('daysSinceDetection').get(function () {
  ...
  const diff = new Date() - this.detection.detectionDate;
...
ConflictSchema.statics.screenForConflicts = function (tenantId, screeningData, callback) {
  const { partyNames } = screeningData;
  const { involvedUsers } = screeningData;
  const queries = [];
  const self = this;
        ...
        for (let i = 0; i < results.length; i++) {
          if (results[i]) {
            for (let j = 0; j < results[i].length; j++) {
              const conflict = results[i][j];
              const conflictId = conflict._id.toString();
    ...
    [
      { $match: { tenantId } },
      {
        $group: {
          ...
          active: {
            $sum: {
              $cond: [{ $in: ['$status', ['pending', 'active', 'escalated']] }, 1, 0],
            },
          },
          resolved: {
            $sum: {
      ...
      },
      {
        $project: {
          ...
          resolved: 1,
          resolutionRate: {
            $cond: [{ $eq: ['$total', 0] }, 0, { $divide: ['$resolved', '$total'] }],
          },
          severityBreakdown: {
            $arrayToObject: {
              $map: {
...
ConflictSchema.methods.escalate = function (newSeverity, reason, escalatedBy, callback) {
  if (['medium', 'high', 'critical'].indexOf(newSeverity) === -1) {
    const error = new Error('Invalid severity level for escalation');
    if (callback) {
      return callback(error);
    }
    throw error;
