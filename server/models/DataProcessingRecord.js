...
const DataProcessingRecordSchema = new mongoose.Schema(
  {
    ...
    processingPurpose: {
      ...
      description:
        'Primary lawful basis for processing (POPIA Condition 1 - Purpose Specification)',
    },
    ...
    processingDetails: {
      processingMethod: {
        type: String,
        required: true,
        enum: ['AUTOMATED', 'MANUAL', 'HYBRID', 'AI_ML_PROCESSING', 'PROFILING_DECISION'],
    ...
    securityMeasures: {
      ...
      securityCertifications: [
        {
          type: String,
          enum: ['ISO_27001', 'SOC_2', 'POPIA_COMPLIANT', 'GDPR_COMPLIANT', 'PCIDSS', 'HIPAA'],
        },
      ],
    ...
    thirdPartyDisclosures: [
      {
        ...
        country: {
          ...
          validate: {
            ...
            message:
              'Cannot transfer data to high-risk jurisdictions without additional safeguards',
          },
        },
        safeguards: [
          {
...
DataProcessingRecordSchema.pre('save', async function (next) {
  ...
  if (
    ...
    if (this.retentionPolicy?.retentionPeriod > 3650) {
      // 10 years
    ...
    if (this.dataCategories?.specialCategoryData) {
      ...
      const hasValidBasis = this.lawfulBases.some((basis) => allowedBases.includes(basis.basis));
...
DataProcessingRecordSchema.statics.findOverdueForErasure = async function () {
  ...
  return allRecords.filter((record) => record.retentionComplianceStatus === 'OVERDUE_FOR_ERASURE');
};
