...
class DataSubjectEncryptionVault {
  static encryptSensitiveData(plaintext, subjectId) {
    ...
    const key = crypto.scryptSync(process.env.ENCRYPTION_KEY + subjectId, 'datasubject_salt', 32);
      ...
      matterInvolvements: [
        {
          ...
          role: {
            type: String,
            enum: ['CLIENT', 'WITNESS', 'EXPERT', 'OPPOSING_PARTY', 'THIRD_PARTY', 'BENEFICIARY'],
            required: true,
          },
          involvementDate: {
            type: Date,
...
const dataSubjectValidationSchema = Joi.object({
  ...
  identityInformation: Joi.object({
    ...
    identityType: Joi.string()
      .valid('SA_ID', 'PASSPORT', 'DRIVERS_LICENSE', 'ASYLUM_SEEKER', 'REFUGEE')
      .required(),
  ...
  languageSupport: Joi.object({
    preferredLanguage: Joi.string()
      .valid(...Object.keys(ConstitutionalLanguageConstants.OFFICIAL_LANGUAGES))
      .required(),
  ...
  demographicInformation: Joi.object({
    ...
    specialCategoryFlags: Joi.object({
      ...
      specialInformationTypes: Joi.array()
        .items(
          Joi.string().valid(
            'RELIGIOUS_BELIEFS',
            'PHILOSOPHICAL_BELIEFS',
    ...
    const dataSubject = new this({
      ...
      contactInformation: {
        ...
        contactHashes: {
          emailHash: crypto
            .createHash('sha256')
            .update(value.contactInformation.email)
            .digest('hex'),
          mobileHash: crypto
            .createHash('sha256')
            .update(value.contactInformation.mobileNumber)
            .digest('hex'),
        },
        verifiedChannels: {
          email: false,
      ...
      languageSupport: {
        ...
        languageProficiency,
        multilingualCapabilities:
          Object.keys(value.languageSupport.languageProficiency || {}).length > 1,
      ...
      auditTrail: {
        ...
        informationOfficer: process.env.DEFAULT_INFORMATION_OFFICER_ID
          ? new mongoose.Types.ObjectId(process.env.DEFAULT_INFORMATION_OFFICER_ID)
          : null,
        accessCount: 0,
      },
      quantumIntegrity: {
 ...
 */
dataSubjectSchema.statics.updateVerificationStatus = async function (
  subjectId,
  verificationStatus,
  verificationData = {},
  session = null
...
dataSubjectSchema.methods.exerciseRight = async function (
  ...
  try {
    ...
    const rightsExercise = {
      ...
      popiaReference:
        ConstitutionalLanguageConstants.POPIA_SECTION_REFERENCES[rightType] || 'Section 23',
    };
    ...
    notices[languageCode] = {
      ...
      regulatoryBodies: {
        southAfrica: {
          ...
          website: 'https://www.justice.gov.za/inforeg/',
          complaintsEmail:
            process.env.INFORMATION_REGULATOR_EMAIL || 'complaints.IR@justice.gov.za',
...
dataSubjectSchema.methods.updateContactInformation = async function (contactData, session = null) {
  try {
    ...
    if (contactData.email) {
      this.contactInformation.contactHashes.emailHash = crypto
        .createHash('sha256')
        .update(contactData.email)
        .digest('hex');
      this.contactInformation.verifiedChannels.email = false;
    }

    if (contactData.mobileNumber) {
      this.contactInformation.contactHashes.mobileHash = crypto
        .createHash('sha256')
        .update(contactData.mobileNumber)
        .digest('hex');
...
dataSubjectSchema.pre('save', function (next) {
  ...
  if (
    ...
    this.quantumIntegrity.dataHash = crypto
      .createHash('sha512')
...
dataSubjectSchema.post('save', async (doc) => {
  try {
