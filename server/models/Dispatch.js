    ...
    content: {
      ...
      documentType: {
        ...
        enum: [
          ...
          'SADC_TREATY_DOCUMENT',
          'ECOWAS_FILING',
          'EAST_AFRICAN_PLEADING',
...
dispatchSchema.post('save', async (doc) => {
...
dispatchSchema.methods.dispatchAfricanSMS = async function (channel) {
  ...
  channel.cost = result.cost || 1.5; // ZAR per SMS
...
dispatchSchema.virtual('africanResilienceScore').get(function () {
  ...
  const factors = {
    ...
    offlineCapable: this.channels.some((c) => ['PHYSICAL_COURIER', 'SMS_OTP'].includes(c.type))
      ? 1
      : 0,
