...
const FirmSchema = new mongoose.Schema(
  {
    ...
    name: {
      ...
      required: [
        true,
        'Legal practice name is required for court registration under Companies Act 2008',
      ],
      trim: true,
    ...
    registrationNumber: {
      ...
      validate: {
        ...
        message:
          'Invalid legal registration number format. Must be LPC/CPT/123456/21, CPT123456, 2021/123456, or CK20123456',
      },
      index: true,
      set(v) {
    ...
    financials: {
      bankingDetails: {
        bankName: {
          ...
          required: [
            true,
            'Bank name is required for trust accounting compliance (LPC Rule 35.13)',
          ],
          trim: true,
          enum: [
            'ABSA',
      ...
      billingStructure: {
        ...
        defaultHourlyRate: {
          type: Number,
          default: process.env.DEFAULT_HOURLY_RATE
            ? parseInt(process.env.DEFAULT_HOURLY_RATE)
            : 2500,
          min: 500,
          max: 50000,
        },
        ...
        pepMatches: [
          {
            ...
            status: {
              type: String,
              enum: ['CONFIRMED', 'FALSE_POSITIVE', 'INVESTIGATING', 'ESCALATED'],
            },
        ...
        overdueItems: [
          {
            $project: {
              popiaOverdue: {
                $lt: ['$popiaCompliance.dataProcessingRegister.nextReview', new Date()],
              },
 ...
 */
FirmSchema.methods.addComplianceEvent = function (
  eventType,
  description,
  metadata = {},
  userInfo = {},
...
FirmSchema.methods.generateComplianceCertificate = function () {
  ...
  const certificateData = {
    ...
    verification: {
      ...
      verificationUrl: `${
        process.env.BASE_URL || 'https://wilsy.os'
...
FirmSchema.methods.generateCertificateSignature = function (certificateId, timestamp) {
  const dataToSign = `${certificateId}:${this.registrationNumber}:${
    this.name
  }:${timestamp.getTime()}:${process.env.CERTIFICATE_SIGNING_SECRET || 'wilsy-secret-2026'}`;
...
FirmSchema.virtual('monthlySubscriptionFee').get(function () {
  ...
  const baseFee = baseFees[this.accreditationLevel] || 1500;
