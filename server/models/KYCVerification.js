...
const KYCVerificationSchema = new mongoose.Schema(
  {
    ...
    ongoingMonitoring: {
      ...
      monitoringFrequency: {
        type: String,
        enum: ['MONTHLY', 'QUARTERLY', 'BIANNUALLY', 'ANNUALLY', 'AD_HOC', 'CONTINUOUS'],
        default() {
...
KYCVerificationSchema.pre('save', async function (next) {
  try {
    ...
    if (this.isNew && !this.verificationId) {
      ...
      const lastVerification = await this.constructor
        .findOne(
    ...
    if (this.isModified() && !this.isNew) {
      ...
      this.evidencePackage.packageHash = crypto
        .createHash('sha256')
 ...
 */
KYCVerificationSchema.post('save', async (doc, next) => {
  try {
    ...
    if (doc.clientId) {
      ...
      await Client.updateOne(
        { _id: doc.clientId },
        {
          $set: {
            'ficaDetails.status':
              doc.verificationStatus === 'APPROVED' ? 'VERIFIED' : 'IN_PROGRESS',
...
KYCVerificationSchema.methods._calculateRiskScore = function () {
  ...
  if (
    this.thirdPartyVerification?.lexisnexisVerification?.worldComplianceCheck?.matches?.length === 0
  ) {
    score -= 5;
  }
