...
magisterialDistrictSchema.statics.findDistrictByLocation = async function (
  ...
  if (activeOnly) {
    ...
    query.$or = [{ effectiveTo: { $exists: false } }, { effectiveTo: { $gte: new Date() } }];
  }
...
magisterialDistrictSchema.statics.syncWithDOJCD = async function (districtCode = null) {
  try {
    ...
    const url = districtCode ? `${DOJCD_API_URL}/${districtCode}` : `${DOJCD_API_URL}/all`;
    ...
    const districts = Array.isArray(response.data) ? response.data : [response.data];
    ...
    for (const districtData of districts) {
      ...
      const result = await this.findOneAndUpdate({ dojcdDistrictCode: districtData.code }, update, {
        upsert: true,
        new: true,
        runValidators: true,
      });
        ...
        pipeline: [
          {
            $match: {
              $expr: {
                $and: [{ $ne: ['$_id', '$$districtId'] }, { $eq: ['$status', 'ACTIVE'] }],
              },
            },
          },
        ],
...
magisterialDistrictSchema.methods.getOptimalSheriffOffice = function (suburbName, options = {}) {
  const { requirePrimary = false, maxDistanceKm = 50 } = options;
...
// =============================================================================
module.exports =
  mongoose.models.MagisterialDistrict ||
