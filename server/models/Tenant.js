...
const tenantSchema = new mongoose.Schema(
  {
    ...
    integrityHash: {
      ...
      default: function () {
        ...
        return crypto.createHash('sha3-512').update(JSON.stringify(hashData)).digest('hex');
      },
    },
...
tenantSchema.virtual('isPOPIACompliant').get(function () {
  return (
    this.popiaComplianceScore >= 80 && this.popiaInformationOfficer.verifiedBySuperAdmin === true
  );
});
...
tenantSchema.methods.encryptData = function (data) {
  ...
  const key = crypto.scryptSync(process.env.ENCRYPTION_KEY_SALT, 'tenant-encryption-salt', 32);
