...
transactionSchema.pre('save', async function (next) {
  try {
    ...
    if (this.isNew && !this.runningBalance) {
      const previousTransaction = await this.constructor
        .findOne({
...
transactionSchema.statics.getComplianceStats = async function (tenantId, firmId = null, days = 30) {
  ...
  const [
    ...
    this.countDocuments({
      ...
      'amlMonitoring.flagSeverity': { $in: ['HIGH', 'CRITICAL'] },
    }),
...
transactionSchema.statics.findByAccount = async function (accountNumber, tenantId, options = {}) {
  ...
  const summary = {
    ...
    creditTotal: transactions
      .filter((tx) => ['CREDIT', 'DEPOSIT', 'RECEIPT', 'TRUST_TRANSFER'].includes(tx.type))
      .reduce((sum, tx) => sum + tx.zarEquivalent, 0),
    debitTotal: transactions
      .filter((tx) => ['DEBIT', 'WITHDRAWAL', 'PAYMENT', 'FEE'].includes(tx.type))
      .reduce((sum, tx) => sum + tx.zarEquivalent, 0),
    averageValue:
      transactions.length > 0
        ? transactions.reduce((sum, tx) => sum + tx.zarEquivalent, 0) / transactions.length
        : 0,
  };
...
transactionSchema.statics.getUnreconciledTransactions = async function (
  ...
  const transactions = await this.find(query).sort({ processedAt: -1 }).lean().exec();
...
transactionSchema.statics.bulkReconcile = async function (
  ...
  return {
    ...
    summary: {
      ...
      successRate:
        transactionIds.length > 0 ? (result.modifiedCount / transactionIds.length) * 100 : 0,
    },
    ...
    volume: {
      ...
      average:
        dailyVolume.length > 0
          ? dailyVolume.reduce((sum, d) => sum + d.volume, 0) / dailyVolume.length
          : 0,
    },
    breakdowns: {
...
transactionSchema.index({
  'amlMonitoring.flagged': 1,
  'amlMonitoring.flagSeverity': 1,
  processedAt: -1,
});
