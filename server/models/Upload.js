...
const uploadSchema = new mongoose.Schema(
  {
    ...
    checksum: {
      ...
      required: [
        true,
        'SHA-256 checksum is mandatory for legal integrity under ECT Act Section 15.',
      ],
      index: true,
      validate: {
        validator: function (v) {
...
uploadSchema.post('save', async function (doc) {
  try {
    ...
    if (doc.mimeType.startsWith('application/pdf') || doc.mimeType.startsWith('image/')) {
...
uploadSchema.statics.calculateStorageUsage = async function (tenantId) {
  ...
  return {
    ...
    quota: {
      ...
      usedPercentage:
        (result[0].totalSize / (process.env.TENANT_STORAGE_LIMIT || 107374182400)) * 100,
    },
  };
};
