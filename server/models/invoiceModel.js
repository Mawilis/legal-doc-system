...
// Native Node.js crypto for financial-grade encryption
...
const FINANCIAL_CONFIG = Object.freeze({
  ...
  PRESCRIPTION: {
    ...
    DEBT_COLLECTION_RULES: {
      MAX_INTEREST_RATE: 0.2, // 20% per annum maximum
      COLLECTION_COST_LIMIT: 0.1, // 10% of debt maximum
    },
  },
...
const LineItemSchema = new Schema(
  {
    ...
    vatRateApplied: {
      ...
      default() {
        switch (this.vatCategory) {
          case 'STANDARD':
            return FINANCIAL_CONFIG.VAT.STANDARD_RATE;
          case 'ZERO_RATED':
            return 0;
          case 'EXEMPT':
          case 'OUT_OF_SCOPE':
    ...
    integrityHash: {
      ...
      default() {
        ...
        return crypto.createHash('sha3-512').update(JSON.stringify(criticalData)).digest('hex');
      },
      immutable: true,
    },
...
const InvoiceSchema = new Schema(
  {
    ...
    paymentTerms: {
      type: String,
      default:
        'Payment due within 30 days of invoice date. Late payments subject to interest at the prescribed rate in terms of the National Credit Act.',
    ...
    latePaymentInterestRate: {
      ...
      max: [0.2, 'Interest rate cannot exceed 20% per month'],
    },
