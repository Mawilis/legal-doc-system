...
const jobSchema = new mongoose.Schema(
  {
    ...
    _id: {
      ...
      immutable: true,
      match: [
        /^job_[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}_[0-9a-f]{16}$/i,
...
jobSchema.pre('save', function (next) {
  ...
  if (this.payload && JSON.stringify(this.payload).length > 10485760) {
    // 10MB
...
jobSchema.virtual('retryInfo').get(function () {
  ...
  const exponentialDelay = baseDelay * 2 ** this.attempts;
...
jobSchema.methods.updateProgress = async function (progress, result = {}, userId) {
  ...
  await this.addAuditEntry('PROGRESS_UPDATED', `Progress updated to ${progress}%`, userId, {
...
jobSchema.methods.markCompleted = async function (result = {}, userId) {
  ...
  await this.addAuditEntry('COMPLETED', 'Task completed successfully', userId, {
...
jobSchema.methods.markFailed = async function (error, userId) {
  ...
  await this.addAuditEntry('FAILED', `Task failed: ${errorObj.message}`, userId, {
        ...
        slaCompliance: [
          ...
          },
          {
            $group: {
              ...
              withinSla: {
                $sum: {
                  $cond: [{ $eq: ['$slaCompliance.withinSla', true] }, 1, 0],
                },
              },
              slaRate: {
                $avg: {
