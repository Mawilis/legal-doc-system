...
const REQUIRED_ENV_VARS = ['MONGO_URI', 'ENCRYPTION_KEY', 'REGULATION_ENCRYPTION_KEY'];
      ...
      if (chapter.sections) {
        chapter.sections.forEach((section) => {
          ...
          if (section.content && !section.containsPII) {
            ...
            section.containsPII = piiKeywords.some((keyword) => contentLower.includes(keyword));
          }
        });
      }
    });
 ...
 */
RegulationSchema.post('save', (doc) => {
...
RegulationSchema.statics.findActiveByJurisdiction = function (jurisdiction, framework) {
  ...
  return this.find({
    ...
    $or: [{ sunsetDate: { $exists: false } }, { sunsetDate: { $gt: now } }],
    isDeleted: false,
