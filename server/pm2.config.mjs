/**
 * ╔═══════════════════════════════════════════════════════════════════════════╗
 * ║ WILSY OS - PM2 PRODUCTION CONFIG (ES MODULE)                             ║
  ...
  apps: [{
    ...
    deploy: {
      production: {
        ...
        'post-deploy': 'npm ci --production && pm2 reload pm2.config.mjs --only wilsy-os && pm2 save',
      ...
      staging: {
        ...
        ssh_options: 'StrictHostKeyChecking=no',
        'post-deploy': 'npm ci && pm2 reload pm2.config.mjs --only wilsy-os --env staging && pm2 save',
        env: {
          NODE_ENV: 'staging',
        },
      },
