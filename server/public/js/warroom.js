#!(function () {
  ...
  function updateDashboard(data) {
    ...
    if (elements.activeQuarantines) {
      elements.activeQuarantines.textContent =
        (data.metrics && data.metrics.activeQuarantines) || 0;
    }
  ...
  function updateThreatsTable(threats) {
    ...
    if (!threats || threats.length === 0) {
      elements.threatsBody.innerHTML =
        '<tr><td colspan="4" class="loading-message">No threats detected</td></tr>';
      return;
    }
    threats.slice(0, 10).forEach(function (threat) {
      ...
      row.insertCell(3).textContent = threat.lastSeen
