 ...
 * POST /api/admin/keys
 */
router.post('/keys', superAdminGuard, auditMiddleware({ action: 'API_KEY_GENERATE' }), async (req, res) => {
 ...
 */
router.patch('/keys/:keyId', superAdminGuard, auditMiddleware({ action: 'API_KEY_UPDATE' }), async (req, res) => {
  const correlationId = req.headers['x-correlation-id'] || `admin-${Date.now()}`;
  
  try {
    const updates = {};
    const allowedUpdates = ['tier', 'isActive', 'name', 'description', 'expiresAt', 'rateLimits', 'allowedIPs', 'allowedDomains', 'webhookUrl'];
 ...
 */
router.delete('/keys/:keyId', superAdminGuard, auditMiddleware({ action: 'API_KEY_REVOKE' }), async (req, res) => {
  const correlationId = req.headers['x-correlation-id'] || `admin-${Date.now()}`;
