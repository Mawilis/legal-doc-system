...
router.get(
  ...
  validateAdminRequest([
    ...
    query('status')
      .optional()
      .isIn(['active', 'suspended', 'pending', 'trial'])
      .withMessage('Invalid status filter'),
    query('plan')
      .optional()
      .isIn(['basic', 'pro', 'enterprise', 'sovereign'])
...
router.post(
  ...
  validateAdminRequest([
    ...
    body('reason')
      .trim()
      .notEmpty()
      .withMessage('Mandatory suspension reason required for audit trail'),
    body('duration')
      .optional()
      .isIn(['24h', '7d', '30d', 'indefinite'])
      .withMessage('Invalid suspension duration'),
...
router.delete(
  ...
  validateAdminRequest([
    param('userId').isMongoId().withMessage('User ID must be a valid MongoID'),
    body('confirmation')
      .equals('CONFIRM_ATOMIC_DELETION')
      .withMessage('Nuclear confirmation code required'),
    body('reason')
      .trim()
      .notEmpty()
      .withMessage('Mandatory deletion reason required for 25-year audit trail'),
  ]),
  async (req, res, next) => {
...
router.post(
  ...
  validateAdminRequest([
    body('confirmation')
      .equals('CONFIRM_SYSTEM_RECOVERY')
...
router.post(
  ...
  validateAdminRequest([
    body('optimizationType')
      .isIn(['database', 'cache', 'network', 'compute'])
