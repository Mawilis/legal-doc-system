...
const specialCategoryProcessingSchema = Joi.object({
  ...
  lawfulCondition: Joi.string()
    .valid('EXPLICIT_CONSENT', 'EMPLOYMENT_LAW', 'VITAL_INTERESTS', 'PUBLIC_HEALTH', 'LEGAL_CLAIMS')
    .required(),
  ...
  async (req, res, next) => {
    try {
      ...
      const taxRecord = createLegalOperation(
        ...
        {
          taxNumber: req.body.taxNumber
            ? crypto.createHash('sha256').update(req.body.taxNumber).digest('hex')
            : null,
          vatRegistered: req.body.vatRegistered,
      ...
      await emitAudit(req, {
        ...
        severity: 'INFO',
        summary: `SARS tax compliance validation for tax number ${
          req.body.taxNumber ? 'provided' : 'not provided'
        }`,
        metadata: {
  ...
  async (req, res, next) => {
    try {
      ...
      const encryptedPII = {
        ...
        nameHash: crypto
          .createHash('sha256')
          .update(req.body.firstName + req.body.lastName)
          .digest('hex'),
 ...
 */
router.get('/health', protect, authorize('admin', 'system_admin'), async (req, res) => {
  try {
  ...
  async (req, res, next) => {
    try {
      ...
      const lpcRecord = createLegalOperation(
        ...
        {
          practiceNumber: req.body.practiceNumber,
          trustAccountNumber: crypto
            .createHash('sha256')
            .update(req.body.trustAccountNumber)
            .digest('hex'),
