...
const dsarLimiter = rateLimit({
  ...
  message: {
    ...
    message:
      'Maximum DSAR requests reached for today. Please try again tomorrow or contact Data Protection Officer.',
...
router.get(
  ...
  [
    ...
    param('id')
      .isUUID(4)
      .withMessage('Invalid UUID v4 format')
      .trim()
      .escape()
      .custom((value) => {
...
router.get(
  ...
  [
    ...
    param('dataSubjectId')
      .notEmpty()
      .withMessage('Data subject ID is required')
      .isLength({ min: 32 })
      .withMessage('Data subject ID must be at least 32 characters (encrypted)')
      .trim()
      .escape()
      .custom((value) => {
...
router.post(
  ...
  [
    ...
    body('encryptionLevel')
      .isIn(['STANDARD', 'ENHANCED', 'REGULATOR'])
    ...
    try {
      ...
      res.json({
        ...
        complianceMarkers: {
          ...
          timestamp: new Date().toISOString(),
          regulatoryReference: `IREG-EXPORT-${new Date().getFullYear()}-${crypto
            .randomBytes(4)
            .toString('hex')}`,
        },
      });
