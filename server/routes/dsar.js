 ...
 * @multitenant Tenant isolation enforced
 */
router.get('/:id', [param('id').isMongoId().withMessage('Invalid DSAR ID')], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
                            ...
                            [
                              {
                                k: '$$type',
                                v: {
                                  $add: [{ $getField: { field: '$$type', input: '$$value' } }, 1],
                                },
                              },
                            ],
                          ],
                            ...
                            [
                              {
                                k: '$$status',
                                v: {
                                  $add: [{ $getField: { field: '$$status', input: '$$value' } }, 1],
                                },
                              },
                            ],
                          ],
                ...
                $cond: [
                  ...
                  100,
                  {
                    $multiply: [
                      {
                        $divide: [
                          { $subtract: ['$fulfilledCount', '$breachedCount'] },
                          '$fulfilledCount',
                        ],
                      },
                      100,
