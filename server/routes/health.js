...
router.get('/ready', async (req, res) => {
  ...
  try {
    ...
    const dependencies = {
      database:
        db.status === 'fulfilled' ? db.value : { status: 'unknown', error: db.reason?.message },
      redis:
        redisHealth.status === 'fulfilled'
          ? redisHealth.value
          : { status: 'unknown', error: redisHealth.reason?.message },
      queues:
        queuesHealth.status === 'fulfilled'
          ? queuesHealth.value
          : { status: 'unknown', error: queuesHealth.reason?.message },
    };
    ...
    const dependencies = {
      ...
      queues:
        queuesHealth.status === 'fulfilled'
          ...
          : { status: 'unknown', error: queuesHealth.reason?.message },
      security:
        securityHealth.status === 'fulfilled'
          ? securityHealth.value
          : { status: 'unknown', error: securityHealth.reason?.message },
      audit:
        auditHealth.status === 'fulfilled'
          ? auditHealth.value
          : { status: 'unknown', error: auditHealth.reason?.message },
      classification:
        classificationHealth.status === 'fulfilled'
          ? classificationHealth.value
          : { status: 'unknown', error: classificationHealth.reason?.message },
    };
