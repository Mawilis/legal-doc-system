...
const consentSchema = {
  processingActivityId: Joi.string()
    .uuid()
    .required()
  ...
  withdrawalMechanism: Joi.string()
    .valid('EMAIL', 'SMS', 'DASHBOARD', 'API')
    .required()
  ...
  thirdPartyDisclosures: Joi.array()
    .items(
      Joi.object({
  ...
  automatedDecisionMaking: Joi.boolean()
    .default(false)
...
const dsarSchema = {
  requestType: Joi.string()
    .valid('ACCESS', 'CORRECTION', 'DELETION', 'RESTRICTION', 'OBJECTION', 'DATA_PORTABILITY')
    .required()
...
const processingRecordSchema = {
  ...
  dataSubjectCategories: Joi.array()
    .items(Joi.string())
    .min(1)
    .required()
...
const automatedDecisionSchema = {
  decisionType: Joi.string()
    .valid('CREDIT_SCORING', 'RISK_ASSESSMENT', 'PROFILING', 'ELIGIBILITY', 'PRICING')
    .required()
    .description('Type of automated decision making'),
  logicDescription: Joi.string()
    .min(20)
    .max(2000)
    .required()
...
const idSchema = {
  id: Joi.string().uuid().required().description('UUID identifier with cryptographic validation'),
};
 ...
 */
router.get('/dsar/:id/status', protect, validate(idSchema, 'params'), async (req, res, next) => {
  try {
      ...
      if (createPrivacyAuditTrail) {
        await createPrivacyAuditTrail(req, {
          ...
          metadata: {
            ...
            criminalActivitySuspected:
              req.body.incidentType === 'RANSOMWARE' || req.body.incidentType === 'INSIDER_THREAT',
          },
        ...
        res.status(201).json({
          ...
          compliance: {
            ...
            dataSubjectNotification:
              req.body.potentialHarm === 'SEVERE' ? 'REQUIRED' : 'ASSESSMENT_PENDING',
...
function generateWithdrawalInstructions(consentId, mechanism) {
  ...
  const mechanisms = {
    ...
    SMS: {
      instruction: `SMS "WITHDRAW ${consentId}" to ${
        process.env.SMS_WITHDRAWAL_NUMBER || '0712345678'
      }`,
