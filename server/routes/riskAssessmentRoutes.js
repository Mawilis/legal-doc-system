...
let riskAssessmentController;
let riskAssessmentValidator;
...
const RISK_LEVELS = Object.freeze({
  ...
  HIGH: {
    ...
    score: 3,
    color: '#F44336',
    action: 'ENHANCED_DUE_DILIGENCE_WITH_MONITORING',
  },
  CRITICAL: {
    code: 'CRITICAL',
    score: 4,
    color: '#9C27B0',
    action: 'REFUSE_OR_TERMINATE_RELATIONSHIP',
  },
});
...
const riskAssessmentSchemas = {
    ...
    categories: Joi.array()
      .items(
        Joi.object({
          category: Joi.string()
            .required()
            .valid(...Object.keys(RISK_CATEGORIES)),
  ...
  reportParams: Joi.object({
    ...
    reportType: Joi.string()
      .valid('DETAILED', 'SUMMARY', 'EXECUTIVE', 'REGULATORY')
      .default('SUMMARY'),
...
async function addDigitalSignature(report, userId, type) {
  const signature = crypto
    .createHmac('sha256', process.env.REPORT_SIGNATURE_KEY || 'default-key')
    .update(report)
    .digest('hex');
...
router.post('/', createAssessmentMiddleware, async (req, res, next) => {
  try {
  ...
  async (req, res, next) => {
    try {
      ...
      if (assessmentData.encryptedData && canDecryptData(req.user?.role)) {
        assessmentData = await decryptSensitiveData(assessmentData.encryptedData, req.user?.id);
      }
...
router.put(
  ...
  async (req, res, next) => {
    try {
      const canUpdate = await verifyUpdatePermissions(req.user?.id, req.params.id, req.user?.role);
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const result = await riskAssessmentController.submitForReview(req.params.id, {
        ...req.body,
      ...
      await notifyReviewOutcome(req.params.id, req.body.decision, req.body.comments, req.user?.id);
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const result = await riskAssessmentController.generateReport(req.params.id, reportOptions);
  ...
  [
    ...
    validateRequest({
      operation: Joi.string()
        .valid('UPDATE_STATUS', 'ASSIGN_REVIEWER', 'SCHEDULE_REVIEW', 'EXPORT_DATA')
        .required(),
  ...
  async (req, res, next) => {
    try {
      ...
      if (req.body.notifyOnComplete && !req.body.validateOnly) {
        await sendBulkOperationNotification(req.user?.id, req.body.operation, bulkResult.summary);
      }
