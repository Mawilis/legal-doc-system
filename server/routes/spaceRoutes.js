...
router.get(
  '/',
  spatialAccessControl(['view_spaces']),
  validateInput(spatialValidationSchema.query),
  async (req, res) => {
    try {
      const {
        page = 1,
  ...
  async (req, res) => {
    try {
      ...
      const newSpace = await SpaceService.createSpace(enhancedSpaceData, {
        ...
        generateVirtualTwin:
          spaceData.type.includes('virtual') || spaceData.type.includes('courtroom'),
    ...
    try {
      ...
      if (space.jurisdiction !== req.spatialContext.jurisdiction) {
        return res.status(403).json({
          ...
          message:
            'Cannot book space in different jurisdiction without cross-jurisdiction permissions',
      ...
      const enhancedBookingData = {
        ...
        compliance: {
          ...
          courtRulesCompliant:
            bookingData.purpose.includes('court') || bookingData.purpose.includes('hearing'),
    ...
    try {
      ...
      const enhancedUpdateData = {
        ...
        complianceUpdates: {
          ...
          buildingRegulations: updateData.capacity
            ? 'SANS 10400 capacity compliance verified'
            : null,
        },
        auditTrail: {
...
router.get(
  '/:id/virtual-session',
  spatialAccessControl(['join_virtual_sessions']),
  async (req, res) => {
    try {
      const spaceId = req.params.id;
