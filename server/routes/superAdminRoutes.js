...
const validateSuperAdminRegistration = [
  body('legalName')
    ...
    .withMessage('Legal name must be 2-100 characters')
    .matches(/^[A-Za-z\s\-']+$/)
    .withMessage('Legal name contains invalid characters'),

  body('idNumber')
    .trim()
    .notEmpty()
    .withMessage('South African ID number is required')
  ...
  body('mobileNumber')
    ...
    .matches(/^\+27[0-9]{9}$/)
    .withMessage('Mobile number must be in format: +27XXXXXXXXX'),

  body('saCitizen')
    .optional()
    .isBoolean()
    .withMessage('South African citizenship must be true or false'),
...
const validateSuperAdminLogin = [
  ...
  body('password')
    ...
    .isLength({ min: 24 })
    .withMessage('Password must be at least 24 characters'),
  ...
  body('mfaToken')
    ...
    .matches(/^\d+$/)
    .withMessage('MFA token must contain only digits'),
...
const validateTenantSuspension = [
  ...
  body('statute')
    .trim()
    .notEmpty()
    .withMessage('Legal statute is required')
    .isIn([
      'POPIA_SECTION_19',
      'FICA_SECTION_43',
  ...
  body('mfaToken')
    .trim()
    .notEmpty()
    .withMessage('MFA token is required for tenant suspension')
    .isLength({ min: 6, max: 6 })
    .withMessage('MFA token must be 6 digits'),
...
const validateEmergencyProtocol = [
  body('protocol')
    .trim()
    .notEmpty()
    .withMessage('Emergency protocol is required')
  ...
  body('notifyAuthorities')
    .optional()
    .isBoolean()
    .withMessage('Notify authorities must be true or false'),
 ...
 */
router.post('/login', supremePublicLimiter, validateSuperAdminLogin, loginSuperAdmin);
 ...
 */
router.post('/logout', divineProtectedLimiter, superAdminAuth, logoutSuperAdmin);
 ...
 * @compliance POPIA Section 19
 */
router.post('/refresh-token', divineProtectedLimiter, refreshToken);
 ...
 */
router.get('/profile', divineProtectedLimiter, superAdminAuth, getSuperAdminProfile);
 ...
 */
router.post('/update-password', divineProtectedLimiter, superAdminAuth, updatePassword);
 ...
 * @compliance ECT Act Section 18, POPIA Section 19
 */
router.post('/setup-mfa', divineProtectedLimiter, superAdminAuth, setupMFA);
 ...
 */
router.post('/verify-mfa', divineProtectedLimiter, superAdminAuth, verifyMFA);
      ...
      res.json({
        ...
        data: {
          complianceOverview: {
            ...
            popiaComplianceRate:
              metrics.totalTenants > 0 ? (metrics.popiaCompliant / metrics.totalTenants) * 100 : 0,
            ficaComplianceRate:
              metrics.totalTenants > 0 ? (metrics.ficaCompliant / metrics.totalTenants) * 100 : 0,
            averageComplianceScore: metrics.averageComplianceScore,
            highRiskPercentage:
              metrics.totalTenants > 0 ? (metrics.highRiskTenants / metrics.totalTenants) * 100 : 0,
          },
          regulatoryStatus: {
            popiaCompliant:
              metrics.popiaComplianceRate >= 95 ? 'FULLY_COMPLIANT' : 'NEEDS_IMPROVEMENT',
            ficaCompliant:
              metrics.ficaComplianceRate >= 95 ? 'FULLY_COMPLIANT' : 'NEEDS_IMPROVEMENT',
 ...
 */
router.get('/health', supremePublicLimiter, async (req, res) => {
  const healthChecks = {
    timestamp: new Date(),
