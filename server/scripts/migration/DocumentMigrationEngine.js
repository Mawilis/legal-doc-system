...
class DocumentMigrationEngine {
  ...
  determineTenantStrategy(ownershipPatterns) {
    ...
    const userBasedConfidence = topUploader
      ? (topUploader.documentCount / this.migrationStats.totalDocuments) * 100
      : 0;
  ...
  generateMigrationReport() {
    const report = {
      ...
      summary: {
        ...
        successRate:
          this.migrationStats.totalDocuments > 0
            ? (this.migrationStats.migratedDocuments / this.migrationStats.totalDocuments) * 100
            : 0,
      },
  ...
  async saveRollbackData() {
    ...
    const rollbackInfo = {
      ...
      checksum: createHash('sha256').update(JSON.stringify(this.rollbackData)).digest('hex'),
    };

    await fs.writeFile(CONFIG.ROLLBACK_FILE, JSON.stringify(rollbackInfo, null, 2), 'utf8');
