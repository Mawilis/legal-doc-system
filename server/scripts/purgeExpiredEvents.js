...
class PurgeOperationManager {
  ...
  async archiveRecords(collectionKey, records, policy) {
    ...
    const archiveId = `ARCHIVE-${collectionKey}-${Date.now()}-${crypto
      .randomBytes(4)
      .toString('hex')}`;
  ...
  async deleteExpiredRecords(policy, records) {
    ...
    try {
      ...
      purgeLogger.info(
        `Deleted ${deleteResult.deletedCount} records from ${policy.collectionName}`,
        {
  ...
  async generateComplianceReport() {
    const report = {
      ...
      legalCompliance: {
        popia:
          this.purgeSummary.complianceViolations.length === 0 ? 'COMPLIANT' : 'VIOLATIONS_DETECTED',
  ...
  async sendComplianceNotifications() {
    ...
    const notification = {
      ...
      body: {
        ...
        complianceStatus:
          this.purgeSummary.complianceViolations.length > 0 ? 'VIOLATIONS DETECTED' : 'COMPLIANT',
        violations: this.purgeSummary.complianceViolations.length,
        actionRequired:
          this.purgeSummary.complianceViolations.length > 0
            ? 'Review compliance violations report'
            : 'No action required',
      },
    };
