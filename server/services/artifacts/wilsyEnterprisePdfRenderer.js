 ...
 * @description Resolves a professional Generated By display name for Wilsy Knowledge Base artifacts.
 * @param {object} knowledgeBase Knowledge-base payload candidate.
 * @param {object} state Existing enterprise render state.
 * @returns {string} Professional display name.
 * @collaboration FG108O3E2, Knowledge Base guard, Document Control, and investor-grade artifact identity.
 */
function resolveWilsyProfessionalGeneratedBy(knowledgeBase = {}, state = {}) {
  const explicitDisplayName = textValue(
  ...
  if (explicitDisplayName && explicitDisplayName !== explicitDisplayName.toLowerCase()) {
    return explicitDisplayName;
  }

  const rawGeneratedBy = textValue(
    knowledgeBase.generatedBy ||
      knowledgeBase.operatorName ||
      knowledgeBase.operatorId ||
      knowledgeBase.operatorHandle ||
      state.generatedBy
  );

  const knownProfessionalNames = {
    wilsonkhanyezi: 'Wilson Khanyezi',
    'wilson.khanyezi': 'Wilson Khanyezi',
    'wilson-khanyezi': 'Wilson Khanyezi',
    'wilson khanyezi': 'Wilson Khanyezi',
  };

  const normalizedKey = rawGeneratedBy.toLowerCase().replace(/^@/, '').trim();

  if (knownProfessionalNames[normalizedKey]) {
    return knownProfessionalNames[normalizedKey];
  }

  if (
    /^[a-z]+[a-z.-]*[a-z]+$/.test(normalizedKey) &&
    normalizedKey.includes('wilson') &&
    normalizedKey.includes('khanyezi')
  ) {
    return 'Wilson Khanyezi';
  }

  if (rawGeneratedBy && rawGeneratedBy !== rawGeneratedBy.toLowerCase()) {
    return rawGeneratedBy;
  }

  return 'Wilsy OS Operator';
}

// WILSY_FG108O3E2_PROFESSIONAL_GENERATED_BY_NORMALIZATION
