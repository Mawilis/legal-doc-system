...
class AuditTrailService extends EventEmitter {
  ...
  async initializeDatabase() {
    try {
      ...
      const mongoUrl = this.config.mongoUrl || process.env.MONGODB_URI
   */
  ...
  async createQueueWorkers() {
    ...
    this.auditWorker = new BullMQ.Worker(
      'audit-processing',
      async (job) => {
    ...
    this.integrityWorker = new BullMQ.Worker(
      'audit-integrity',
      async (job) => {
        await this.performIntegrityCheck();
      },
  ...
  async log(auditData) {
    ...
    try {
      ...
      const auditRecord = {
        ...
        severity:
          auditData.severity || categoryConfig.severity || this.determineSeverity(auditData),
    ...
    try {
      ...
      auditRecords.forEach((record) => {
        ...
        this.merkleTree.set(leafHash, {
          ...
          blockIndex:
            this.serviceState.chainHeight - auditRecords.length + auditRecords.indexOf(record),
        });
      });
  ...
  generateIntegrityRecommendations(chainIntegrity, merkleIntegrity, anomalies) {
    ...
    if (!chainIntegrity.valid) {
      recommendations.push({
        ...
        steps: ['Identify broken links', 'Recalculate hashes', 'Reconstruct chain'],
        deadline: 'IMMEDIATE',
      });
    }
    ...
    if (!merkleIntegrity.valid) {
      recommendations.push({
        ...
        steps: ['Backup current tree', 'Rebuild from leaf nodes', 'Verify new root'],
  ...
  async handleIntegrityStatus(status, anomalies) {
    switch (status) {
      ...
      case 'HEALTHY':
        ...
        if (Math.random() < 0.1) {
          // 10% chance to log healthy status
          await this.log({
            userId: 'SYSTEM',
  ...
  async generateComplianceReport(reportType, dateRange = {}) {
    try {
      ...
      const auditRecords = await this.AuditTrailModel.find(query).sort({ timestamp: -1 }).lean();
      ...
      const report = {
        ...
        metadata: {
          ...
          reportDuration: `${startDate.toISOString().split('T')[0]} to ${
  ...
  assessRetentionCompliance(auditRecords, requiredRetentionDays) {
    ...
    if (recordsInScope.length < auditRecords.length * 0.9) {
      // Less than 90% retention
  ...
  assessValidationCompliance(auditRecords, validationRules) {
    ...
    validationRules.forEach((rule) => {
      ...
      const compliantRecords = auditRecords.filter(
        (record) =>
      ...
      if (complianceRate < 95) {
        // Less than 95% compliance for validation rules
        compliance.overallCompliance = 'PARTIALLY_COMPLIANT';
        compliance.findings.push({
  ...
  generateEnhancedComplianceRecommendations(stats, gaps, assessment) {
    ...
    gaps.forEach((gap) => {
      recommendations.push({
        priority:
          gap.severity === 'CRITICAL' ? 'HIGH' : gap.severity === 'HIGH' ? 'HIGH' : 'MEDIUM',
  ...
  collectPerformanceMetrics() {
    ...
    if (this.serviceState.performanceHistory.length > 1000) {
      this.serviceState.performanceHistory = this.serviceState.performanceHistory.slice(-1000);
    }
  ...
  checkPerformanceDegradation(metrics) {
    ...
    if (metrics.performance.averageLogTime > 100) {
  ...
  getLastDayMetrics() {
    ...
    const totalLogs = dailyMetrics.reduce((sum, m) => sum + (m.performance.logsProcessed || 0), 0);
  ...
  async shutdown() {
    try {
      ...
      // Close workers
      const workers = [this.auditWorker, this.integrityWorker];
