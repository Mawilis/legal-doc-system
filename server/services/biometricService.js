...
const BIOMETRIC_CONFIG = {
  ...
  FICA: {
    ...
    ACCEPTABLE_BIOMETRIC_METHODS: ['FINGERPRINT', 'FACIAL_RECOGNITION', 'IRIS_SCAN', 'VOICE_PRINT'],
...
class BiometricService {
  ...
  async generateBiometricSignature(user, documentHash, biometricData) {
    try {
      ...
      const verification = await this.verifyBiometric(user._id, biometricData, {
      ...
      const signatureId = `BIOMETRIC_SIGNATURE_${Date.now()}_${crypto
        .randomBytes(4)
        .toString('hex')}`;
  ...
  async revokeBiometric(userId) {
    try {
      ...
      if (user.consent?.biometric?.consentId) {
        await this.consentService.revokeConsent(userId, user.consent.biometric.consentId);
      }
