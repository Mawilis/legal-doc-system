...
const QUANTUM_CONFIG = {
  ...
  API_KEY:
    process.env.NODE_ENV === 'production'
      ...
      : process.env.CIPC_SANDBOX_API_KEY || process.env.CIPC_API_KEY,
...
class CIPCAPIClient {
          ...
          switch (error.response.status) {
            ...
            case 429:
              ...
              if (
                ...
                const delay = QUANTUM_CONFIG.RETRY_DELAY * 2 ** originalRequest._retryCount;
    ...
    try {
      ...
      await this._logComplianceAction('ANNUAL_RETURN_SUBMISSION', {
        ...
        certificateHash: crypto
          .createHash('sha256')
          .update(JSON.stringify(submissionCertificate))
          .digest('hex'),
      });
    ...
    const processedData = {
      ...
      complianceMarkers: {
        ...
        annualReturnUpToDate: rawData.lastAnnualReturnDate
          ? moment().diff(moment(rawData.lastAnnualReturnDate), 'years') < 1
          : false,
        directorsInCompliance:
          rawData.directors?.every((dir) => dir.complianceStatus === 'Compliant') || false,
    ...
    return {
      ...
      requirements: {
        ...
        annualReturns: {
          required: true,
          status: lastARDate
            ? now.diff(lastARDate, 'years') < 1
              ? 'Compliant'
              : 'Overdue'
            : 'Missing',
          dueDate: lastARDate ? lastARDate.add(1, 'year').toISOString() : null,
          penalty:
            lastARDate && now.diff(lastARDate, 'years') >= 1
              ? this._calculatePenalty(lastARDate)
              : 0,
        },
        financialStatements: {
          required:
          ...
          exemption:
            companyDetails.turnover <= 5000000 ? 'May qualify for SME exemption' : 'Not exempt',
        },
        directorCompliance: {
          required: true,
          status: directors.every((d) => d.complianceStatus === 'Compliant')
            ? 'Compliant'
            : 'Non-Compliant',
          nonCompliantDirectors: directors
  ...
  _generateRegistrationCertificate(registrationData) {
    const certificatePayload = {
      ...
      quantumSignature: crypto
        .createHash('sha256')
        .update(JSON.stringify(registrationData))
        .digest('hex'),
    };
...
export default {
  ...
  getCacheStats: () => quantumCache.getStats(),
};
