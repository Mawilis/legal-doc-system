...
const DASHBOARD_CONFIG = {
  ...
  METRICS: {
    COMPLIANCE_SCORE_WEIGHTS: {
      ...
      ECT_ACT: 0.2, // Electronic Communications and Transactions Act
      COMPANIES_ACT: 0.2, // Companies Act 71 of 2008
      FICA: 0.1, // Financial Intelligence Centre Act
      OTHER: 0.1, // Other regulations (GDPR, etc.)
    },
    RISK_THRESHOLDS: {
...
class ComplianceDashboardService {
  ...
  async handleConnection(socket) {
    ...
    socket.emit(DASHBOARD_EVENTS.CONNECTION.AUTHENTICATED, {
      ...
      dataProcessingNotice:
        'Your compliance data is being processed in real-time for monitoring purposes.',
    });
  ...
  async generateComplianceExport(userId, timeframe, dataType) {
    ...
    return {
      ...
      paiaNotice:
        'This data is provided in accordance with the Promotion of Access to Information Act, 2000',
      contactInformation: {
        informationOfficer: await this.getInformationOfficer(userId),
   ...
   */
  getTotalConnections() {
    return Array.from(this.userSessions.values()).reduce((sum, sessions) => sum + sessions.size, 0);
  }
