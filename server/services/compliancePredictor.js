...
const REQUIRED_ENV_VARS = ['MONGO_URI', 'ENCRYPTION_KEY', 'MODEL_ENCRYPTION_KEY'];
...
class CompliancePredictor {
    ...
    try {
      ...
      this.modelPerformance = {
        ...
        recall: history.history.recall
          ? history.history.recall[history.history.recall.length - 1]
          : 0,
  ...
  async extractFeaturesFromEvent(event) {
    ...
    if (event.metadata.documentId) {
      const doc = await LegalDocument.findById(event.metadata.documentId)
        .lean()
        .catch(() => null);
    ...
    if (event.metadata.userId) {
      const user = await User.findById(event.metadata.userId)
        .lean()
        .catch(() => null);
  ...
  createLabelFromOutcome(event) {
    ...
    if (event.metadata.hasViolation === true) {
      if (event.metadata.penaltyAmount > 1000000) {
        // R1M+ penalty
        label[0] = 1; // CRITICAL
      } else if (event.metadata.penaltyAmount > 100000) {
        // R100K+ penalty
        label[1] = 1; // HIGH
      } else if (event.metadata.penaltyAmount > 10000) {
        // R10K+ penalty
        label[2] = 1; // MEDIUM
      } else {
        label[3] = 1; // LOW
  ...
  async extractConfidenceFactors(context) {
    ...
    if (context.documentId) {
      const doc = await LegalDocument.findById(context.documentId)
        .lean()
        .catch(() => null);
      if (doc) {
        factors.push({
  ...
  getRegulatoryReferences(jurisdiction) {
    const references = {
      POPIA: [
        {
          section: 'Section 19',
          title: 'Responsible party to secure integrity of personal information',
        },
        {
          section: 'Section 20',
          title: 'Information processed by operator or person acting under authority',
        },
      ...
      default: [{ section: 'General Compliance', title: 'Standard legal compliance requirements' }],
    };
      ...
      checks.push({
        ...
        checks: [
          { check: 'Data minimization', passed: this.checkDataMinimization(document.content) },
          {
            ...
            passed: this.checkPurposeSpecification(document.content),
          },
  ...
  generateDocumentRecommendations(overallRisk, riskPrediction, contentAnalysis, complianceCheck) {
    ...
    complianceCheck.checks.forEach((frameworkCheck) => {
      ...
      if (failedChecks.length > 0) {
        recommendations.push(
          `Address ${frameworkCheck.framework} compliance issues: ${failedChecks
            .map((c) => c.check)
            .join(', ')}`
        );
      }
...
if (process.env.NODE_ENV === 'test') {
  const testPredictor = async () => {
    ...
    const mockDocument = {
      _id: 'test-doc-456',
      content:
        'This agreement is made between Party A and Party B for the purpose of data processing.',
