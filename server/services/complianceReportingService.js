    ...
    try {
      ...
      const reportData = {
        ...
        dsarPerformance: {
          ...
          section23Compliance: dsarRequests.find((r) => r._id === 'OVERDUE')
            ? 'NON_COMPLIANT'
            : 'COMPLIANT',
          averageResponseTime:
    ...
    try {
      ...
      const reportData = {
        ...
        retentionCompliance: {
          ...
          section24Compliance: retentionData.every((d) => d.over7Years === 0)
            ? 'COMPLIANT'
            : 'NON_COMPLIANT',
  ...
  async generatePDFReport(reportData, reportType = 'POPIA') {
    return new Promise((resolve, reject) => {
      try {
        ...
        // Header
        doc
          .image(path.join(__dirname, '../assets/wilsy-logo.png'), 50, 45, { width: 100 })
          .fillColor('#1a237e')
          .fontSize(20)
    ...
    try {
      ...
      const healthReport = {
        ...
        checks: results.map((result, index) => ({
          check: ['POPIA', 'PAIA', 'Companies Act', 'Data Retention', 'Audit Log', 'Encryption'][
            index
          ],
