    ...
    try {
      ...
      const baseRules = [
        ...
        },
        {
          ...
          condition:
            '$.customer.idNumber and $.customer.dateOfBirth and $.customer.residentialAddress',
          severity: 'CRITICAL',
          priority: 1,
          active: true,
          metadata: {
      ...
      for (const rule of baseRules) {
        ...
        if (this.Rule) {
          await this.Rule.findOneAndUpdate({ ruleId: rule.ruleId }, rule, {
            upsert: true,
            new: true,
          });
        }
  ...
  async evaluateRule(ruleId, context, data) {
    ...
    try {
      ...
      const result = {
        ...
        transformedData:
          rule.type === RULE_ENGINE_CONSTANTS.RULE_TYPES.TRANSFORMATION
            ? transformedData
            : undefined,
        performance: {
          totalMs: Date.now() - startTime,
      ...
      try {
        ...
        this.Rule = mongoose.model(
          ...
          new mongoose.Schema({
            ...
            type: {
              type: String,
              enum: Object.values(RULE_ENGINE_CONSTANTS.RULE_TYPES),
              required: true,
            },
            ...
            context: {
              type: String,
              enum: Object.values(RULE_ENGINE_CONSTANTS.EXECUTION_CONTEXT),
              required: true,
            },
        ...
        this.RuleEvaluation = mongoose.model(
          ...
          new mongoose.Schema({
            ...
            status: {
              type: String,
              enum: Object.values(RULE_ENGINE_CONSTANTS.EVALUATION_STATUS),
              required: true,
            },
  ...
  async checkPerformanceMetrics() {
    try {
      ...
      return {
        name: 'Performance',
        status:
          evaluationsPerSecond > 10 ? 'HEALTHY' : evaluationsPerSecond > 1 ? 'DEGRADED' : 'POOR',
        details: {
