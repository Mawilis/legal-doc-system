...
class CryptoService {
  ...
  async _escrowQuantumKey(keyId, privateKey, metadata) {
    try {
      ...
      await fs.writeFile(escrowPath, JSON.stringify(escrowPackage, null, 2), { mode: 0o600 });
  ...
  async rotateMasterKey(reason = 'SCHEDULED_ROTATION') {
    try {
      ...
      await this._escrowQuantumKey(oldKeyId, this.keyManagement.masterKey.toString('hex'), {
