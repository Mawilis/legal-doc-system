...
const { appendAuditEntry } = require('../lib/auditLedger');
...
class MultiTenantStorageEngine {
  ...
  async storeDocument(fileBuffer, metadata) {
    ...
    try {
      ...
      const wrappedKey = await wrapKey(dek.toString('base64'), `tenant-${metadata.tenantId}`, {
      ...
      const fileName = `${metadata.tenantId}_${storageId.toString()}_${
        metadata.originalFileName || 'document'
      }`;
  ...
  _generateContentHash(buffer) {
    return crypto.createHash(DOCUMENT_CONFIG.HASH_ALGORITHM).update(buffer).digest('hex');
  }
...
class MultiTenantDocumentService {
  ...
  async updateDocument(documentId, updates, userContext) {
    ...
    try {
      ...
      // Create new version
      const newVersion = {
        major: updates.majorVersion
          ? existingDocument.version.major + 1
          : existingDocument.version.major,
  ...
  _clearDocumentCaches(tenantId, documentId) {
    const cacheKeys = [`${tenantId}:metadata:${documentId}`, `${tenantId}:document:${documentId}`];
  ...
  async _auditDocumentCompliance(document) {
    ...
    if (document.piiClassification.containsPII && document.confidentialityLevel === 'PUBLIC') {
      complianceIssues.push({
