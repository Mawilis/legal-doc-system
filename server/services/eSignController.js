...
class ESignController {
  ...
  async validateAdvancedSignature(signatureData) {
    ...
    try {
      ...
      const validationId = `ESIGN-VAL-${Date.now()}-${Math.random()
        .toString(36)
        .substr(2, 9)
        .toUpperCase()}`;
      ...
      const validationResult = {
        ...
        complianceMarkers: {
          ...
          nonRepudiation:
            signatureType === 'ADVANCED' || signatureType === 'QUALIFIED'
              ? 'GUARANTEED'
              : 'NOT_GUARANTEED',
      ...
      logger.info(
        'Advanced electronic signature validated',
        quantumCrypto.redactSensitive({
          tenantId,
          validationId,
          signatureType,
          documentType,
      ...
      logger.error(
        'Advanced signature validation failed',
        quantumCrypto.redactSensitive({
          tenantId,
          error: error.message,
          signatureType,
  ...
  async generateDigitalSignature(document, signatory, tenantId) {
    ...
    try {
      ...
      const signatureId = `ESIGN-${Date.now()}-${Math.random()
        .toString(36)
        .substr(2, 9)
        .toUpperCase()}`;
      ...
      const signaturePackage = {
        ...
        verificationUrl: `${
          process.env.APP_URL || 'https://app.wilsy.co.za'
        }/esign/verify/${signatureId}`,
  ...
  async getSignatureStatus(signatureId, tenantId) {
    try {
      ...
      logger.error(
        'Signature status retrieval failed',
        quantumCrypto.redactSensitive({
          tenantId,
          signatureId,
          error: error.message,
  ...
  _generateSignatureLegalNotice(signaturePackage) {
    return {
      notice:
        'This advanced electronic signature is provided under Section 13 of the Electronic Communications and Transactions Act 25 of 2002.',
      legalReferences: [
      ...
      retentionNotice:
        'This signature record will be retained for 10 years as required by the Companies Act.',
