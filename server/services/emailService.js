...
class TemplateEngine {
  ...
  async loadTemplate(templateName, locale = EMAIL_CONFIG.TEMPLATES.DEFAULT_LOCALE) {
    ...
    try {
      ...
      let templatePath = path.join(EMAIL_CONFIG.TEMPLATES.DIRECTORY, locale, `${templateName}.hbs`);
      ...
      // Fallback to English
      if (!(await this.fileExists(templatePath))) {
        templatePath = path.join(EMAIL_CONFIG.TEMPLATES.DIRECTORY, 'en', `${templateName}.hbs`);
      }
...
class EmailDeliveryService {
  ...
  cleanupRateLimitTracker() {
    ...
    for (const key of this.rateLimitTracker.keys()) {
      ...
      if ((type === 'minute' && ts < minuteAgo) || (type === 'day' && ts < dayAgo)) {
...
class EmailService {
  ...
  static async sendTemplate(templateName, data = {}, options = {}) {
    ...
    try {
      // 1. Render template
      const rendered = await templateEngine.render(templateName, data, options.locale);
