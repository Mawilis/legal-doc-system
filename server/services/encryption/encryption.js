...
const initializeKMSClient = () => {
  if (process.env.NODE_ENV === 'production' && ENCRYPTION_CONFIG.KEY_MANAGEMENT.STORAGE.AWS_KMS) {
    const kmsConfig = {
      ...
      retryDelayOptions: {
        base: 100,
        customBackoff: (retryCount) => Math.min(1000, 50 * 2 ** retryCount),
      },
      httpOptions: {
        timeout: 10000,
        connectTimeout: 5000,
...
class KeyManagementService {
  ...
  static async generateDataKey(tenantId, keyPurpose = 'default') {
    ...
    const keyId = `key_${tenantId}_${keyPurpose}_${Date.now()}_${crypto
      .randomBytes(8)
      .toString('hex')}`;
...
class EncryptionService {
  ...
  static async encrypt(plaintext, tenantId, keyPurpose = 'default') {
    ...
    try {
      ...
      const hmac = crypto.createHmac(ENCRYPTION_CONFIG.ALGORITHMS.HASH.SHA_512, keyMaterial);
      hmac.update(iv);
  ...
  static hmac(data, key) {
    ...
    return crypto
      .createHmac(ENCRYPTION_CONFIG.ALGORITHMS.HASH.SHA_512, keyBuffer)
      .update(dataBuffer)
      .digest('hex');
  }
}
...
class FieldEncryptionService {
  ...
  static isEncrypted(value) {
    return value && typeof value === 'object' && value.ciphertext && value.keyId && value.algorithm;
  }
}
...
class EncryptionHealthMonitor {
  ...
  static async getHealthStatus() {
    ...
    try {
      ...
      const kmsHealthy = kmsClient
        ? await this.checkKMSHealth()
