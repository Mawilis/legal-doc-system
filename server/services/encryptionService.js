...
class EncryptionService {
  ...
  generateQuantumIV() {
    ...
    return crypto
      ...
      .digest()
      .slice(0, QUANTUM_ENCRYPTION_STANDARDS.AES.IV_LENGTH);
  }
  ...
  async simulateHSMSigning(data, keyId) {
    const signature = crypto.createSign('SHA512').update(data).sign(this.masterKey.derivedKey);
  ...
  async simulateHSMVerification(data, signature, keyId) {
    const verifier = crypto.createVerify('SHA512').update(data);

    const isValid = verifier.verify(this.masterKey.derivedKey, Buffer.from(signature, 'base64'));
  ...
  async healthCheck() {
    ...
    try {
      ...
      const activeKey = this.encryptionState.activeKeyVersion
        ? this.encryptionState.keyRing.get(this.encryptionState.activeKeyVersion)
        : null;
      checks.push({
        check: 'ACTIVE_KEY',
