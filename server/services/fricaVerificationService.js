...
class FicaVerificationService {
  ...
  generateRequestSignature(data, timestamp) {
    ...
    return crypto
      .createHmac('sha256', process.env.FICA_SIGNING_KEY || process.env.FICA_API_SECRET)
      .update(payload)
      .digest('hex');
  }
  ...
  async fallbackIdentityVerification(identityData, tenantId, originalError) {
    ...
    const firstNameMatch = identityData.firstName && identityData.firstName.trim().length >= 2;
    const lastNameMatch = identityData.lastName && identityData.lastName.trim().length >= 2;
  ...
  async verifyDocument(documentBuffer, documentType, metadata, tenantId) {
    try {
      ...
      return this.fallbackDocumentVerification(
        documentBuffer,
        documentType,
        metadata,
        tenantId,
   ...
   * @returns {Promise<Object>} Fallback verification result
   */
  async fallbackDocumentVerification(
    documentBuffer,
    documentType,
    metadata,
    tenantId,
  ...
  async screenPEPAndSanctions(screeningData, tenantId) {
    try {
      ...
      const encryptedId = screeningData.idNumber
        ? encryptSensitiveData(screeningData.idNumber, 'FICA')
        : null;
      ...
      const screeningResult = {
        ...
        overallRisk: this.calculateOverallRisk(pepResponse.data, sanctionsResponse.data),
        referenceNumbers: {
      ...
      await this.logVerificationEvent({
        ...
        identityHash: screeningData.idNumber
          ? crypto.createHash('sha256').update(screeningData.idNumber).digest('hex')
          : 'no_id',
  ...
  async performRiskAssessment(clientId, tenantId) {
    try {
      ...
      const [identityRisk, documentRisk, pepRisk, transactionRisk] = await Promise.all([
