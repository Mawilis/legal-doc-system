...
async function safeResolveSecret(envVarValue, secretRef) {
  ...
  if (typeof secretRef === 'string' && secretRef.startsWith('kms://')) {
    if (!KMS || typeof KMS.resolveSecret !== 'function') {
      logger.warn('integrationService: KMS resolver not configured but secretRef provided', {
        secretRef,
      });
      return null;
    }
    try {
      ...
      logger.warn('integrationService: KMS resolve failed', {
        secretRef,
  ...
  logger.warn('integrationService: plaintext secretRef detected in DB; ignoring for security', {
    secretRef,
  });
  return null;
}
...
async function getProviderConfig(provider) {
  ...
  try {
    ...
    logger.error('integrationService: DB lookup failed', {
      provider: key,
      err: e && e.message ? e.message : e,
    });
    return null;
  ...
  try {
    ...
    logger.warn('integrationService: credential merge failed', {
      provider: key,
      err: e && e.message ? e.message : e,
    });
  }
...
async function listProviders({ enabledOnly = true } = {}) {
  ...
  const docs = await Integration.find(q)
    .select('provider name enabled lastHealthStatus lastHealthCheckAt')
    .lean();
  return docs || [];
}
...
async function emitIntegrationAudit({
  ...
  try {
    ...
    logger.warn('integrationService: audit write failed', {
      err: e && e.message ? e.message : e,
      eventType,
    });
  }
