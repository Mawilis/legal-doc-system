...
class ComplianceChecker {
  ...
  generateRegulationSummary(results) {
    ...
    return `${passed}/${total} rules passed (${percentage.toFixed(1)}%)${
...
${
  criticalFailures > 0
    ? '⚠️  IMMEDIATE ACTION REQUIRED: Critical compliance failures detected'
    ...
    for (const [regName, regData] of Object.entries(results)) {
      ...
      for (const rule of failedRules) {
        recommendations.push({
          ...
          priority:
            rule.severity === 'critical' ? 'HIGH' : rule.severity === 'high' ? 'MEDIUM' : 'LOW',
        });
      }
    }
...
if (process.env.NODE_ENV === 'test') {
  ...
  describe('ComplianceChecker Tests', () => {
    ...
    test('should check for clauses correctly', () => {
      const result = checker.checkForClause(compliantDocument, [
        'data processing agreement',
        'POPIA',
        'encryption',
      ]);
