...
class ContractAnalyzer {
  ...
  validateContractInput(contractText) {
    ...
    if (contractText.length > 1000000) {
      // 1MB limit
    ...
    const inputHash = crypto.createHash('sha256').update(sanitizedText).digest('hex');
...
- summary`;
  ...
  checkCompliance(clauses) {
    ...
    const complianceScore = totalIssues > 0 ? Math.max(0, 100 - totalIssues * 10) : 100;
...
${
  riskScore >= 7
    ? '⚠️  RECOMMENDED: Legal review strongly advised'
    : '✅ Contract appears acceptable with standard review'
}`;
  }
}
