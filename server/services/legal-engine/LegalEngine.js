...
class LegalEngine {
  ...
  async _executeECTSignatureRule(data) {
    const {
      contract,
      minimumSignatures = 1,
      signatureType = 'ADVANCED_ELECTRONIC_SIGNATURE',
    } = data;
    ...
    try {
      ...
      const auditEntry = new AuditLedger({
        ...
        complianceReferences: [
          auditData.ruleStatute
            ? `${auditData.ruleStatute} ${auditData.ruleSection}`
            : 'GENERAL_COMPLIANCE',
        ],
  ...
  async resolveRuleConflict(ruleA, ruleB, data) {
    ...
    // Execute both rules
    const [resultA, resultB] = await Promise.all([ruleAObj.execute(data), ruleBObj.execute(data)]);
