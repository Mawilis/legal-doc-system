...
class AuditChain {
  ...
  _calculateHash(block) {
    ...
    return crypto.createHash('sha3-512').update(blockString).digest('hex');
  }
  ...
  _generateEvidenceHash(block) {
    ...
    return crypto.createHash('sha3-512').update(evidenceString).digest('hex');
  }
  ...
  _generateMerkleProof(block) {
    ...
    const proof = {
      ...
      transactionEvidence: {
        ...
        hashRange: `${transactionHashes[0]?.substring(0, 8)}...${transactionHashes[
...
class LpcService {
  ...
  async init(config) {
    ...
    try {
      ...
      this._metrics.errorCount++;
      throw this._errorHandler.handleServiceUnavailableError('LPC Service initialization failed', {
        service: 'LPCService',
        error: error.message,
        code: 'LPC_INIT_001',
      });
  ...
  _validateInitConfig(config) {
    ...
    try {
      ...
      throw this._errorHandler.handleValidationError('Invalid LPC API Base URL', {
        field: 'lpcApiBaseUrl',
  ...
  async _initCacheLayer(redisUrl) {
    try {
      ...
      this._redisClient = redis.createClient({
        ...
        socket: {
          reconnectStrategy: (retries) => {
            if (retries > 10) {
              throw this._errorHandler.handleRetryableError('Redis max retries exceeded', {
      ...
      this._redisClient.on('error', (err) => {
        ...
        this._errorHandler.handleServiceUnavailableError('Redis connection failed', {
          service: 'Redis',
          error: err.message,
          fallbackActive: true,
          code: 'LPC_CACHE_002',
  ...
  _initializeComplianceMonitors() {
    ...
    this._anomalyDetector = setInterval(() => this._detectAnomalies(), 5 * 60 * 1000).unref();
  }
  ...
  _trackApiCall(tenantId, duration) {
    ...
    if (currentCount + 1 > LPC_STATUTORY_LIMITS.API_RATE_LIMIT) {
      throw this._errorHandler.handleRateLimitError('API rate limit exceeded', {
  ...
  validateTenantId(tenantId) {
    try {
      ...
      throw this._errorHandler.handleDataIntegrityError('Invalid tenant ID format', {
        entityType: 'Tenant',
        entityId: tenantId,
      ...
      switch (resourceType) {
        case 'attorney': {
          resource = await AttorneyProfile.findOne({
            $or: [{ lpcNumber: resourceId }, { practiceNumber: resourceId }, { _id: resourceId }],
            tenantId,
            deleted: false,
          })
            .lean()
        ...
        case 'cpd_record': {
          resource = await CPDRecord.findOne({
            $or: [{ activityId: resourceId }, { certificateId: resourceId }, { _id: resourceId }],
            tenantId,
            deleted: false,
          })
            .lean()
        ...
        case 'compliance_audit': {
          resource = await ComplianceAudit.findOne({
            $or: [{ auditId: resourceId }, { _id: resourceId }],
            tenantId,
            deleted: false,
          })
            .lean()
        ...
        case 'transaction': {
          resource = await Transaction.findOne({
            $or: [{ transactionId: resourceId }, { _id: resourceId }],
            tenantId,
            deleted: false,
          })
            .lean()
        ...
        default:
          throw this._errorHandler.handleValidationError(`Unknown resource type: ${resourceType}`, {
            field: 'resourceType',
            value: resourceType,
            constraint:
              'attorney, trust_account, cpd_record, fidelity_certificate, compliance_audit, transaction, user',
            code: 'LPC_ACCESS_001',
          });
      }
  ...
  detectPIIViolation(data) {
    ...
    if (violations.length > 0) {
      this._errorHandler.handlePOPIAComplianceError('PII violation detected', {
        section: '19',
  ...
  async generatePOPIACertificate(dataSubjectId, userContext) {
    ...
    const certificateId = `POPIA-CERT-${DateTime.now().toFormat('yyyyMMdd')}-${crypto
      .randomBytes(4)
      .toString('hex')
      .toUpperCase()}`;

    const accessLogs = await AuditLedger.find({
      $or: [{ identifier: dataSubjectId }, { 'metadata.dataSubjectId': dataSubjectId }],
  ...
  async createAttorneyProfile(profileData, userContext) {
    ...
    try {
      ...
      if (!LPC_VALIDATION_PATTERNS.LPC_NUMBER.test(profileData.lpcNumber)) {
        throw this._errorHandler.handleValidationError('Invalid LPC number format', {
          field: 'lpcNumber',
  ...
  async getAttorneyProfile(lpcNumber, tenantId, userContext) {
    ...
    await this._auditService.recordAccess('attorney_profile', lpcNumber, userContext, 'VIEW', {
      tenantId,
      accessId,
    ...
    if (cached) {
      ...
      await this._auditService.recordAccess('cache', cacheKey, userContext, 'CACHE_HIT', {
    ...
    if (!attorney) {
      throw this._errorHandler.handleDataIntegrityError(`Attorney not found: ${lpcNumber}`, {
        entityType: 'AttorneyProfile',
        entityId: lpcNumber,
        expectedHash: 'exists',
  ...
  async updateAttorneyProfile(lpcNumber, updates, userContext) {
    ...
    await this._auditService.recordAccess('attorney_profile', attorney._id, userContext, 'UPDATE', {
      lpcNumber,
      changes,
    });
  ...
  async createTrustAccount(accountData, attorneyLpcNumber, userContext) {
    ...
    if (!accountData.bankDetails?.accountNumber || !accountData.bankDetails?.branchCode) {
      throw this._errorHandler.handleValidationError('Invalid bank account details', {
        field: 'bankDetails',
  ...
  async confirmTrustAccount(accountNumber, confirmationData, userContext) {
    ...
    if (!trustAccount) {
      throw this._errorHandler.handleDataIntegrityError('Trust account not found', {
        entityType: 'TrustAccount',
    ...
    if (trustAccount.status !== 'PENDING_CONFIRMATION') {
      throw this._errorHandler.handleValidationError('Account already confirmed', {
        field: 'status',
    ...
    if (!isValid) {
      ...
      throw this._errorHandler.handleValidationError('Invalid confirmation code', {
        field: 'code',
  ...
  async processTrustTransaction(transactionData, attorneyLpcNumber, userContext) {
    ...
    try {
      ...
      throw this._errorHandler.handleRetryableError('Transaction processing failed', {
  ...
  async getTrustAccountBalance(accountId, userContext) {
    ...
    await this._auditService.recordAccess('trust_balance', accountId, userContext, 'CHECK', {
      checkId,
    ...
    if (!trustAccount) {
      throw this._errorHandler.handleDataIntegrityError(`Trust account not found: ${accountId}`, {
        entityType: 'TrustAccount',
        entityId: accountId,
        expectedHash: 'exists',
        actualHash: 'not_found',
    ...
    const balanceData = {
      ...
      statistics: {
        ...
        averageTransactionValue:
          trustAccount.transactionCount > 0
  ...
  async triggerTrustReconciliation(attorneyLpcNumber, userContext) {
    ...
    try {
      ...
      const complianceAudit = new ComplianceAudit({
        ...
        findings:
          reconciliationResult.discrepancies?.map((d) => ({
            ...
            severity:
              Math.abs(d.difference) > 10000
                ? 'CRITICAL'
                : Math.abs(d.difference) > 5000
                  ? 'HIGH'
                  : 'MEDIUM',
      ...
      if (!reconciliationResult.isReconciled) {
        ...
        this._errorHandler.handleLPCComplianceError('Trust reconciliation discrepancy detected', {
          rule: 'LPC_3.4.1',
  ...
  async getTrustAccountSummary(attorneyLpcNumber, userContext) {
    ...
    return {
      ...
      clientSummary: {
        ...
        activeClients: trustAccount.clientBalances?.filter((c) => c.balance > 0).length || 0,
        totalClientBalances:
          trustAccount.clientBalances?.reduce((sum, c) => sum + c.balance, 0) || 0,
      ...
      transactionSummary: {
        ...
        lastTransactionDate:
          trustAccount.transactions?.[trustAccount.transactions.length - 1]?.date,
        averageTransactionValue:
          trustAccount.transactionCount > 0
            ? trustAccount.balances.current / trustAccount.transactionCount
            : 0,
      ...
      reconciliationStatus: {
        ...
        reconciliationScore: trustAccount.compliance.reconciliationScore,
        complianceLevel:
          trustAccount.compliance.reconciliationScore >= 90
            ? 'EXCELLENT'
            : trustAccount.compliance.reconciliationScore >= 75
              ? 'GOOD'
              : trustAccount.compliance.reconciliationScore >= 60
                ? 'FAIR'
                : 'POOR',
      },
      compliance: {
  ...
  async trackCPDActivity(activityData, attorneyLpcNumber, userContext) {
    ...
    try {
      ...
      if (activityData.category === 'ETHICS' && activityData.hours < 1) {
        throw this._errorHandler.handleValidationError('Ethics CPD must be at least 1 hour', {
          field: 'hours',
          value: activityData.hours,
          constraint: '≥ 1 hour',
      ...
      await this._auditService.recordAccess('cpd_activity', activityId, userContext, 'SUBMIT', {
        attorneyLpcNumber,
        hours: activityData.hours,
        category: activityData.category,
        isCompliant: cpdSummary.compliance.isCompliant,
  ...
  async getAttorneyCPDStatus(attorneyLpcNumber, year, userContext) {
    ...
    const tenantId = userContext?.tenantId;
    if (!tenantId) {
      throw this._errorHandler.handleValidationError('Tenant ID required for CPD status access', {
        field: 'tenantId',
        value: userContext?.tenantId,
        constraint: 'non-empty string',
        attorneyLpcNumber,
    ...
    await this._auditService.recordAccess('cpd_status', attorneyLpcNumber, userContext, 'VIEW', {
    ...
    if (!summary.compliance.isCompliant) {
      ...
      this._errorHandler.handleRegulatoryDeadlineError('CPD compliance deadline approaching', {
  ...
  async getCPDCertificate(activityId, userContext) {
    ...
    if (!cpdRecord) {
      throw this._errorHandler.handleDataIntegrityError(`CPD record not found: ${activityId}`, {
    ...
    await this._auditService.recordAccess('cpd_certificate', activityId, userContext, 'DOWNLOAD', {
  ...
  async _verifyCPDProvider(providerData) {
    ...
    try {
      ...
      this._errorHandler.handleServiceUnavailableError('Provider verification failed', {
  ...
  async applyCPDExemption(attorneyLpcNumber, exemptionData, userContext) {
    ...
    if (!attorney) {
      throw this._errorHandler.handleDataIntegrityError('Attorney not found', {
        entityType: 'AttorneyProfile',
        entityId: attorneyLpcNumber,
        expectedHash: 'exists',
        actualHash: 'not_found',
    ...
    if (!validGrounds.includes(exemptionData.grounds)) {
      throw this._errorHandler.handleValidationError('Invalid exemption grounds', {
        field: 'grounds',
  ...
  async bulkVerifyCPDActivities(firmId, year, userContext) {
    ...
    for (const activity of activities) {
      try {
        ...
        this._errorHandler.handleRetryableError('CPD verification failed', {
  ...
  async calculateFidelityFundContribution(attorneyLpcNumber, annualTurnover, userContext) {
    ...
    try {
      ...
      if (finalContribution < LPC_STATUTORY_LIMITS.FIDELITY_MINIMUM_CONTRIBUTION) {
        this._errorHandler.handleLPCComplianceError('Fidelity contribution below minimum', {
          rule: 'LPC_55.2',
          severity: 'HIGH',
          deadline: '30 days',
          attorneyLpcNumber,
  ...
  async processFidelityPayment(certificateId, paymentData, userContext) {
    ...
    if (!certificate) {
      throw this._errorHandler.handleDataIntegrityError('Fidelity certificate not found', {
    ...
    if (certificate.status !== 'PENDING_PAYMENT') {
      throw this._errorHandler.handleValidationError('Certificate not in PENDING_PAYMENT status', {
        field: 'status',
    ...
    if (paymentData.amount !== certificate.payment.amount) {
      throw this._errorHandler.handleValidationError('Incorrect payment amount', {
        field: 'amount',
    ...
    certificate.expiryDate = DateTime.fromJSDate(paymentResult.paidAt)
      .plus({ years: 1 })
      .toJSDate();
  ...
  async verifyFidelityCertificate(certificateId, userContext) {
    ...
    if (!certificate) {
      throw this._errorHandler.handleDataIntegrityError(`Certificate not found: ${certificateId}`, {
        entityType: 'FidelityFund',
        entityId: certificateId,
        expectedHash: 'exists',
        actualHash: 'not_found',
  ...
  async submitFidelityClaim(attorneyLpcNumber, claimData, userContext) {
    ...
    const startTime = Date.now();
    const claimId = `FCL-${DateTime.now().toFormat('yyyy')}-${crypto
      .randomBytes(4)
      .toString('hex')
      .toUpperCase()}`;
    ...
    try {
      ...
      if (!currentCertificate) {
        throw this._errorHandler.handleLPCComplianceError('No valid fidelity certificate found', {
          rule: 'LPC_55.6(1)',
          severity: 'CRITICAL',
          deadline: '30 days',
          attorneyLpcNumber,
      ...
      if (claimData.amount > LPC_STATUTORY_LIMITS.FIDELITY_CLAIM_LIMIT) {
        throw this._errorHandler.handleValidationError('Claim amount exceeds statutory limit', {
          field: 'amount',
      ...
      const claim = {
        ...
        submittedIp: userContext.ipAddress,
        submittedUserAgent: userContext.userAgent,
        estimatedDecisionDate: now
          .plus({ days: LPC_STATUTORY_LIMITS.FIDELITY_CLAIM_PROCESSING_DAYS })
          .toJSDate(),
      ...
      this._evidenceRegistry.set(`fidelity-claim:${claimId}`, {
        ...
        evidenceHash: crypto.createHash('sha3-512').update(JSON.stringify(claim)).digest('hex'),
      });
      ...
      if (sarRequired) {
        try {
          ...
          await this._auditService.recordAccess('fica_sar', claimId, userContext, 'SAR_SUBMITTED', {
            amount: claim.amount,
          ...
          this._errorHandler.handleRetryableError('SAR submission failed, queued for retry', {
      ...
      await this._auditService.recordAccess('fidelity_claim', claimId, userContext, 'SUBMIT', {
        attorneyLpcNumber,
        amount: claimData.amount,
        certificateId: currentCertificate.certificateId,
        sarRequired: claim.sarRequired,
  ...
  async _submitSARToFIC(claimData, userContext) {
    ...
    try {
      ...
      if (!this._config?.fic?.apiKey) {
        throw this._errorHandler.handleAuthenticationError('FIC API key not configured', {
      ...
      if (isAuthError) {
        throw this._errorHandler.handleAuthenticationError('FIC API authentication failed', {
          method: 'FIC_SUBMISSION',
          code: 'FICA_AUTH_001',
          endpoint: ficEndpoint,
        });
      } else if (isValidationError) {
        throw this._errorHandler.handleValidationError('Invalid SAR submission data', {
          field: 'claimData',
          value: claimData,
        ...
        throw this._errorHandler.handleRetryableError('FIC API unavailable', {
          operation: 'sar_submission',
          claimId: claimData.claimId,
          retryCount: 0,
          maxRetries: LPC_STATUTORY_LIMITS.FIC_MAX_RETRY_ATTEMPTS,
        ...
        throw this._errorHandler.handleServiceUnavailableError('FIC service unavailable', {
          service: 'FIC',
          endpoint: ficEndpoint,
        ...
        throw this._errorHandler.handleFICAComplianceError('SAR submission failed', {
  ...
  async performComplianceAudit(subjectId, subjectType, userContext) {
    ...
    const startTime = Date.now();
    const auditId = `AUDIT-${DateTime.now().toFormat('yyyy')}-${Date.now()
      .toString()
      .slice(-6)}-${crypto.randomBytes(4).toString('hex').toUpperCase()}`;
    ...
    try {
      ...
      const complianceAudit = new ComplianceAudit({
        ...
        subjectId,
        subjectModel:
          subjectType === 'attorney'
            ? 'AttorneyProfile'
            : subjectType === 'trust_account'
              ? 'TrustAccount'
              : 'Firm',
      ...
      if (auditData.score < 70) {
        this._errorHandler.handleLPCComplianceError('Compliance audit failed', {
          rule: 'LPC_95.3',
          severity: 'CRITICAL',
          deadline: '30 days',
      ...
      await this._auditService.recordAccess('compliance_audit', auditId, userContext, 'COMPLETE', {
        subjectType,
        subjectIdentifier: auditData.identifier,
        score: auditData.score,
        issuesFound: auditData.issues?.length || 0,
  ...
  async _auditAttorney(attorneyId, userContext) {
    ...
    if (!attorney) {
      throw this._errorHandler.handleDataIntegrityError(`Attorney not found: ${attorneyId}`, {
        entityType: 'AttorneyProfile',
        entityId: attorneyId,
        expectedHash: 'exists',
        actualHash: 'not_found',
    ...
    if (!cpdSummary.compliance.isCompliant) {
      ...
      findings.push({
        ...
        description: `Attorney is not CPD compliant for ${new Date().getFullYear()}. Required: 12 hours (2 ethics), Completed: ${
          cpdSummary.summary.effectiveHours
        } hours (${cpdSummary.summary.ethicsHours} ethics)`,
        severity,
        status: 'OPEN',
        detectedAt: new Date(),
  ...
  async getComplianceDashboard(tenantId, userContext) {
    ...
    const dashboard = {
      ...
      trustAccounts: {
        ...
        averageBalance:
          trustStats.totalAccounts > 0 ? trustStats.totalBalance / trustStats.totalAccounts : 0,
      ...
      fidelityFunds: {
        ...
        averageContribution:
          fidelityStats.active > 0 ? fidelityStats.totalContribution / fidelityStats.active : 0,
      },
      complianceAudits: {
    ...
    await this._auditService.recordAccess('compliance_dashboard', tenantId, userContext, 'VIEW', {
      overallScore,
      riskLevel,
    });
  ...
  async getComplianceReport(firmId, reportType, userContext) {
    ...
    if (firmId && firmId !== 'ALL') {
      ...
      if (!firmBelongsToTenant) {
        throw this._errorHandler.handleAuthenticationError('Firm does not belong to tenant', {
          userId: userContext.userId,
          method: 'TENANT_VALIDATION',
          code: 'LPC_AUTH_005',
        });
    ...
    const reportId = `RPT-${DateTime.now().toFormat('yyyyMMdd-HHmmss')}-${crypto
      .randomBytes(4)
      .toString('hex')
      .toUpperCase()}`;
    const reportTime = new Date();

    await this._auditService.recordAccess('compliance_report', reportId, userContext, 'GENERATE', {
      firmId: firmId || 'ALL',
      reportType,
    ...
    const complianceRate = this._calculateComplianceRate([
      attorneyStats,
      trustStats,
      cpdStats,
      fidelityStats,
    ]);
    const riskLevel = this._assessOverallRiskLevel([
      attorneyStats,
      trustStats,
      cpdStats,
      fidelityStats,
    ]);
    const highRiskAreas = this._identifyHighRiskAreas([
      attorneyStats,
      trustStats,
      cpdStats,
      fidelityStats,
  ...
  async getLPCMetrics(firmId, dateRange, userContext) {
    ...
    const metricsId = `METRICS-${DateTime.now().toFormat('yyyyMMdd-HHmmss')}-${crypto
      .randomBytes(4)
      .toString('hex')
      .toUpperCase()}`;

    await this._auditService.recordAccess('lpc_metrics', metricsId, userContext, 'ACCESS', {
      firmId: firmId || 'ALL',
      dateRange,
    ...
    const lpcScore = this._calculateLPCComplianceScore(
      attorneyMetrics,
      trustMetrics,
      cpdMetrics,
      fidelityMetrics
    ...
    const metrics = {
      ...
      period: {
        ...
        description: `${startDate.toISOString().split('T')[0]} to ${
    ...
    const cacheKey = `lpc-metrics:${tenantId}:${firmId || 'ALL'}:${
  ...
  async getMatterTransactions(matterId, accountNumber, userContext) {
    ...
    await this._auditService.recordAccess('matter_transactions', matterId, userContext, 'QUERY', {
    ...
    if (accountNumber) {
      if (!LPC_VALIDATION_PATTERNS.TRUST_ACCOUNT.test(accountNumber)) {
        throw this._errorHandler.handleValidationError('Invalid trust account number format', {
          field: 'accountNumber',
          value: accountNumber,
  ...
  async _checkFidelityCertificateExpiry() {
    ...
    for (const certificate of expiringCertificates) {
      ...
      if (daysUntilExpiry <= 7) {
        ...
        this._errorHandler.handleRegulatoryDeadlineError('Fidelity certificate expiring soon', {
  ...
  async _collectAttorneyMetrics(filters, startDate, endDate) {
    ...
    if (!filters?.tenantId) {
      throw this._errorHandler.handleValidationError('Tenant ID required for attorney metrics', {
  ...
  async _collectTrustMetrics(filters, startDate, endDate) {
    ...
    if (!filters?.tenantId) {
      throw this._errorHandler.handleValidationError('Tenant ID required for trust metrics', {
        field: 'filters.tenantId',
        value: filters?.tenantId,
        constraint: 'non-empty string',
        rule: 'LPC_86.5',
  ...
  async _collectCPDMetrics(filters, startDate, endDate) {
    ...
    if (!filters?.tenantId) {
      throw this._errorHandler.handleValidationError('Tenant ID required for CPD metrics', {
        field: 'filters.tenantId',
        value: filters?.tenantId,
        constraint: 'non-empty string',
  ...
  async _collectFidelityMetrics(filters, startDate, endDate) {
    ...
    if (!filters?.tenantId) {
      throw this._errorHandler.handleValidationError('Tenant ID required for fidelity metrics', {
        field: 'filters.tenantId',
        value: filters?.tenantId,
        constraint: 'non-empty string',
    ...
    const complianceRate = total > 0 ? (active / total) * 100 : 100;
  ...
  async _collectAuditMetrics(filters, startDate, endDate) {
    ...
    if (!filters?.tenantId) {
      throw this._errorHandler.handleValidationError('Tenant ID required for audit metrics', {
        field: 'filters.tenantId',
        value: filters?.tenantId,
        constraint: 'non-empty string',
    ...
    const complianceRate = totalAudits > 0 ? (completedAudits / totalAudits) * 100 : 100;
    ...
    metrics.forEach((metric, index) => {
      ...
      if (score < 70) {
        highRiskAreas.push({
          ...
          regulatoryRef:
            index === 0 ? 'LPC_55' : index === 1 ? 'LPC_86' : index === 2 ? 'LPC_3' : 'LPC_55',
  ...
  async _calculateTrends(tenantId, firmId, startDate, endDate) {
    ...
    if (!tenantId) {
      throw this._errorHandler.handleValidationError('Tenant ID required for trend analysis', {
        field: 'tenantId',
        value: tenantId,
        constraint: 'non-empty string',
    ...
    const history = await TrustAccount.aggregate([
      { $match: query },
      {
        $group: {
          _id: {
            date: {
              $dateToString: { format: '%Y-%m-%d', date: '$compliance.lastReconciliationDate' },
            },
          },
          count: { $sum: 1 },
  ...
  async _getCPDCategoryBreakdown(tenantId, firmId = null, year) {
    ...
    return breakdown.map((item) => ({
      ...
      percentage:
        item.hours > 0
          ? Math.round((item.hours / breakdown.reduce((sum, i) => sum + i.hours, 0)) * 100)
          : 0,
    }));
  }
  ...
  async _getFidelityExpiryForecast(tenantId, firmId = null, days = 90) {
    ...
    const certificates = await FidelityFund.find(query).sort({ expiryDate: 1 }).lean().exec();

    const forecast = [];

    for (let i = 0; i <= days; i += 7) {
      const date = DateTime.now().plus({ days: i }).toJSDate();
      const expiringCount = certificates.filter((c) => new Date(c.expiryDate) <= date).length;
  ...
  _generateRiskMatrix(metrics) {
    const likelihood = {
      attorney:
        metrics[0]?.complianceRate < 70
          ? 'HIGH'
          : metrics[0]?.complianceRate < 85
            ? 'MEDIUM'
            : 'LOW',
      trust:
        metrics[1]?.complianceRate < 70
          ? 'HIGH'
          : metrics[1]?.complianceRate < 85
            ? 'MEDIUM'
            : 'LOW',
      cpd:
        metrics[2]?.complianceRate < 70
          ? 'HIGH'
          : metrics[2]?.complianceRate < 85
            ? 'MEDIUM'
            : 'LOW',
      fidelity:
        metrics[3]?.complianceRate < 70
          ? 'HIGH'
          : metrics[3]?.complianceRate < 85
            ? 'MEDIUM'
            : 'LOW',
    };
  ...
  async _detectAnomalies() {
    ...
    try {
      const [
        ...
        TrustAccount.aggregate([
          { $match: { 'compliance.lastReconciliationDate': { $gte: dayAgo } } },
          {
            $group: {
              _id: null,
              total: { $sum: '$compliance.reconciliationScore' },
              avg: { $avg: '$compliance.reconciliationScore' },
            },
          },
        ]),
      ...
      if (trustDiscrepancies[0]?.avg < 70) {
        ...
        this._errorHandler.handleDataIntegrityError('Trust reconciliation anomaly detected', {
          entityType: 'TrustAccount',
      ...
      if (cpdNonCompliant.length > 10) {
        ...
        this._errorHandler.handleRegulatoryDeadlineError('CPD compliance anomaly detected', {
          requirement: 'LPC_CPD_ANNUAL',
          deadline: DateTime.fromObject({
            year: new Date().getFullYear(),
            month: 12,
      ...
      if (errorRate > this._anomalyThresholds.errorRate) {
        ...
        this._errorHandler.handleServiceUnavailableError('System error rate anomaly detected', {
          service: 'LPCService',
      ...
      if (rateLimitExceeded > 10) {
        ...
        this._errorHandler.handleRateLimitError('Rate limit anomaly detected', {
          limit: 1000,
      ...
      if (circuitBreakerTrips > 3) {
        ...
        this._errorHandler.handleCircuitBreakerError('Circuit breaker anomaly detected', {
          service: 'LPCService',
          state: 'OPEN',
  ...
  async _verifyBankConfirmation(accountNumber, code, amount) {
    ...
    if (trustAccount.status !== 'PENDING_CONFIRMATION') {
      throw this._errorHandler.handleValidationError('Account not in PENDING_CONFIRMATION status', {
        field: 'status',
        value: trustAccount.status,
        constraint: 'PENDING_CONFIRMATION',
        accountNumber,
    ...
    if (isValid) {
      ...
      if (trustAccount.metadata.verificationAttempts >= 3) {
        ...
        trustAccount.metadata.verificationLockedUntil = DateTime.now()
          .plus({ hours: 24 })
          .toJSDate();
        await trustAccount.save();
