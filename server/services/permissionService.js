...
function normalizeOverride(o) {
  ...
  if (typeof o === 'object' && o.permission) {
    ...
    return {
      permission: normalizePermission(o.permission),
...
async function fetchRoleByIdOrKey(idOrKey, { ttl = DEFAULT_TTL_MS, logger = defaultLogger } = {}) {
  ...
  try {
    ...
    if (!role) {
      role = await Role.findOne({ $or: [{ key: cacheKey }, { name: cacheKey }] })
        .lean()
        .exec();
    }
    if (role) {
    ...
    logger.error('permissionService.fetchRoleByIdOrKey error', {
      idOrKey,
...
async function computeEffectivePermissions(user = {}, opts = {}) {
  ...
  try {
    ...
    logger.error('permissionService.computeEffectivePermissions failed', {
      err: err && err.message ? err.message : err,
