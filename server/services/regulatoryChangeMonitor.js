...
class RegulatoryChangeMonitor extends EventEmitter {
        ...
        const baseRegulations = [
          {
            ...
            summary:
              'South African data protection legislation governing processing of personal information',
    ...
    this.alertSystem = {
      sendAlert: async (alertData) => {
        try {
          ...
          // Cache in Redis
          if (this.redisClient) {
            await this.redisClient.setex(`alert:${alertId}`, 604800, JSON.stringify(fullAlert));
          }
      ...
      getStats: () => ({
        ...
        last24Hours: Array.from(this.alertRegistry.values()).filter((alert) => {
  ...
  async storeRegulatoryUpdate(update, contentHash, analysis) {
    ...
    if (this.redisClient && update.id) {
      try {
        await this.redisClient.setex(`hash:${update.id}`, 30 * 86400, contentHash);
      } catch (error) {
  ...
  async generateRegulatoryAlert(updateAnalysis) {
    try {
      ...
      const alertData = {
        ...
        message: `A ${updateAnalysis.changeType.toLowerCase()} has been detected in ${
          updateAnalysis.category
        } regulations. Impact level: ${alertLevel}.`,
        level: alertLevel,
        ...
        attachments: [
          {
            ...
            fields: [
              ...
              {
                title: 'Deadline',
                value: new Date(alertData.deadline).toLocaleDateString(),
                short: true,
              },
            ],
          },
  ...
  getMetrics() {
    ...
    return {
      ...
      performance: {
        totalScans: this.monitoringState.totalScans,
        changeDetectionRate:
          this.monitoringState.totalScans > 0
            ? (this.monitoringState.changesDetected / this.monitoringState.totalScans).toFixed(2)
            : 0,
        alertGenerationRate:
