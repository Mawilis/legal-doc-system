...
class RiskAssessmentService {
  ...
  async assessFirmRisks(firmId, options = {}) {
    try {
      ...
      const [firm, users, documents, auditLogs, complianceReports] = await Promise.all([
        Firm.findById(firmId).populate('complianceOfficer').populate('trustAccountManager'),

        User.find({ firmId })
          .select('+popiaConsents +ficaVerification +signatureAuthority')
          .limit(100),

        Document.find({ firmId }).select('+metadata +retentionPeriod +signatureStatus').limit(500),
        ...
        ComplianceReport.find({ firmId }).sort({ createdAt: -1 }).limit(10),
      ]);
  ...
  async identifyRiskAreas(metrics) {
    ...
    if (metrics.compliance.electronicSignatureValidity < 90) {
      riskAreas.push({
        ...
        evidence: `${
          100 - metrics.compliance.electronicSignatureValidity
        }% of signatures have validity issues`,
  ...
  mapRisksToCompliance(riskAreas) {
    ...
    const complianceDictionary = {
      ...
      [RISK_CATEGORIES.COMPLIANCE.ECT_ACT_SIGNATURE]: {
        act: 'ECT Act',
        sections: ['13', '15'],
        requirements: ['Advanced electronic signatures', 'Non-repudiation'],
      },
  ...
  async assessDocumentRisks(documentId, options = {}) {
    try {
      // Retrieve document
      const document = await Document.findById(documentId).populate('createdBy').populate('firmId');
