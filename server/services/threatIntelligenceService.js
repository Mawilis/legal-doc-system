...
class ThreatDetectionEngine {
  ...
  async evaluateModel() {
    ...
    const accuracy = Array.isArray(evaluation)
      ? evaluation[1].dataSync()[0]
      : evaluation.dataSync()[0];
  ...
  async loadTrainingDataFromDatabase() {
    ...
    threats.forEach((threat) => {
      if (threat.mlPrediction?.features) {
        ...
        labels.push([
          threat.impact.severity === 'CRITICAL' || threat.impact.severity === 'HIGH' ? 1 : 0,
        ]);
      }
    });
  ...
  determineThreatType(features, prediction) {
    ...
    } else if (features[6] === 1.0) {
      // AUDIT_LOG access
      return 'INSIDER_THREAT';
    } else if (features[5] > 0.9) {
      // SUPREME_OVERRIDE
      return 'INSIDER_THREAT';
  ...
  async createThreatRecord(auditLog, mlAnalysis, threatIntelligence, riskAssessment) {
    const threatRecord = new ThreatIntelligence({
      ...
      compliance: {
        cybercrimesAct: {
          reportable:
            riskAssessment.threatLevel === 'CRITICAL' || riskAssessment.threatLevel === 'HIGH',
          reported: false,
        },
        popia: {
...
${
  threatRecord.integrity.blockchainAnchored
    ? `Blockchain Proof: ${threatRecord.integrity.blockchainTxHash}`
    : ''
}
...
`;
  ...
  assessPOPIACompliance(threats) {
    ...
    return {
      ...
      notificationRate:
        dataBreaches.length > 0 ? (notified.length / dataBreaches.length) * 100 : 100,
  ...
  async reportToSACERT(report) {
    try {
      ...
      const response = await fetch(
        THREAT_INTELLIGENCE_CONFIG.THREAT_FEEDS.SA_CERT.endpoint + '/report',
        {
          method: 'POST',
          headers: {
  ...
  async getThreatStatistics() {
          ...
          complianceStatus: [
            ...
            },
            {
              $group: {
                ...
                popiaNotified: {
                  $sum: {
                    $cond: [{ $eq: ['$compliance.popia.informationOfficerNotified', true] }, 1, 0],
                  },
                },
              },
            },
    ...
    return {
      ...
      threatFeeds: {
        active: Object.values(THREAT_INTELLIGENCE_CONFIG.THREAT_FEEDS).filter((f) => f.enabled)
          .length,
        totalThreats: this.threatFeeds.size,
      },
    };
