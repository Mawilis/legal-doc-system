...
describe('RetentionPolicy Model', function () {
  ...
  describe('Forensic Integrity', function () {
    ...
    it('should link versions with hash chain', async function () {
      ...
      const v2 = await v1.createNewVersion({ policyName: 'Updated Litigation Policy' }, testUserId);
  ...
  describe('Static Methods', function () {
    it('getActivePolicy should return active policy', async function () {
      ...
      const active = await RetentionPolicy.getActivePolicy(testTenantId, 'litigation');
    ...
    it('getExpiringPolicies should return policies needing review', async function () {
      ...
      expect(expiring.length).to.be.at.least(0);
    });
  });
  ...
  describe('Error Handling', function () {
    it('should handle duplicate policy names gracefully', async function () {
      ...
      // Create a policy with same data but different policyId (auto-generated)
      const policy2 = new RetentionPolicy(validPolicyData);

      try {
        await policy2.save();
        // If it doesn't throw, that's fine - policyId is unique so it should work
        expect(policy2.policyId).to.not.equal(policy1.policyId);
      } catch (error) {
