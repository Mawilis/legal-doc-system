...
describe('🧠 WILSY OS - Neural Predictive Engine', () => {
  ...
  before(() => {
    ...
    const historicalCases = [
      {
        input: { evidenceStrength: 0.9, precedentAlignment: 0.8, jurisdictionRisk: 0.2 },
        output: { winProbability: 0.95 },
      },
      {
        input: { evidenceStrength: 0.3, precedentAlignment: 0.4, jurisdictionRisk: 0.9 },
        output: { winProbability: 0.1 },
      },
      {
        input: { evidenceStrength: 0.7, precedentAlignment: 0.6, jurisdictionRisk: 0.5 },
        output: { winProbability: 0.65 },
      },
      {
        input: { evidenceStrength: 0.1, precedentAlignment: 0.2, jurisdictionRisk: 0.8 },
        output: { winProbability: 0.05 },
      },
      {
        input: { evidenceStrength: 0.8, precedentAlignment: 0.9, jurisdictionRisk: 0.1 },
        output: { winProbability: 0.9 },
      },
    ];
  ...
  it('should accurately predict a highly favorable case outcome', () => {
    ...
    const prediction = predictor.predict({
      ...
      precedentAlignment: 0.85,
      jurisdictionRisk: 0.15,
    });

    expect(prediction.winProbability).to.be.greaterThan(0.8);
  ...
  it('should accurately predict a high-risk, unfavorable case', () => {
    ...
    const prediction = predictor.predict({
      ...
      precedentAlignment: 0.3,
      jurisdictionRisk: 0.85,
    });

    expect(prediction.winProbability).to.be.lessThan(0.3);
  });
});
