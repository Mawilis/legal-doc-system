...
class FaceRecognitionProcessor {
  ...
  generateLivenessProof(imageBuffer, landmarks) {
    ...
    return {
      ...
      timestamp: new Date().toISOString(),
      landmarkHash: crypto.createHash('sha256').update(JSON.stringify(landmarks)).digest('hex'),
    };
  }
...
class BiometricManager {
  ...
  generateConsentSignature(consent, biometricType) {
    const data = `${consent.userId}:${biometricType}:${consent.purpose}:${consent.timestamp}:${
  ...
  exportForDSAR(userId, storedTemplate) {
    try {
      ...
      const exportData = {
        ...
        complianceStatement:
          'This biometric data export complies with POPIA Section 23 (Right of Access). Biometric templates are encrypted and only commitment hashes are provided to protect Special Personal Information.',
      };
  ...
  generateExportSignature(exportData) {
    const data = `${exportData.requestId}:${exportData.userId}:${exportData.exportedAt}:${
      process.env.DSAR_SIGNING_SECRET || 'wilsy-dsar-2026'
    }`;
...
function generateFICAReport(verificationResult, userId) {
  ...
  report.signature = crypto.createHash('sha512').update(JSON.stringify(report)).digest('hex');
