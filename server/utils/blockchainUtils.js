...
class QuantumBlock {
  ...
  _calculateMerkleRoot() {
    ...
    const hash2 = crypto
      .createHash('sha256')
      .update(this.timestamp + this.index)
      .digest('hex');
...
class QuantumBlockchain {
  ...
  addLegalOperation(operationData) {
    ...
    return {
      ...
      operationId: this.pendingOperations[this.pendingOperations.length - 1].operationId,
    };
  }
  ...
  _signProof(proof) {
    ...
    return crypto.createHmac('sha256', signatureKey).update(proofString).digest('hex');
  }
...
function runBlockchainTests() {
  ...
  const testOperation = createLegalOperation(
    'DOCUMENT_ACCESS',
    {
      documentId: 'doc_123',
      userId: 'user_456',
