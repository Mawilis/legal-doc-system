...
<body>
  ...
  registerTemplateHelpers() {
    ...
    Handlebars.registerHelper('percentage', (value) => `${(value * 100).toFixed(1)}%`);

    // Legal reference formatting
    Handlebars.registerHelper('legalRef', (section) => `Section ${section}`);
  }
  ...
  async generateCompaniesActReport(options = {}) {
    ...
    try {
      ...
      const report = {
        ...
        financialYear:
          options.financialYear || `${new Date().getFullYear() - 1}-${new Date().getFullYear()}`,
        generatedAt: new Date(),
        sections: reportSections,
        retentionCompliance,
      ...
      subsections: [
        {
          ...
          content:
            'Monthly compliance reviews, breach management oversight, staff training coordination',
        },
      ],
    });
    ...
    : 'NO'
      ...
      metrics: [
        { label: 'Breaches Resolved', value: data.breachData.resolutionStatus.resolved },
        {
          ...
          value: `${data.breachData.resolutionStatus.averageResolutionHours} hours`,
        },
        {
          label: 'Regulator Notifications',
          value: data.breachData.notificationStatus.regulatorNotified,
        },
      ],
    });
      ...
      subsections: [
        {
          title: 'Key Strengths',
          content:
            '• Complete security safeguard implementation<br>• Timely breach notifications<br>• High consent validity rate',
        },
        {
          title: 'Areas for Improvement',
          content:
            '• Reduce DSAR response times<br>• Increase explicit consent rate<br>• Complete Information Officer registration',
        },
      ],
    });
  ...
  generateGapAnalysis(complianceScore, data) {
    ...
    return `The following compliance gaps have been identified:<br><ul>${gaps
      .map((gap) => `<li>${gap}</li>`)
      .join('')}</ul>`;
  }
  ...
  async exportToPDF(report, options = {}) {
    ...
    try {
      ...
      doc
        .fontSize(this.config.formats.pdf.fontSize)
      ...
      report.sections.forEach((section, index) => {
        ...
        if (section.content) {
          doc.fontSize(10).text(this.stripHtml(section.content), { align: 'left' }).moveDown();
        }
      });
