...
async function decryptField(encryptedString, keyType = 'field') {
  try {
    ...
    if (encryptedData.keyType && encryptedData.keyType !== keyType) {
      ...
      key = keyType === 'data' ? keyVault.getDataEncryptionKey() : keyVault.getFieldEncryptionKey();
    }
...
async function findDecryptionKey(keyType, timestamp) {
  ...
  // Use current key
  return keyType === 'data' ? keyVault.getDataEncryptionKey() : keyVault.getFieldEncryptionKey();
}
