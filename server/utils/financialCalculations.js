...
function calculateAfricanLegalTechLTVAdjustment() {
  ...
  const adjustments = {
    ...
    KES: 1.2, // Kenya: Strong mobile adoption
...
function calculateMonthlyChurnBreakdown(churnData, periodDays) {
  ...
  return breakdown.map((month) => ({
    ...
    averageRevenuePerChurn:
      month.churnCount > 0 ? financialRound(month.lostRevenue / month.churnCount) : 0,
    churnRate:
      month.churnCount > 0
        ? financialRound(
          month.churnCount
...
function calculateSegmentedRecoveryRates(churnData) {
  const recoveryRates = {
    PRICING: 0.4, // Price-sensitive customers - high recovery potential
    FEATURES: 0.25, // Feature missing - moderate recovery
    SUPPORT: 0.35, // Support issues - good recovery
    PERFORMANCE: 0.2, // Performance issues - moderate recovery
    OTHER: 0.15, // Other reasons - low recovery
    UNKNOWN: 0.1, // Unknown reasons - very low recovery
  };
...
function calculateParetoAnalysis(churnReasons) {
  ...
  return {
    paretoPrinciple:
      topReasons.length <= Math.ceil(churnReasons.length * 0.2) ? 'CONFIRMED' : 'NOT_CONFIRMED',
    topReasons,
...
function generateSegmentRecoveryActions(segment) {
  ...
  return (
    actionTemplates[segment.reason] || {
