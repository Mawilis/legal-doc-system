...
if (!process.env.REDIS_URL) {
  logger.error('QUANTUM BREACH: REDIS_URL missing from .env');
  ...
  async initRedis() {
    try {
      this.client = redis.createClient({
        ...
        socket: {
          ...
          reconnectStrategy: (retries) => {
            if (retries > 10) {
              logger.error('QUANTUM CACHE: Redis connection failed after 10 retries');
      ...
      this.client.on('error', (err) => {
        logger.error('QUANTUM CACHE ERROR:', err);
        this.ready = false;
      });

      this.client.on('connect', () => {
        logger.info('QUANTUM CACHE: Redis connected successfully');
        this.ready = true;
      });
      ...
      logger.error('QUANTUM CACHE: Failed to initialize Redis:', error);
      this.ready = false;
    }
  }
  ...
  async get(key) {
    if (this.ready && this.client) {
      try {
        ...
        logger.warn('QUANTUM CACHE: Redis get failed, falling back to memory:', error);
  ...
  async set(key, value, ttlSeconds = 300) {
    ...
    if (this.ready && this.client) {
      try {
        ...
        logger.warn('QUANTUM CACHE: Redis set failed, falling back to memory:', error);
  ...
  async delete(key) {
    if (this.ready && this.client) {
      try {
        ...
        logger.warn('QUANTUM CACHE: Redis delete failed:', error);
      }
    }
  ...
  async increment(key, ttlSeconds = 60) {
    if (this.ready && this.client) {
      try {
        ...
        logger.warn('QUANTUM CACHE: Redis increment failed:', error);
      }
    }
  ...
  async clearPattern(pattern) {
    if (this.ready && this.client) {
      try {
        ...
        logger.error('QUANTUM CACHE: Clear pattern failed:', error);
      }
    }
