...
class QuantumKeyManager {
  ...
  decryptWithMasterKey(encryptedData) {
    ...
    return Buffer.concat([decipher.update(Buffer.from(data.encrypted, 'hex')), decipher.final()]);
  }
  ...
  logKeyEvent(eventData) {
    ...
    if (process.env.NODE_ENV === 'production') {
      logger.info('KEY_AUDIT_LOG:', JSON.stringify(auditLog));
    } else {
      logger.info('KEY_EVENT:', auditLog);
    }
  }
}
...
class QuantumEncryptionEngine {
  ...
  calculateIntegrityHash(encryptedPackage) {
    const hmac = crypto.createHmac('sha256', Buffer.from(process.env.HMAC_SIGNING_KEY, 'hex'));
...
const decryptField = (encryptedValue) => {
  ...
  try {
    ...
    logger.error('Field decryption failed:', error.message);
