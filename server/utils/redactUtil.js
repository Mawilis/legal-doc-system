...
function redactString(text, context, replacement) {
  ...
  if (context === 'paia') {
    ...
    redacted = redacted.replace(/(?:\[.*?\]\s*)?without prejudice[\s\S]*/gi, '[LEGAL PRIVILEGE - Without Prejudice]');
    redacted = redacted.replace(/(?:\[.*?\]\s*)?legal opinion[\s\S]*/gi, '[LEGAL PRIVILEGE - Legal Opinion]');
    redacted = redacted.replace(/(?:\[.*?\]\s*)?counsel advises[\s\S]*/gi, '[LEGAL PRIVILEGE - Counsel Advice]');
  } 
...
function redactObject(obj, context, replacement) {
  ...
  const sensitiveFields = [
    'idNumber', 'identityNumber', 'passportNumber',
    'email', 'phone', 'cellphone', 'mobile',
    'bankAccount', 'accountNumber', 'membershipNumber',
    'salary', 'profit', 'revenue', 'ebitda'
  ];
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    const shouldPreserve = context === 'commercial' && 
                           preserveInCommercial.some(field => keyLower.includes(field));
...
export function redactWithAudit(data, context = 'general') {
  ...
  const countRedactions = (obj) => {
    if (typeof obj === 'string') {
      const matches = (obj.match(/\[REDACTED\]/g) || []).length +
