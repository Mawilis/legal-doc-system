...
class QuantumReportGenerator {
  ...
  generateDigitalSignature(reportData) {
    try {
      ...
      const dataToSign = Buffer.concat([reportData, Buffer.from(timestamp)]);
...
`;
    ...
    doc.fontSize(9).fillColor('#333333').text(disclaimerText, 50, 130, {
      align: 'left',
      width: 500,
      lineGap: 4,
    });
  ...
  _addExecutiveSummary(doc, data, reportType) {
    ...
    doc
      .text('Overview:', 50, doc.y, { underline: true })
      .moveDown(0.5)
  ...
  _addDetailedFindings(doc, data, reportType) {
    ...
    data.findings.forEach((finding, index) => {
      ...
      if (finding.description) {
        doc
          .text(finding.description, 60, doc.y, {
            width: 480,
            lineGap: 3,
          })
          .moveDown(0.5);
  ...
  _addComplianceMatrix(doc, matrix, framework) {
    ...
    matrix.forEach((row, rowIndex) => {
      ...
      columns.forEach((cell, i) => {
        ...
        doc.fillColor('#000000').font('Helvetica').text(cell, x, currentY, {
          width: columnWidths[i],
          align: 'left',
        });
      });
  ...
  _addRecommendations(doc, recommendations) {
    ...
    recommendations.forEach((rec, index) => {
      ...
      if (rec.responsible) {
        doc.text(`Responsible: ${rec.responsible}`, 60, doc.y).moveDown(0.5);
      }
  ...
  _addPDFFooter(doc, data, reportType) {
    ...
    const signatureData = this.generateDigitalSignature(Buffer.from(JSON.stringify(data)));
...
`;
    ...
    doc.fontSize(10).fillColor('#000000').font('Helvetica').text(signatureText, 50, 100, {
      width: 500,
      lineGap: 4,
    });
  }
  ...
  async generatePOPIAReport(auditData, format = 'pdf') {
    const reportId = `POPIA-${new Date().toISOString().split('T')[0]}-${crypto
      .randomBytes(4)
      .toString('hex')}`;
  ...
  async generateCompaniesActReport(retentionData, format = 'pdf') {
    const reportId = `COMPANIES-ACT-${new Date().toISOString().split('T')[0]}-${crypto
      .randomBytes(4)
      .toString('hex')}`;
  ...
  async generateMultiFormatReport(data, reportType, formats = ['pdf', 'excel']) {
    ...
    try {
      ...
      for (const format of formats) {
        ...
        switch (format) {
          case 'pdf':
            buffer = await this.generatePDFReport(data, reportType, {
              reportId: `${reportId}-pdf`,
            });
            filename = `${reportType}_Report_${reportId}.pdf`;
            break;
          case 'excel':
            buffer = await this.generateExcelReport(data, reportType, {
              reportId: `${reportId}-excel`,
            });
