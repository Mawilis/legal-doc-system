...
class SecurityAlert {
    ...
    generateAlertId() {
        const hash = crypto.createHash('sha256')
            .update(`${this.type}-${this.timestamp.toISOString()}-${this.requestId}-${crypto.randomBytes(8).toString('hex')}`)
            .digest('hex')
            .substring(0, 16);
    ...
    determineSeverity() {
        ...
        if (this.metadata.affectedDataSubjects > 0 ||
    ...
    async sendNotifications() {
        ...
        for (const channel of channels) {
            try {
                switch (channel) {
                    case 'log':
                        logger[this.severity === SECURITY_ALERT_LEVELS.INFO ? 'info' :
                            this.severity === SECURITY_ALERT_LEVELS.WARNING ? 'warn' : 'error'](
...
export const triggerCriticalAlert = async (type, metadata, requestId) => {
    return logSecurityAlert(
        type,
        { ...metadata, forceCritical: true },
        requestId
    );
};
...
export const requiresBreachNotification = (type, metadata) => {
    ...
    return breachTypes.includes(type) &&
