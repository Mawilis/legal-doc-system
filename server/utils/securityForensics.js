...
class SecurityForensicsEngine {
    ...
    detectPatterns(event) {
        ...
        if (hour < this.PATTERNS.UNUSUAL_HOURS.START_HOUR || 
        ...
        if (event.dataVolume && event.dataVolume > 1000000) { // > 1MB
            patterns.push({
    ...
    generateCaseReport(caseId) {
        ...
        const analyses = Array.from(this.forensicStore.values())
            .filter(item => item.analysis.eventId === caseId);
        
        const report = {
            reportId: crypto.createHash(this.HASH_ALGORITHM)
        ...
        report.forensicHash = crypto.createHash(this.HASH_ALGORITHM)
            .update(canonicalData)
            .digest(this.HASH_ENCODING);
    ...
    summarizeEvents(events) {
        ...
        summary.averageRiskScore = events.length > 0 
            ? Math.round((totalRiskScore / events.length) * 10) / 10 
            : 0;
    ...
    verifyEvidenceIntegrity(evidenceId) {
        ...
        const calculatedHash = crypto.createHash(this.HASH_ALGORITHM)
            .update(canonicalData)
            .digest(this.HASH_ENCODING);
    ...
    getStatistics() {
        ...
        return {
            ...
            averageRiskScore: analyses.length > 0
                ? Math.round((analyses.reduce((sum, a) => sum + (a.riskScore || 0), 0) / analyses.length) * 10) / 10
                : 0,
