...
if (!process.env.MIN_AGREEMENT_TERMS_LENGTH) {
  logger.warn('QUANTUM WARNING: MIN_AGREEMENT_TERMS_LENGTH not set, using default 50');
}
if (!process.env.MAX_AGREEMENT_PARTIES) {
  logger.warn('QUANTUM WARNING: MAX_AGREEMENT_PARTIES not set, using default 10');
}
...
const zarCurrencySchema = Joi.number()
  ...
  .custom((value, helpers) => {
    ...
    if (cents === 0 && value > 10000) {
      logger.warn('QUANTUM ALERT: Suspicious round number detected');
    }
...
const partySchema = Joi.object({
  ...
  partyId: ObjectId().required().messages({ 'any.required': 'Party ID is required' }),

  partyType: Joi.string()
    .valid('individual', 'company', 'trust', 'partnership', 'cc', 'close_corporation', 'ngo')
    .required()
    .messages({
      'any.required': 'Party type is required',
      'any.only':
        'Party type must be one of: individual, company, trust, partnership, cc, close_corporation, ngo',
    }),
  ...
  legalCapacity: Joi.object({
    capacityType: Joi.string()
      .valid('director', 'owner', 'authorized_representative', 'trustee', 'member')
      .required(),
...
const createAgreementSchema = Joi.object({
  ...
  parties: Joi.array()
    ...
    .custom((parties, helpers) => {
      ...
      for (const party of parties) {
        ...
        if (registrationNumbers.has(party.registrationNumber)) {
          return helpers.error('any.invalid', {
            message: 'Duplicate registration number detected',
          });
        }
      ...
      if (!hasSAResident) {
        logger.warn('QUANTUM WARNING: No South African resident party detected');
      }
    ...
    .messages({
      ...
      'array.max': `Agreement cannot have more than ${
        process.env.MAX_AGREEMENT_PARTIES || 10
      } parties`,
  ...
  description: Joi.string()
    ...
    .required()
    .messages({
      'string.min': `Description must be at least ${
        process.env.MIN_AGREEMENT_TERMS_LENGTH || 50
      } characters`,
  ...
  compliance: Joi.object({
    ...
    language: Joi.string()
      .valid(...SA_LEGAL_CONSTANTS.REQUIRED_LANGUAGES)
      .default('en'),
  ...
  financialTerms: Joi.object({
    ...
    paymentMethod: Joi.string()
      .valid('eft', 'credit_card', 'cash', 'debit_order', 'payfast')
      .default('eft'),
  }).required(),
  ...
  security: Joi.object({
    encryptionLevel: Joi.string()
      .valid('AES-128', 'AES-256-GCM', 'quantum-resistant')
      .default('AES-256-GCM'),
  ...
  metadata: Joi.object({
    ...
    confidentialityLevel: Joi.string()
      .valid('public', 'internal', 'confidential', 'secret')
  ...
  terms: Joi.string()
    ...
    .custom((terms, helpers) => {
      if (terms) {
        ...
        logger.info(`QUANTUM AUDIT: Terms updated with hash: ${changeHash}`);
      }
      return terms;
    }),
...
const validateCreateAgreement = (req, res, next) => {
  ...
  logger.info(`QUANTUM VALIDATION: Agreement creation validated for ${value.agreementReference}`);
...
const validateUpdateAgreement = (req, res, next) => {
  ...
  logger.info(`QUANTUM VALIDATION: Agreement ${req.params.agreementId} update validated`);
...
const validateSearchAgreement = (req, res, next) => {
  ...
  logger.info(`QUANTUM SEARCH: Validated search with token ${value.searchToken}`);
...
const generateAgreementChecksum = (agreementData) => {
  ...
  logger.info(`QUANTUM CHECKSUM: Generated for agreement ${agreementData.agreementReference}`);
