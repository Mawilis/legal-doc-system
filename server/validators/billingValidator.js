...
class BillingValidator {
  ...
  initializeSchemas() {
    return {
      ...
      invoice: Joi.object({
        ...
        vatInclusive: Joi.boolean().required().messages({
      ...
      payment: Joi.object({
        paymentReference: Joi.string()
          ...
          .messages({
            'string.pattern.base':
              'Payment reference must follow format: PAY-XX-YYYY-NNNNNN-XXXXXX',
          }),
        ...
        bankDetails: Joi.object({
          ...
          branchCode: Joi.string().when('paymentMethod', {
            is: 'electronic_funds_transfer',
            then: Joi.string().pattern(REGEX_PATTERNS.ZA_BRANCH_CODE).required(),
            otherwise: Joi.string().optional(),
          }),

          accountHolder: Joi.string().when('paymentMethod', {
            is: 'electronic_funds_transfer',
        ...
        cardDetails: Joi.object({
          lastFour: Joi.string().when('paymentMethod', {
            is: 'credit_card',
            then: Joi.string()
              .pattern(/^[0-9]{4}$/)
              .required(),
          ...
          cardType: Joi.string().when('paymentMethod', {
            is: 'credit_card',
        ...
        includesVAT: Joi.boolean().required(),

        vatAmount: Joi.number().when('includesVAT', {
          is: true,
        ...
        // AUDIT QUANTUM
        payerIp: Joi.string().required().ip(),

        userAgent: Joi.string().required().max(500),

        // SECURITY QUANTUM
        fraudScore: Joi.number().min(0).max(100).default(0),
      ...
      subscription: Joi.object({
        ...
        taxExemptionCertificate: Joi.string().when('taxExempt', {
          is: true,
        ...
        signedDate: Joi.date().required().max('now'),
      ...
      creditNote: Joi.object({
        ...
        taxAdjustment: Joi.boolean().required(),

        adjustedVAT: Joi.number().when('taxAdjustment', {
          is: true,
        ...
        approvalDate: Joi.date().required().max('now'),
      }),
    };
  }
  ...
  async validateInvoice(data) {
    try {
      ...
      const schemaValidation = await this.schemas.invoice.validateAsync(sanitizedData, {
        abortEarly: false,
        stripUnknown: true,
  ...
  async validatePayment(data) {
    try {
      ...
      const schemaValidation = await this.schemas.payment.validateAsync(sanitizedData, {
        abortEarly: false,
        stripUnknown: true,
      });
  ...
  async validateSubscription(data) {
    try {
      ...
      const schemaValidation = await this.schemas.subscription.validateAsync(sanitizedData, {
        abortEarly: false,
        stripUnknown: true,
      });
  ...
  async validateCreditNote(data) {
    try {
      ...
      const schemaValidation = await this.schemas.creditNote.validateAsync(sanitizedData, {
        abortEarly: false,
        stripUnknown: true,
      });
      ...
      logger.info(
        `Credit note validation successful for invoice: ${schemaValidation.originalInvoiceId}`
      );
  ...
  validateVATCalculation(value, helpers) {
    ...
    if (Math.abs(value - expectedVAT) > tolerance) {
      return helpers.error('any.custom', {
        message: `VAT amount mismatch. Expected approximately ${expectedVAT.toFixed(
          2
        )} based on rate ${vatRate}`,
      });
    }
  ...
  async validateInvoiceBusinessLogic(invoiceData) {
    ...
    if (Math.abs(lineItemsTotal - invoiceData.amount) > tolerance) {
      throw new Error(
        `Line items total (${lineItemsTotal}) doesn't match invoice amount (${invoiceData.amount})`
      );
    }
    ...
    if (invoiceData.vatAmount > 0) {
      ...
      if (!vatValid) {
        throw new Error(
          `Invalid VAT number: ${invoiceData.taxReferenceNumber} for country ${invoiceData.countryCode}`
        );
      }
    }
  }
    ...
    return {
      ...
      reason:
        fraudIndicators.length > 0
          ? `Fraud indicators detected: ${fraudIndicators.join(', ')}`
          : null,
    };
  }
  ...
  async performAMLCheck(paymentData) {
    ...
    const level =
      score >= 30 ? 'enhanced_due_diligence' : score >= 15 ? 'standard_due_diligence' : 'low_risk';
