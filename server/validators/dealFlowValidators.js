...
const DEAL_TYPES = ['acquisition', 'merger', 'joint_venture', 'strategic_investment', 'divestiture', 'spin_off', 'takeover'];
const DEAL_STAGES = ['identification', 'screening', 'initial_contact', 'nda', 'preliminary_dd', 'indicative_offer', 'confirmatory_dd', 'final_agreement', 'regulatory_approval', 'shareholder_approval', 'closing', 'integration', 'completed', 'withdrawn'];
const CURRENCIES = ['ZAR', 'USD', 'EUR', 'GBP'];
const JURISDICTIONS = ['ZA', 'NA', 'BW', 'KE', 'NG', 'GB', 'EU', 'US', 'CN', 'IN'];
const INDUSTRIES = ['technology', 'financial', 'healthcare', 'manufacturing', 'retail', 'legal', 'mining', 'energy', 'telecom', 'agriculture', 'transport', 'construction'];
...
export const targetIdentificationSchema = Joi.object({
  ...
  minRevenue: Joi.number().min(0).when('maxRevenue', {
    is: Joi.exist(),
  ...
  targetIndustries: Joi.array().items(
    Joi.string().valid(...INDUSTRIES)
  ).min(1).max(10),
  
  relatedIndustries: Joi.array().items(
    Joi.string().valid(...INDUSTRIES)
  ),
  
  preferredRegions: Joi.array().items(
    Joi.string().valid(...JURISDICTIONS)
  ),
...
}).min(1).message('At least one search criterion must be provided');
...
export const dealCreationSchema = Joi.object({
  acquirerId: Joi.string().pattern(/^[0-9a-fA-F]{24}$/).required()
    .messages({
  ...
  dealType: Joi.string().valid(...DEAL_TYPES).required()
    .messages({
  ...
  currency: Joi.string().valid(...CURRENCIES).default('ZAR'),
  
  jurisdiction: Joi.string().valid(...JURISDICTIONS).default('ZA'),
  ...
  team: Joi.array().items(
    Joi.object({
      ...
      role: Joi.string().valid('lead', 'financial', 'legal', 'tax', 'technical', 'advisor').required()
    })
  ),
