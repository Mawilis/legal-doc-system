...
const consentSchema = Joi.object({
  ...
  processingActivities: Joi.array().optional().items(Joi.string().max(100)).max(30),
  ...
  metadata: Joi.object({
    ...
    language: Joi.string()
      .valid('en', 'zu', 'xh', 'af', 'st', 'tn', 'ts', 'ss', 've', 'nr')
      .default('en'),
    accessibility: Joi.string()
      .valid('STANDARD', 'HIGH_CONTRAST', 'SCREEN_READER', 'LARGE_TEXT')
      .default('STANDARD'),
  }).optional(),
...
const updateSchema = Joi.object({
  status: Joi.string().valid('ACTIVE', 'SUSPENDED', 'REVOKED').optional(),
...
function sanitizeConsentData(data) {
  ...
  if (sanitized.purposes) {
    sanitized.purposes = sanitized.purposes.map((purpose) => purpose.trim().replace(/[<>]/g, ''));
  }
