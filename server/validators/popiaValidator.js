...
try {
  ...
  redisClient.connect().then(() => logger.info('✅ Redis connected for POPIA validation caching'));
} catch (error) {
  logger.warn('⚠️ Redis not available for caching. Install: npm install redis@^4.6.0');
}
...
try {
  ...
  auditLogger = {
    logSecurityEvent: (data) => logger.info('🔒 Security Event:', data),
    logComplianceEvent: (data) => logger.info('📋 Compliance Event:', data),
  };
  logger.warn('⚠️ Using console fallback for audit logging');
}
...
try {
  ...
  logger.warn('⚠️ Using basic identity validation fallback');
}
...
const validatePOPIAEnvironment = () => {
  logger.info('🔍 Validating POPIA environment with forensic .env history...');
  ...
  if (errors.length > 0) {
    logger.error('❌ POPIA Environment validation failed:');
    errors.forEach((error) => logger.error(`   - ${error}`));
    logger.error('\n💡 SOLUTION: Update /server/.env with these POPIA variables:');
    Object.entries(requiredPOPIAVars).forEach(([varName, config]) => {
      logger.error(`   ${varName}=your_${varName.toLowerCase()}_value # ${config.description}`);
    });
    throw new Error('POPIA Environment validation failed. Check .env configuration.');
  }

  if (warnings.length > 0) {
    logger.warn('⚠️ POPIA Environment warnings:');
    warnings.forEach((warning) => logger.warn(`   - ${warning}`));
  }

  logger.info('✅ POPIA Environment validation passed');
  return true;
};
...
class QuantumPOPIAValidator {
  constructor() {
    ...
    logger.info('🚀 Quantum POPIA Validator Initialized for South African Legal Market');
    logger.info(`   Minimum Age: ${process.env.POPIA_MINIMUM_AGE}`);
    logger.info(`   Information Officer: ${process.env.POPIA_INFO_OFFICER_EMAIL}`);
    logger.info(`   Breach Notification: ${process.env.POPIA_BREACH_NOTIFICATION_HOURS} hours`);
  }
  ...
  async setupCaching() {
    ...
    try {
      ...
      logger.info('✅ Redis caching configured for POPIA validation');
    } catch (error) {
      logger.warn('⚠️ Redis caching setup failed:', error.message);
    }
  }
  ...
  initializeSchemas() {
    return {
      ...
      consent: Joi.object({
        ...
        consentId: Joi.string().required().pattern(SA_LEGAL_PATTERNS.CONSENT_ID).messages({
        ...
        consentType: Joi.string()
          .when('lawfulCondition', {
            is: 'consent',
            then: Joi.string()
              .valid('explicit', 'informed', 'specific', 'freely_given', 'unambiguous')
              .required(),
        ...
        processingPurpose: Joi.string()
          ...
          .messages({
            'string.min':
              'Processing purpose must be specific and detailed (minimum 20 characters)',
        ...
        dataCategories: Joi.array()
          .items(
            Joi.object({
              category: Joi.string().required().pattern(SA_LEGAL_PATTERNS.DATA_CATEGORY).messages({
          ...
          .messages({
            ...
            'array.max': `Maximum ${
              process.env.MAX_DATA_CATEGORIES || '20'
            } data categories allowed per processing activity`,
          }),

        // Special personal information (Sections 26-33)
        containsSpecialInfo: Joi.boolean().required().messages({
        ...
        specialInfoCategories: Joi.array()
          ...
          .messages({
            'array.min':
              'Special information categories must be specified when processing special personal information',
          }),
        ...
        specialInfoJustification: Joi.string()
          ...
          .messages({
            ...
            'any.required':
              'Justification required for processing special personal information per POPIA Section 27',
          }),

        // Cross-border transfers (Section 72)
        crossBorderTransfer: Joi.boolean().required(),
        ...
        automatedDecisions: Joi.boolean().required(),
        ...
        autoDecisionLogic: Joi.string()
          ...
          .messages({
            ...
            'any.required':
              'Decision logic required for automated decision-making per POPIA Section 71',
          }),
        ...
        optOutMechanism: Joi.string()
          .when('directMarketing', {
            is: true,
            then: Joi.string()
              .valid('email', 'sms', 'phone', 'post', 'in_app', 'web_portal')
              .required(),
            otherwise: Joi.string().optional(),
          })
          .messages({
        ...
        informationOfficerId: Joi.string()
          ...
          .messages({
            'string.pattern.base':
              'Information Officer registration number must follow format: IO-999999-XX',
        ...
        securityMeasures: Joi.array()
          ...
          .messages({
            'array.min': `At least ${
              process.env.MIN_SECURITY_MEASURES || '3'
            } security measures must be specified per POPIA Section 19`,
          }),
      ...
      dataSubjectRequest: Joi.object({
        requestId: Joi.string().required().pattern(SA_LEGAL_PATTERNS.DSR_ID).messages({
        ...
        requestType: Joi.string()
          ...
          .messages({
            'any.only':
              'Invalid request type - must be one of: access, correction, deletion, objection, complaint, restriction, portability',
          }),
        ...
        verificationData: Joi.object({
          ...
          idNumber: Joi.string()
            ...
            .when('idType', {
              is: 'sa_id',
              then: Joi.string()
                .pattern(SA_LEGAL_PATTERNS.SA_ID)
          ...
          verificationPhoto: Joi.string().optional().uri().messages({
        ...
        timestamp: Joi.date().required().max('now'),

        expectedCompletion: Joi.date()
          .required()
          .min(Joi.ref('timestamp'))
          .max(Joi.ref('timestamp'))
          .add(30, 'days')
          .messages({
            'date.max':
              'Expected completion cannot exceed 30 days from request date per POPIA Section 25',
          }),
      ...
      dataBreachNotification: Joi.object({
        breachId: Joi.string().required().pattern(SA_LEGAL_PATTERNS.BREACH_ID).messages({
        ...
        detectionTime: Joi.date().required().max('now'),

        occurrenceTime: Joi.date().required().max(Joi.ref('detectionTime')),
        ...
        affectedRecords: Joi.number().required().min(1).max(10000000),

        affectedDataSubjects: Joi.number().required().min(1).max(10000000),

        // Special information breach
        specialInfoBreached: Joi.boolean().required(),
        ...
        notifyRegulator: Joi.boolean().required(),

        notifySubjects: Joi.boolean().required(),
        ...
        notificationDeadline: Joi.date()
          ...
          .add(parseInt(process.env.POPIA_BREACH_NOTIFICATION_HOURS), 'hours')
          .messages({
        ...
        ioNotified: Joi.boolean().required(),

        ioNotificationTime: Joi.date().when('ioNotified', {
          is: true,
        ...
        // Legal reporting
        regulatorReference: Joi.string().when('notifyRegulator', {
          is: true,
        ...
        // Impact assessment
        financialImpact: Joi.number().optional().min(0).max(1000000000),
      ...
      crossBorderTransfer: Joi.object({
        transferId: Joi.string().required().pattern(SA_LEGAL_PATTERNS.CBTA_CODE).messages({
          'string.pattern.base': 'Transfer ID must follow format: CBTA-XX-YYYYMMDD',
        }),

        // Countries involved
        sourceCountry: Joi.string().required().valid('ZA'),
        ...
        adequacyAssessment: Joi.object({
          countryAdequate: Joi.boolean().required(),

          adequacyBasis: Joi.string().when('countryAdequate', {
            is: false,
            then: Joi.string()
              .valid('appropriate_safeguards', 'derogation', 'binding_corporate_rules')
              .required(),
            otherwise: Joi.string().optional(),
          }),
          ...
          derogationReason: Joi.string().when('adequacyBasis', {
            is: 'derogation',
        ...
        thirdPartyProcessor: Joi.object({
          ...
          contractInPlace: Joi.boolean().required(),

          contractId: Joi.string().when('contractInPlace', {
            is: true,
            then: Joi.string().required(),
            otherwise: Joi.string().optional(),
          }),

          dataProcessingAgreement: Joi.boolean().required().valid(true).messages({
            'any.only':
              'Data Processing Agreement required for cross-border transfers per POPIA Section 72',
          }),
        ...
        ioApproval: Joi.boolean().required().valid(true).messages({
          'any.only': 'Information Officer approval required for cross-border transfers',
        }),

        ioApprovalDate: Joi.date().required().max('now'),

        // Data subject notification
        subjectsNotified: Joi.boolean().required(),

        notificationMethod: Joi.string().when('subjectsNotified', {
          is: true,
          then: Joi.string().required(),
          otherwise: Joi.string().optional(),
        }),
      ...
      retentionPolicy: Joi.object({
        policyId: Joi.string().required().pattern(SA_LEGAL_PATTERNS.RETENTION_CODE).messages({
          'string.pattern.base': 'Policy ID must follow format: RET-XX-999Y',
        }),

        dataCategory: Joi.string().required().pattern(SA_LEGAL_PATTERNS.DATA_CATEGORY),
        ...
        retentionUnit: Joi.string().required().valid('years', 'months', 'days'),
        ...
        lastReviewed: Joi.date().required().max('now'),

        nextReview: Joi.date().required().greater(Joi.ref('lastReviewed')),

        // Legal holds
        legalHold: Joi.boolean().required(),

        legalHoldReference: Joi.string().when('legalHold', {
          is: true,
          then: Joi.string().required(),
          otherwise: Joi.string().optional(),
        }),
  ...
  async validateConsent(data) {
    ...
    try {
      ...
      logger.error('❌ POPIA Consent validation failed:', validationError.message);
  ...
  async validateDataSubjectRequest(data) {
    ...
    try {
      ...
      logger.info(`✅ DSR validated: ${schemaValidation.requestId}, Priority: ${priority}`);
      return result;
    } catch (error) {
      this.validationStats.failedValidations++;
      const validationError = this.formatPOPIAValidationError(error);
  ...
  async validateDataBreachNotification(data) {
    ...
    try {
      ...
      const schemaValidation = await this.schemas.dataBreachNotification.validateAsync(
        sanitizedData,
        {
          abortEarly: false,
          stripUnknown: true,
  ...
  async validateCrossBorderTransfer(data) {
    ...
    try {
      ...
      logger.info(`✅ Cross-border transfer validated: ${schemaValidation.transferId}`);
      return result;
    } catch (error) {
      this.validationStats.failedValidations++;
      const validationError = this.formatPOPIAValidationError(error);
  ...
  async validateRetentionPolicy(data) {
    ...
    try {
      ...
      logger.info(`✅ Retention policy validated: ${schemaValidation.policyId}`);
      return result;
    } catch (error) {
      this.validationStats.failedValidations++;
      const validationError = this.formatPOPIAValidationError(error);
  ...
  validateAuthorizedDestinations(value, helpers) {
    const authorizedCountries = process.env.POPIA_CROSS_BORDER_AUTHORIZED
      ? process.env.POPIA_CROSS_BORDER_AUTHORIZED.split(',')
      : ['ZA'];
  ...
  validateConsentBusinessRules(value, helpers) {
    ...
    if (value.lawfulCondition === 'consent' && value.processingPurpose.length < 50) {
      return helpers.error('any.custom', {
        message:
          'Processing purpose must be specific (minimum 50 characters) when using consent as lawful condition',
      });
    }
    ...
    if (value.containsSpecialInfo && value.lawfulCondition !== 'consent') {
      return helpers.error('any.custom', {
        message:
          'Special personal information requires explicit consent as lawful condition per POPIA Section 27',
      });
    }
  ...
  validateTransferBusinessRules(value, helpers) {
    ...
    if (
      ...
      return helpers.error('any.custom', {
        message:
          'Cross-border transfers require adequacy determination, appropriate safeguards, or valid derogation',
      });
    }
    ...
    if (!value.thirdPartyProcessor.dataProcessingAgreement) {
      return helpers.error('any.custom', {
        message:
          'Third-party processor must have Data Processing Agreement for cross-border transfers',
      });
    }
      ...
      case 'legitimate_interests':
        if (data.automatedDecisions) {
          return {
            valid: false,
            violation:
              'Legitimate interests cannot be used for automated decisions with legal effect',
          };
        }
        break;
    }
  ...
  async logPOPIAValidation(entityType, data, result, error) {
    ...
    try {
      ...
      logger.warn('⚠️ Failed to log POPIA validation:', logError.message);
    }
  }
  ...
  async healthCheck() {
    ...
    try {
      const testData = {
        ...
        consentStatement:
          'I explicitly consent to the processing of my personal data for the specified purposes.',
