...
function startHealthServer(_port) {
...
async function gracefulShutdown(workers = [], timeoutMs = DEFAULTS.shutdownTimeoutMs) {
  ...
  const closePromises = workers.map(async (_w) => {
    if (!w) return;
    try {
      ...
      if (typeof w.close === 'function') {
        ...
        await Promise.race([
          ...
          setTimeoutAsync(timeoutMs).then(() => {
            throw new Error('worker close timeout');
          }),
        ]);
        logger.info('worker-bootstrap: worker closed', {
          worker: w.name || w.queueName || 'unknown',
        });
  ...
  const jobServiceClose = (async () => {
    try {
      await Promise.race([
        JobService.shutdown(),
        setTimeoutAsync(timeoutMs).then(() => {
          throw new Error('JobService shutdown timeout');
        }),
      ]);
      logger.info('worker-bootstrap: JobService shutdown complete');
    } catch (_e) {
      logger.warn('worker-bootstrap: JobService shutdown failed', {
...
async function start() {
  ...
  try {
    ...
    if (fileScanHandle) {
      ...
      logger.info('worker-bootstrap: fileScan worker registered', {
        concurrency: DEFAULTS.fileScanConcurrency,
      });
    } else {
      logger.warn(
    ...
    if (metadataWorker && (metadataWorker.register || metadataWorker.registerWithJobService)) {
      ...
      if (metadataHandle) {
        ...
        logger.info('worker-bootstrap: metadata worker registered', {
          concurrency: DEFAULTS.metadataConcurrency,
        });
      }
    }
    ...
    const shutdownHandler = async (_signal) => {
      ...
      try {
        ...
        logger.error('worker-bootstrap: gracefulShutdown failed', {
          err: e && e.message ? e.message : e,
        });
        process.exit(1);
      }
    ...
    process.on('uncaughtException', async (_err) => {
      logger.error('worker-bootstrap: uncaughtException', {
    ...
    process.on('unhandledRejection', async (_reason) => {
      logger.error('worker-bootstrap: unhandledRejection', {
    ...
    logger.info('worker-bootstrap: all workers registered and running', {
      registeredCount: registeredWorkers.length,
    });
  } catch (_err) {
    logger.error('worker-bootstrap: startup failed', {
...
if (require.main === module) {
  start().catch((_err) => {
    logger.error('worker-bootstrap: fatal error on start', {
      err: err && err.message ? err.message : err,
    });
    process.exit(1);
  });
