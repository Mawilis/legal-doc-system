...
export async function getLegalEmbeddingPipeline() {
  if (!legalEmbeddingPipeline) {
    try {
      ...
      const { pipeline } = await getTransformers();
      legalEmbeddingPipeline = await pipeline('feature-extraction', 'Xenova/legal-bert-base-uncased');
      logger.info('✅ Legal embedding pipeline initialized');
    } catch (error) {
      logger.warn('⚠️ Legal model not available, falling back to general model', { error: error.message });
