...
class RetentionAgenda {
  ...
  setupEventHandlers() {
    this.agenda.on('start', (_job) => {
      ...
      if (this.auditLedger) {
        this.auditLedger
          .logJobStart(job.attrs._id.toString(), job.attrs.name, job.attrs.data)
          .catch((_err) => this.logger.error('Failed to log job start:', err));
      }
    });
    ...
    this.agenda.on('complete', (_job) => {
      ...
      if (this.auditLedger) {
        this.auditLedger
          .logJobComplete(job.attrs._id.toString(), job.attrs.name, job.attrs.result)
          .catch((_err) => this.logger.error('Failed to log job completion:', err));
      }
    });
    ...
    this.agenda.on('fail', (err, job) => {
      ...
      if (this.auditLedger) {
        this.auditLedger
          .logJobFailure(job.attrs._id.toString(), job.attrs.name, err.message, job.attrs.data)
          .catch((_logErr) => this.logger.error('Failed to log job failure:', logErr));
      }
  ...
  async checkRetentionDeadlines(job, done) {
    ...
    try {
      ...
      const activeTenants = await Tenant.find({
        ...
        .select('_id name settings.retentionPolicies')
        .lean();
      ...
      for (let i = 0; i < activeTenants.length; i += concurrencyLimit) {
        ...
        const batchPromises = batch.map(async (_tenant) => {
          try {
            ...
            if (this.auditLedger) {
              await this.auditLedger
                ...
                .catch((_err) => this.logger.error('Failed to log error:', err));
            }
      ...
      this.logger.info(
        `[${jobId}] Retention check completed: ${processedTenants} tenants, ${totalDueRecords} records due`
      );
      ...
      if (this.auditLedger) {
        await this.auditLedger
          .logRetentionError('SYSTEM', 'RETENTION_CHECK_CRITICAL_FAILURE', error.message, {
            jobId,
            stack: error.stack,
          })
          .catch((_err) => this.logger.error('Failed to log critical error:', err));
      }
  ...
  async checkTenantRetention(tenant) {
    ...
    try {
      const Document =
        mongoose.models.Document || mongoose.model('Document', new mongoose.Schema({}));
    ...
    try {
      const AuditLedger =
        mongoose.models.AuditLedger || mongoose.model('AuditLedger', new mongoose.Schema({}));
  ...
  async executeTenantDisposal(job, done) {
    const { tenantId, tenantName, recordType, recordId, recordData, disposalMethod } =
      job.attrs.data;
    const jobId = job.attrs._id.toString();

    try {
      this.logger.info(
        `[${jobId}] Starting disposal: ${recordType} ${recordId} for tenant ${tenantName}`
      );
      ...
      await this.sendDisposalNotification(
        tenantId,
        tenantName,
        recordType,
        recordId,
      ...
      if (this.auditLedger) {
        await this.auditLedger
          .logDisposalFailure(tenantId, recordType, recordId, disposalMethod, error.message, {
            jobId,
            stack: error.stack,
          })
          .catch((_logErr) => this.logger.error('Failed to log disposal failure:', logErr));
      }
  ...
  async generateRecordHash(recordType, recordId) {
    try {
      ...
      return crypto
        .createHash('sha256')
        .update(`${recordType}:${recordId}:${Date.now()}`)
        .digest('hex');
    }
  }
  ...
  async sendDisposalNotification(tenantId, tenantName, recordType, recordId, disposalMethod) {
    try {
      // This would integrate with your notification service
      this.logger.info(
        `Disposal notification: ${recordType} ${recordId} disposed for tenant ${tenantName} using ${disposalMethod}`
      );
    } catch (_error) {
...
if (process.env.NODE_ENV === 'test' || typeof describe !== 'undefined') {
  ...
  describe('RetentionAgenda Tests', () => {
    ...
    test('should determine disposal method correctly', () => {
      ...
      expect(retentionWorker.determineDisposalMethod('Document', documentConfidential)).toBe(
        retentionWorker.config.disposalMethods.PERMANENT_DELETE
      );

      expect(retentionWorker.determineDisposalMethod('Document', documentFinancial)).toBe(
        retentionWorker.config.disposalMethods.ARCHIVE
      );

      expect(retentionWorker.determineDisposalMethod('Case', caseCriminal)).toBe(
        retentionWorker.config.disposalMethods.ARCHIVE
      );

      expect(retentionWorker.determineDisposalMethod('AuditLedger', {})).toBe(
        retentionWorker.config.disposalMethods.SOFT_DELETE
      );
    });
