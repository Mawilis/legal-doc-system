...
const DEFAULT_CONFIG = {
  connection: {
    ...
    retryStrategy: (_times) => Math.min(times * 50, 2000),
  },
  defaultJobOptions: {
    attempts: 3,
    backoff: {
...
class SarsFilingWorker {
      ...
      async (_job) => {
        ...
        tenantContext.runWithTenant(tenantId, async () => {
          ...
          try {
            ...
            const result = await this.sarsService.makePayment(submissionId, amount, paymentData);
  ...
  async queueStatusCheck(data, options = {}) {
    return this.queues.STATUS_CHECK.add(JOB_TYPES.CHECK_STATUS, data, {
  ...
  async queueAssessmentRetrieval(data, options = {}) {
    return this.queues.ASSESSMENT_RETRIEVAL.add(JOB_TYPES.RETRIEVE_ASSESSMENT, data, {
      priority: JOB_PRIORITIES.MEDIUM,
      ...options,
    });
  }
  ...
  async queuePaymentProcessing(data, options = {}) {
    return this.queues.PAYMENT_PROCESSING.add(JOB_TYPES.PROCESS_PAYMENT, data, {
      priority: JOB_PRIORITIES.HIGH,
      ...options,
    });
  }
  ...
  async shutdown() {
    ...
    // Close all workers
    await Promise.all(this.workers.map((_w) => w.close()));

    // Close all queues
    await Promise.all(Object.values(this.queues).map((_q) => q.close()));
