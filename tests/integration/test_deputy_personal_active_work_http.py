...
TITLE: WILSY OS Deputy Personal Work and Field Capability HTTP Certificate
VERSION: v1.1.0-L8-6D-DEPUTY-FIELD-CAPABILITY-HTTP-CERT
AUTHORITY: Direct HTTP certification of L8-6C/L8-6D deputy-only read routes.
EPITOME: Preserve personal active-work HTTP proof and certify the L8-6D field
         capability route derives tenant/principal scope from authenticated
         context, returns only bound-deputy opaque P2 locators plus state-valid
         command kinds, and never turns capability visibility into mutation IAM.
...
COLLABORATION / OWNERSHIP: HTTP composition certificate only; IAM owns caller
                            admission, L8-6B owns identity binding, L8-6C owns
                            personal active work, and L8-6D owns field-capability
                            composition. Command IAM remains independently enforced.
CERTIFICATION / UPDATE DATE: 2026-09-23
CHANGELOG: 2026-09-23 v1.1.0-L8-6D-DEPUTY-FIELD-CAPABILITY-HTTP-CERT
           adds exact deputy field-capability dispatch/response proof, missing
           binding 403, bounded locator/capability 503, no caller scope input,
           and router v1.5.0 binding while preserving L8-6C active-work tests.
           2026-09-23 v1.1.0-L8-6D-DEPUTY-FIELD-CAPABILITY-HTTP-CERT
...
AUTHORITY BOUNDARY: Deputy read transport/capability certificate only; capability is not lifecycle mutation authorization.
FINANCIAL AUTHORITY BOUNDARY: Kennel EOS remains exclusive.
FAIL-CLOSED DECLARATION: Missing IAM, binding, locator/capability evidence, or
                         persistence denies without sheriff-queue, command, or
                         fixture fallback.
"""
...
def _capability() -> FieldCommandCapabilityEntry:
    """Build one bounded ALLOCATED capability returned by L8-6D."""
    return FieldCommandCapabilityEntry(
        ...
        next_command_kinds=(FieldCommandKind.TRANSITION_TO_ATTEMPTED,),
    )
...
def test_authorized_deputy_field_capability_dispatch_is_server_scoped_and_bounded(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    lifecycle, binding = _Collection(), _Collection()
    calls: list[dict[str, Any]] = []
    ...
    with TestClient(_app(lifecycle, binding, authorize=True)) as client:
        response = client.get(
            "/api/legal-operations/deputy/field-capabilities"
        )
    ...
    assert body["deputy_id"] == DEPUTY
    assert body["capabilities"] == [_capability().to_dict()]
    serialized = str(body).casefold()
    for forbidden in (
        "principal_id",
        "office_receipt",
...
def test_field_capability_missing_binding_is_bounded_forbidden(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.parametrize(
    "code",
    (
        "L8_6D_SNAPSHOT_LOCATOR_UNAVAILABLE",
        "L8_6D_FIELD_CAPABILITY_INVALID",
        "L8_6C_ATTEMPT_EVIDENCE_UNAVAILABLE",
    ),
)
def test_field_capability_failures_are_bounded_without_command_fallback(
    monkeypatch: pytest.MonkeyPatch,
    code: str,
) -> None:
    def reject(**_kwargs: Any) -> object:
    ...
    assert (
        ...
        == "LEGAL_OPERATIONS_DEPUTY_FIELD_CAPABILITY_UNAVAILABLE"
    )
...
def test_route_permission_and_router_version_are_exact() -> None:
    ...
    assert "/legal-operations/deputy/field-capabilities" in paths
    ...
    assert VERSION == "v1.1.0-L8-6D-DEPUTY-FIELD-CAPABILITY-HTTP-CERT"


# ARTIFACT: test_deputy_personal_active_work_http.py
# VERSION: v1.1.0-L8-6D-DEPUTY-FIELD-CAPABILITY-HTTP-CERT
# AUTHORITY BOUNDARY: direct deputy-only personal active-work and field-capability HTTP certificate only
# TENANT POSTURE: tenant/principal are server-derived from authorized identity context
# FAIL-CLOSED POSTURE: missing IAM/binding/locator/capability/persistence denies without command or queue fallback
