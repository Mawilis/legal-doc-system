...
CHANGELOG: v1.0.0 certifies canonical role resolution and fail-closed repository behavior.
...
def test_each_active_canonical_role_resolves(role):
    result = resolve_current_tenant_business_role(principal_id="p", tenant_id="t", repository=Repo({role: Assignment(RoleAssignmentStatus.ACTIVE)}))
    assert result.role == role and result.resolution is BusinessRoleResolution.RESOLVED

def test_fail_closed_matrix():
    revoked = Repo({"tenant_owner": Assignment(RoleAssignmentStatus.REVOKED)})
    ...
    assert resolve_current_tenant_business_role(principal_id="p", tenant_id="t", repository=Repo({"ADMIN": Assignment(RoleAssignmentStatus.ACTIVE)})).role is None
    assert resolve_current_tenant_business_role(principal_id="p", tenant_id="t", repository=Repo({"tenant_owner": Assignment(RoleAssignmentStatus.ACTIVE), "tenant_admin": Assignment(RoleAssignmentStatus.ACTIVE)})).resolution is BusinessRoleResolution.MULTIPLE_ACTIVE_TENANT_BUSINESS_ROLES
    assert resolve_current_tenant_business_role(principal_id="p", tenant_id="t", repository=Repo({"tenant_owner": Assignment(RoleAssignmentStatus.ACTIVE), "tenant_admin": Assignment(RoleAssignmentStatus.REVOKED)})).role == "tenant_owner"
    ...
    assert resolve_current_tenant_business_role(principal_id="p", tenant_id="other", repository=Repo({"tenant_owner": Assignment(RoleAssignmentStatus.ACTIVE)})).role == "tenant_owner"
...
def test_exact_scope_and_non_authority():
    repo = Repo({"tenant_owner": Assignment(RoleAssignmentStatus.ACTIVE)})
